...
static const char entrypoint_file[] = "/mtd_rwarea/oipf_entrypoint";

void extract_iptv_entrypoint(const uint8_t* temp)
{
    uint32_t enterprise_number = -1;
    uint8_t sub_option_number = -1;
    uint8_t enc_field = -1;
    ...
    enterprise_number = (temp[0]<< 24) + (temp[1]<< 16) + (temp[2]<< 8) + temp[3];
	printf("enterprise_number:%d\n", enterprise_number);

    sub_option_number = temp[5];
	printf("sub_option_number:%d\n", sub_option_number);

    if (sub_option_number != 1 /*IPTV_ENTRYPOINT*/)
    {
		printf("extract_iptv_entrypoint: unsupported sub_option_number (%d)\n", sub_option_number);
		return;
    }
	...
	remove(entrypoint_file);
    switch(enc_field)
    {
	    case 0x01: //IP address
	    {
            FILE * pFile = fopen(entrypoint_file, "w");

            fprintf(pFile, "%d.%d.%d.%d", temp[8], temp[9], temp[10], temp[11]);
            printf("IPTV entrypoint found (IP address):\n");
            printf("%d.%d.%d.%d\n", temp[8], temp[9], temp[10], temp[11]);
            fclose(pFile);
	        break;
	    }
	    case 0x02: //Fully-Qualified Domain Name
        {
            FILE * pFile = fopen(entrypoint_file, "w");
		    uint8_t sub_option_len = temp[6];
  			uint8_t* iptv_entrypoint = xmalloc( (sub_option_len-1)*sizeof(uint8_t));

            memset(iptv_entrypoint, 0, sub_option_len-1);
            memcpy(iptv_entrypoint, temp + 8, sub_option_len-1);
            iptv_entrypoint[sub_option_len-1] = '\0';

            fprintf(pFile, "%s", iptv_entrypoint);
            printf("IPTV entrypoint found (FQDN address):\n");
            printf("%s\n", iptv_entrypoint);
            fclose(pFile);

            free(iptv_entrypoint);
	        break;
        }
	    default:
        {
			printf("extract_iptv_entrypoint: unsupported enc_field (%d)\n", enc_field);
	        break;
		}
	}
}
	...
	*curr = xmalloc(sizeof("ip=255.255.255.255"));
	...
	while (*opt_name) {
		...
		*found_ptr &= ~found_mask; /* leave only unknown options */
		...
		/* Analyse option 125 */		
		if (code == DHCP_VENDOR_SP) 
		{
			extract_iptv_entrypoint(temp);
			goto next;
		}
...
static NOINLINE int send_discover(uint32_t xid, uint32_t requested)
{
	...
	uint8_t option124_oipf[17]={DHCP_VENDOR_CL, //option 124
								14,	 			//option full length
								0,  			//Samsung Electronics identifier
								0,  			//Samsung Electronics identifier
								0,  			//Samsung Electronics identifier
								236,	 		//Samsung Electronics identifier
								9, 	 			//length of vendor specific class
								'O','I','T','F','_','I','P','T','V'};
	...
	udhcp_add_binary_option(&packet, option124_oipf);
