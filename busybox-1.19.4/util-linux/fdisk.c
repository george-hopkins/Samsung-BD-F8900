...
struct globals {
	...
	sector_t minimum_io_size, alignment_offset;
...
#define minimum_io_size 		(G.minimum_io_size)
#define alignment_offset 		(G.alignment_offset)
...
warn_limits(void) {
        if (total_number_of_sectors > UINT_MAX && !nowarn) {
                ...
                fprintf(stderr, "\n"
"WARNING: The size of this disk is %d.%d TB (%u bytes).\n"
...
get_topology() {
	...
	if (user_set_sector_size)
		return;
	pr = blkid_new_probe();
	if (pr && blkid_probe_set_device(pr, dev_fd, 0, 0) == 0) {
 ...
 */
void update_sector_offset(void)
{
        if (dos_compatible_flag) {
                 ...
                 */
                if (g_sectors && alignment_offset && !lba_is_aligned(g_sectors))
                        fprintf(stderr, 
...
#if ENABLE_FEATURE_FDISK_WRITABLE
static sector_t
read_int_sx(sector_t low, sector_t dflt, sector_t high, sector_t base, const char *mesg, int *suffix)
{
	sector_t value;
	int default_ok = 1;
...
*/
static sector_t 
read_int(sector_t low, sector_t dflt, sector_t high,
...
static void
check_alignment(const struct partition *p, int partition)
{
        if (!lba_is_aligned(get_start_sect(p)))
                printf("Partition %i does not start on physical block boundary.\n",
                        partition + 1);
}
...
list_disk_geometry(void)
{
	...
	printf("\nUnits = %s of %u * %u = %u bytes\n\n",
        ...
        printf("Sector size (logical/physical): %u bytes / %u bytes\n",
                                sector_size, minimum_io_size);
        if (alignment_offset)
                printf("Alignment offset: %u bytes\n", alignment_offset);
}
...
add_partition(int n, int sys)
{
	...
	do {
		...
		if (!num_read && start == temp) {
			...
			saved_start = start;
			start = read_int(cround(saved_start), cround(dflt), cround(limit), 0, mesg);
			if (display_in_cyl_units) {
	...
	if (cround(start) == cround(limit)) {
		...
		snprintf(mesg, sizeof(mesg),
			 ...
			 str_units(SINGULAR));
		stop = read_int_sx(cround(start), cround(limit), cround(limit), cround(start), mesg, &sx);
		if (display_in_cyl_units) {
