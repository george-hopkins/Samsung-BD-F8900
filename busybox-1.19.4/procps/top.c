...
static unsigned long display_header(int scr_width, int *lines_rem_p, int isVtp)
{
	FILE *fp;
	char buf[80];
	char scrbuf[80];
...
static NOINLINE void display_process_list(int lines_rem, int scr_width, int isVtp)
{
	enum {
	...
	top_status_t *s;
	char vsz_str_buf[8];
	unsigned long total_memory = display_header(scr_width, &lines_rem, isVtp); /* or use total_vsz? */
...
# define FMT "%4u%%"
	...
	s = top;
       if(isVtp){
               top5 = lines_rem - 5; // for CPU top 5
       }
	while (--lines_rem >= 0) {
		unsigned col;
        ...
        else
        {
            ...
            {
#if ENABLE_FEATURE_TOP_CPU_USAGE_PERCENTAGE
                snprintf(line_buf, scr_width, FMT " ", SHOW_STAT(pcpu));
#endif
                if ((int)(col + 1) < scr_width)
                    read_cmdline(line_buf + col, scr_width - col - 1, s->pid, s->comm);
                fputs(line_buf, stdout);
...
int top_main(int argc UNUSED_PARAM, char **argv)
{
	...
	cpu_prev_jif = &prev_jif;
#endif
       if(argv[3] && (!strcmp("VTP", argv[3]))){ // argv[3] NULL check & argv[3] string check
               vtp = 1;
       }
       else{
               vtp = 0;
	...
	col = getopt32(argv, "d:n:bVTP"IF_FEATURE_TOPMEM("m"), &str_interval, &str_iterations);
#if ENABLE_FEATURE_TOPMEM
	...
	if (col & OPT_n) {
       ...
       /* We are displaying VTP statistics, so disable TOPMEM support */
       if(vtp == 1)
               scan_mask = TOP_MASK;
	/* change to /proc */
	xchdir("/proc");
...
#endif
		if (scan_mask != TOPMEM_MASK || vtp == 1)
			display_process_list(lines, col, vtp);
#if ENABLE_FEATURE_TOPMEM
		else
