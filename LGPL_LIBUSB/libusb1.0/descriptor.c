...
static void clear_interface(struct libusb_interface *interface)
{
	...
	if (interface->altsetting) {
		for (i = 0; i < interface->num_altsetting; i++) {
			...
			if (ifp->extra)
				LibUsbFreeMem(ifp->extra);
			if (ifp->endpoint) {
				for (j = 0; j < ifp->bNumEndpoints; j++)
					clear_endpoint((struct libusb_endpoint_descriptor *)
						ifp->endpoint + j);
				LibUsbFreeMem(ifp->endpoint);
			}
		}
		LibUsbFreeMem(interface->altsetting);
	...
	if (config->interface) {
		...
		for (i = 0; i < config->bNumInterfaces; i++)
			clear_interface((struct libusb_interface *)
				config->interface + i);
		LibUsbFreeMem(config->interface);
	}
	if (config->extra)
		LibUsbFreeMem(config->extra);
}
