...
static int sysfs_can_relate_devices = 0;   // Default enable for usbfs.

/* do we have a descriptors file? */
static int sysfs_has_descriptors = 0;// Default enable for usbfs.
...
#if defined(REQUIREMENT_USB_PORT_SUPPORT)

#define MAX_USB_PORT		    10
...
typedef enum
{
	...
	UsbBusEight,
	MaxUsbBusSupported
}UsbBusSupported_t;    				/*Add here  per supported bus requirement.*/
...
typedef enum
{
	...
	UsbPortThree,
	UsbPortFour,
	MaxUsbPortSupported				/*Add here  per supported port requirement.*/
}UsbPortSupported_t;					
...
typedef struct
{
	UsbPortInfo_t UsbPortInfo[MAX_USB_PORT];
}UsbDeviceFileInfo_t;

static UsbDeviceFileInfo_t UsbDeviceFileInfo;

/*This function Copies the sub string*/
static void substrcpy(char *dest, char *src, int position, int length)
{
	dest[0]='\0';
...
/*This function Parse Proc use device file*/

static int ParseProcUsbDevFile (UsbDeviceFileInfo_t *pUsbDeviceFileInfo) 
{
	...
	/*Store the devices proc usb device info in File*/
	system("cat /proc/bus/usb/devices > Devices.txt");
    in = fopen( "Devices.txt", "r" );
	memset(pUsbDeviceFileInfo, -1, sizeof(UsbDeviceFileInfo_t));
	/*Now Parse the file*/
    while ( fgets( buff, BUFSIZ, in ) != NULL ) 
	{
		...
		{
            strcpy( newbuff, buff );
			substrcpy(tmpbuff,buff,8,2);
			pUsbDeviceFileInfo->UsbPortInfo[indx].Bus = atoi(tmpbuff);         /*Bus Info*/
			substrcpy(tmpbuff,buff,15,2);
			pUsbDeviceFileInfo->UsbPortInfo[indx].Level = atoi(tmpbuff);		/*Level Info is basicaly Port info*/
			substrcpy(tmpbuff,buff,23,2);
			pUsbDeviceFileInfo->UsbPortInfo[indx].Prnt = atoi(tmpbuff);			/*Prnt Info*/
			substrcpy(tmpbuff,buff,31,2);
			pUsbDeviceFileInfo->UsbPortInfo[indx].Port = atoi(tmpbuff);			/*Port Info*/
			substrcpy(tmpbuff,buff,38,2);
			pUsbDeviceFileInfo->UsbPortInfo[indx].Cnt = atoi(tmpbuff);			/*Cnt Info*/
			substrcpy(tmpbuff,buff,46,3);
			pUsbDeviceFileInfo->UsbPortInfo[indx].Dev = atoi(tmpbuff);			/*Dev Info*/
			indx++;
        } 
    }
    fclose( in );
...
/*This function checks the Usb bus supported*/
static int CheckIfUsbBusSupported(UsbDeviceFileInfo_t *pUsbDeviceFileInfo,int *pbusnum)
{
	   int indx;
	   int UsbPortSupportFlag = -1;
	   for(indx = 0; indx < MAX_USB_PORT;indx++)
	   {
		/*More check cases can be added here as per supported bus requirement*/
		if((pUsbDeviceFileInfo->UsbPortInfo[indx].Bus == UsbBusTwo))
		{
			 *pbusnum = pUsbDeviceFileInfo->UsbPortInfo[indx].Bus;
			 UsbPortSupportFlag = 0;
		}
		
	   }

   return UsbPortSupportFlag;
}

/*This function checks the Usb port supported*/
static int CheckIfUsbPortSupported(UsbDeviceFileInfo_t *pUsbDeviceFileInfo,int *pPortnum)
{
	   int indx;
	   int UsbPortSupportFlag = -1;
	   for(indx = 0; indx < MAX_USB_PORT ;indx++)
	   {
		/*More check cases can be added here as per supported ports requirement*/
		if((pUsbDeviceFileInfo->UsbPortInfo[indx].Level == UsbPortTwo))
		{
			*pPortnum = pUsbDeviceFileInfo->UsbPortInfo[indx].Level;  /*Level is port number*/
			UsbPortSupportFlag = 0;
		}
	...
	unsigned char *buffer)
{
	...
	snprintf(filename, PATH_MAX, "%s/%s/%s",
			 SYSFS_DEVICE_PATH, priv->sysfs_dir, "descriptors");

	 /* Check file existence. */
      rval = access (filename, F_OK);
      if(rval != 0)
      	{
	   usbi_err(DEVICE_CTX(dev),"File[%s] Not Exist",filename);	  
	   return LIBUSB_ERROR_IO;
	}
	...
	rval = access (filename, R_OK);
      if(rval != 0)
      	{
	   usbi_err(DEVICE_CTX(dev),"File[%s] No Read Permission%s",filename);	   
	   return LIBUSB_ERROR_IO;
	}
	...
	{	
		usbi_err(DEVICE_CTX(dev),"sysfs descriptors not available");		
		return LIBUSB_ERROR_IO;
	}	  
	/*Only after confirming above steps open and read descriptor file*/
	...
	if (r < 0) {
      ...
      fprintf(stderr,"Descriptor File[%s] read SUCCESS\n",filename);
	return 0;
}
	...
	unsigned char *buffer, size_t len)
{
	...
	if (len > sizeof(tmp)) {
		...
		} else if (r < (ssize_t)(len - sizeof(tmp))) {
	...
	int active_config)
{
	...
	if (priv->config_descriptor)
		LibUsbFreeMem(priv->config_descriptor);
	...
	uint8_t devaddr, const char *sysfs_dir)
{
	...
	if (sysfs_dir) {
		...
		if (!priv->sysfs_dir)
			return LIBUSB_ERROR_NO_MEM;
		/*Prevent Tool Minor Bug FIX*/
		strncpy(priv->sysfs_dir, sysfs_dir, strlen(sysfs_dir) + 1);
	}
	...
	if (!sysfs_can_relate_devices) {
		if (active_config == -1) {
			...
			if (active_config < 0) {
#ifdef	PATCH_FOR_BAD_CONFIG
				 usbi_dbg("failed to read active configuration because buggy device or abnormal status of device");  
				 device_configured = 0;
#else
				close(fd);
				return active_config;
	...
	struct discovered_devs **_discdevs, uint8_t busnum)
{
	...
	for(r_r = readdir_r(dir, &entry, &result); result != NULL && r_r == 0; r_r = readdir_r(dir, &entry, &result)) {
		int devaddr;
...
#if defined(REQUIREMENT_USB_PORT_SUPPORT)
		if(0!= CheckIfUsbPortSupported(&UsbDeviceFileInfo,&devaddr))
		{
			usbi_dbg("usb port not supported");
			goto out;
		}
#endif
...
#if defined(REQUIREMENT_USB_PORT_SUPPORT)
/*Parse /proc/usb/bus/devices file*/
ParseProcUsbDevFile(&UsbDeviceFileInfo);
/*
{
	int indx=0;
	printf("===============================================================\n");
	for(indx = 0; indx < MAX_USB_PORT ;indx++)
	{
		printf("Indx = %d Bus=%d Lev=%d Prnt =%d Port =%d Cnt =%d Dev =%d\n",indx,UsbDeviceFileInfo.UsbPortInfo[indx].Bus,UsbDeviceFileInfo.UsbPortInfo[indx].Level,UsbDeviceFileInfo.UsbPortInfo[indx].Prnt,UsbDeviceFileInfo.UsbPortInfo[indx].Port,UsbDeviceFileInfo.UsbPortInfo[indx].Cnt,UsbDeviceFileInfo.UsbPortInfo[indx].Dev);
	}
	printf("===============================================================\n");
}
*/
	...
	for(r_r = readdir_r(buses, &entry, &result); result != NULL && r_r == 0; r_r = readdir_r(buses, &entry, &result)) {
...
#if 0 //defined(REQUIREMENT_USB_PORT_SUPPORT)  

		if(0 != CheckIfUsbBusSupported(&UsbDeviceFileInfo,&busnum))
		{
			usbi_dbg("Usb Bus Not Supported");
			goto out;
		}
#endif	

#if defined(USB_PORT_ONLY)
		if(busnum != 2) // 2-1.2~4 are user usb port. so scan only bus #2.
		{
			usbi_dbg("dir entry %s not support in this platform", entry.d_name);	// The genoa platform can be check for usb bus #2 only.
			continue;
		}
		else
		{
			usbi_dbg("supported USB Host controller entry ->  %s", entry.d_name);
		}
#endif
	...
	int *usbfs_fallback)
{
...
#if 1
                        /*FIX::Sysopenfs Error*/
                        /*Calling usbfs_get_device_list creates NULL Device problem*/
			sysfs_can_relate_devices = 1;
			printf("\n!!!!!!!!!!!!FIX for sysfs open error !!!!!!!!!!!!!!\n");
#else
			sysfs_can_relate_devices = 0;
			*usbfs_fallback = 1;
#endif			
	...
	sysfs_can_relate_devices = 1;
         /*Prevent Tool Minor Bug FIX*/
	r = fread( (char *)&busnum, 1, sizeof(int), fd);
	fclose(fd);
	if(r > 0)
	{
	   busnum = atoi(( char *)&busnum);
	}
	else
	{
		usbi_err(ctx, "fread busnum %d returned %d, errno=%d", busnum, r, errno);
		return LIBUSB_ERROR_IO;
	}
	...
	if (!fd) {
        ...
        /*Prevent Tool Minor Bug FIX*/ 
	r = fread( (char *)&devaddr, 1, sizeof(int), fd);
	fclose(fd);
	if(r > 0)
	{
		devaddr = atoi(( char *)&devaddr);
	}
	else
	{
		usbi_err(ctx, "fread devaddr %d returned %d, errno=%d", devaddr, r, errno);
		return LIBUSB_ERROR_IO;
	}
        /*Prevent Tool Minor Bug FIX*/
	struct stat statbuf;
	...
	struct discovered_devs **_discdevs, int *usbfs_fallback)
{
	...
	for(r_r = readdir_r(devices, &entry, &result); result != NULL && r_r == 0; r_r = readdir_r(devices, &entry, &result)) {
		struct discovered_devs *discdevs_new = discdevs;
...
static void op_destroy_device(struct libusb_device *dev)
{
	...
	if (!sysfs_has_descriptors) {
		if (priv->dev_descriptor)
			LibUsbFreeMem(priv->dev_descriptor);
		if (priv->config_descriptor)
			LibUsbFreeMem(priv->config_descriptor);
	}
	if (priv->sysfs_dir)
		LibUsbFreeMem(priv->sysfs_dir);
}
...
static int submit_iso_transfer(struct usbi_transfer *itransfer)
{
	...
	for (i = 0; i < num_packets; i++) {
		...
		if (packet_len > (unsigned int)space_remaining) {
			num_urbs++;
	...
	for (i = 0; i < num_urbs; i++) {
		...
		while (packet_offset < transfer->num_iso_packets) {
			...
			if (packet_len <= (unsigned int)space_remaining_in_urb) {
				/* throw it in */
				urb_packet_offset++;
				packet_offset++;
	...
	struct usbfs_urb *urb)
{
	...
	if (tpriv->reap_action != NORMAL) {
		...
		if (tpriv->num_retired == (unsigned int)num_urbs) {
