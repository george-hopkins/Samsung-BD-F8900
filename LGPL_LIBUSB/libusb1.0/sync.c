	...
	unsigned char *data, uint16_t wLength, unsigned int timeout)
{
	...
	libusb_fill_control_setup((struct libusb_control_setup *)buffer, bmRequestType, bRequest, wValue, wIndex,
		wLength);
	if ((bmRequestType & LIBUSB_ENDPOINT_DIR_MASK) == LIBUSB_ENDPOINT_OUT)
		memcpy(buffer + LIBUSB_CONTROL_SETUP_SIZE, data, wLength);

	libusb_fill_control_transfer(transfer, dev_handle,(struct libusb_control_setup *) buffer,
