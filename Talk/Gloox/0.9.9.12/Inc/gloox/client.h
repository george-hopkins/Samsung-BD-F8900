...
namespace gloox
{
  ...
  class GLOOX_API Client : public ClientBase
  {
    public:
       ...
       */
      void setResource( const std::string &resource1 ) { m_jid.setResource( resource1 ); }
	  ...
	  void setSharedStatus( int shared_presence, const std::string& shared_status );

	  void getSharedStatus();
    ...
    private:
      ...
      void sendPresence();
	  void sendSharedStatus();
	  void requestSharedStatus();
      void createSession();
      ...
      Presence m_presence;
      std::string m_status;
	  std::string m_shared_status;
	  int m_shared_presence;
