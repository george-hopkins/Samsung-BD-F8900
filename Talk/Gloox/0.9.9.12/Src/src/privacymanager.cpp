...
namespace gloox
{
  ...
  std::string PrivacyManager::requestBlockList()
  {
	const std::string& id = m_parent->getID();
	...
	q->addAttribute( "xmlns:gr", "google:roster");
	q->addAttribute( "gr:ext", "2");
	
	m_parent->trackID( this, id, PLRequestBlockList );
	m_parent->send( iq );
	return id;
  }  
  ...
  std::string PrivacyManager::setBlock( const std::string& jid )
  {
    const std::string& id = m_parent->getID();
	...
	q->addAttribute( "xmlns:gr", "google:roster");
	q->addAttribute( "gr:ext", "2");
    Tag *a = new Tag( q, "item" );
    a->addAttribute( "jid", jid );
	a->addAttribute( "gr:t", "B" );

    m_parent->trackID( this, id, PLBlock );
    m_parent->send( iq );
    return id;
  }

  std::string PrivacyManager::unsetBlock( const std::string& jid )
  {
    const std::string& id = m_parent->getID();
    ...
    Tag *a = new Tag( q, "item" );
    a->addAttribute( "jid", jid );
	a->addAttribute( "gr:t", "P" );


    m_parent->trackID( this, id, PLUnsetBlock );
    m_parent->send( iq );
    return id;
  }
      ...
      case StanzaIqResult:
        switch( context )
        {
          ...
          case PLRemove:
            ...
            break;
		  case PLBlock:
			m_privacyListHandler->handlePrivacyListResult( stanza->id(), ResultBlockSuccess);
			break;
		  case PLUnsetBlock:
			m_privacyListHandler->handlePrivacyListResult( stanza->id(), ResultUnsetBlockSuccess);
			break;						
          case PLRequestNames:
          {
            StringList lists;
          ...
          case PLRequestBlockList:
          {
            ...
            for( ; it != l.end(); ++it )
            {
        	const std::string& block1 = (*it)->findAttribute( "gr:t" ); /* modified by k.pradip on request of SungJoo Kim<hikaru@samsung.com>*/
		const std::string& block2 = (*it)->findAttribute( "ros:t" ); /* modified by k.pradip on request of SungJoo Kim<hikaru@samsung.com>*/
		const std::string& jid = (*it)->findAttribute( "jid" );
		if( block1 == "B" || block2 == "B" )/* modified by k.pradip on request of SungJoo Kim<hikaru@samsung.com>*/
		{
		   lists.push_back( jid );
		}
            }

            m_privacyListHandler->handleBlockList( lists );
            break;		
          }		  
        }
        break;
