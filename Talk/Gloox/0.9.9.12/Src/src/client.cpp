...
//       }
  ...
  void Client::setSharedStatus( int shared_presence, const std::string& shared_status )
  {
    m_shared_status = shared_status;
	m_shared_presence = shared_presence;

    sendSharedStatus();
  }

  void Client::getSharedStatus()
  {
    requestSharedStatus();
  }  
  ...
  void Client::sendSharedStatus()
  {
  		const std::string& id = this->getID();

		Tag *iq = new Tag( "iq" );	  
		iq->addAttribute( "type", "set" );	 
		//iq->addAttribute( "to", "glooxuser@gmail.com/gloox" );	 
		iq->addAttribute( "id", id );	 
		Tag *q = new Tag( iq, "query" );	 
		q->addAttribute( "xmlns", "google:shared-status");	
		q->addAttribute( "version", "2");
		Tag *s = new Tag( q, "status", m_shared_status);
		switch (m_shared_presence)
		{
			case 0:
			{
				Tag *a = new Tag( q, "show", "default");
				break;
			}	
			case 1:
			{
				Tag *a = new Tag( q, "show", "dnd");
				break;
			}	
			default:
			{
...
/*
		Tag *sl1 = new Tag( q, "status-list" );
		sl1->addAttribute( "show", "default" );
		Tag *sl11 = new Tag( sl1, "status", "test1");
		Tag *sl12 = new Tag( sl1, "status", "test2");	

		Tag *sl2 = new Tag( q, "status-list" );
		sl2->addAttribute( "show", "dnd" );
		Tag *sl21 = new Tag( sl2, "status", "test3");
		Tag *sl22 = new Tag( sl2, "status", "test4");	
*/		
		Tag *i = new Tag( q, "invisible");
		i->addAttribute( "value", "false");

		send( iq );	
  }

  void Client::requestSharedStatus()
  {
  		const std::string& id = this->getID();

		Tag *iq = new Tag( "iq" );	  
		iq->addAttribute( "type", "get" );	 
		//iq->addAttribute( "to", "glooxuser@gmail.com" );
		iq->addAttribute( "id", id );	 
		Tag *q = new Tag( iq, "query" );	 
		q->addAttribute( "xmlns", "google:shared-status");	 
		q->addAttribute( "version", "2");
		...
		Tag *iq = new Tag( "iq" );	  
		iq->addAttribute( "type", "get" );	 
		iq->addAttribute( "to", "gmail.com" );	 	 
		Tag *q = new Tag( iq, "query" );	 
		q->addAttribute( "xmlns", "http://jabber.org/protocol/disco#info");	
#endif
		send( iq );	
  }  
