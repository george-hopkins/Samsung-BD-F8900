...
//                                    m_stats.compression );
  ...
  void ClientBase::notifyIqHandlers( Stanza *stanza )
  {
	...
	if(q!=NULL )
	{
		const std::string& xmls = q->findAttribute( "xmlns" );
		if( xmls == "google:shared-status")
		{
			std::string status = "";
			std::string show = "default";
				
			const Tag::TagList& l = q->children();
			Tag::TagList::const_iterator it_s = l.begin();
			for( ; it_s != l.end(); ++it_s )
			{
			  const std::string& name = (*it_s)->name();
			  if( name == "status" )
			  {
				status = (*it_s)->cdata();
			  } 
			...
			printf("\n@@@@@@@ status is [%s], show is [%s] @@@@@@@\n", status.c_str(), show.c_str());

			PresenceHandlerList::const_iterator it = m_presenceHandlers.begin();
			if(stanza->hasAttribute("type", "result"))
			{
				printf("\n-------google shared-status get--------\n");
				for( ; it != m_presenceHandlers.end(); ++it )
				{
				  (*it)->handleSharedStatus( 1, status, show );
				}
			}
			else if (stanza->hasAttribute("type", "set") )
			{
				printf("\n-------google shared-status set--------\n");
				for( ; it != m_presenceHandlers.end(); ++it )
				{
				  (*it)->handleSharedStatus( 2, status, show );
				}
			}
			else
			{
			...
			if (stanza->hasAttribute("type", "result") )
			{
				...
				for( ; it_s != l.end(); ++it_s )
				{
				  ...
				  if( name == "status" )
				  {
				    ...
				    for( ; it != m_presenceHandlers.end(); ++it )
				    {
				      (*it)->handleSharedStatus( 1, status );
				    }
			...
			else if (stanza->hasAttribute("type", "set") )
			{
				...
				for( ; it_s != l.end(); ++it_s )
				{
				  ...
				  if( name == "status" )
				  {
				    ...
				    for( ; it != m_presenceHandlers.end(); ++it )
				    {
				      (*it)->handleSharedStatus( 2, status );
				    }
