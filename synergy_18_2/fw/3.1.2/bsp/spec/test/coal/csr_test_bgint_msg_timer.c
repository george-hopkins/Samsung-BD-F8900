...
int srv_handle_tc_bgint_msg_timer(void **gash)
{
    ...
    else if (event == TEST1_PRIM)
    {
        ...
        {
            test_bgint_unregister(inst);
            /* We're stopping, so no more messages expected */
            set_tc_verdict(TC_FAILED);
            ret = TC_FINISHED;
        }
    }
...
static void timeout(CsrUint16 i, void *arg)
{
    BgintTestInstanceData *inst;
...
void srv_start_tc_bgint_msg_timer(void **gash)
{
    BgintTestInstanceData *inst;
