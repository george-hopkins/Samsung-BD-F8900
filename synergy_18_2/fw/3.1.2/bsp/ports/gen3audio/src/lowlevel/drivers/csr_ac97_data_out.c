...
{
    DMA_SOURCE_ADDRESS_SET(dma - 1, data);
    DMA_SOURCE_CONTROL_SET(dma - 1, 0, 0);
    DMA_DESTINATION_ADDRESS_SET(dma - 1, &rAC_PCMDATA);
    DMA_DESTINATION_CONTROL_SET(dma - 1, 0, 1, 1);
    DMA_CON_SET(dma - 1, DMA_CON_HANDSHAKE | DMA_CON_SYNC_APB | DMA_CON_INT | DMA_CON_SINGLESERVICE | DMA_CON_HARDTRIGGER | DMA_CON_NOAUTORELOAD | DMA_CON_WORD, 4 + dma, length);
    DMA_ON(dma - 1);
}
