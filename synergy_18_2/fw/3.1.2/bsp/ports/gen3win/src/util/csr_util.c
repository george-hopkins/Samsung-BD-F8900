...
{
    CsrUint16 size=0;
    StringCchLengthA(src,4096,&size);
    StringCchCopyA(dest, size+1, src);
 /*
     char *cp = dst;
     while (*cp++ = *src++);
     return dst;
