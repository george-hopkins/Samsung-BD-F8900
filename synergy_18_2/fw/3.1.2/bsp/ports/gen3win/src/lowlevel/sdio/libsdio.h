 ...
 *
 * libsdio also provides a compatible implementation of the CSR
 * Synergy SDIO API.  This must be initialized by calling
 * CsrSdioModuleInit() and then cards may be added with
 * CsrSdioCardAdd().
 */
/*@{*/
...
int LIBSDIOAPI sdio_write_async(sdio_uif_t uif, int func, CsrUint32 addr, const CsrUint8 *data,
    ...
    sdio_io_callback_t callback, void *arg);
/**
 * Perform a hard reset of the card (if possible) and reinitialze the
 * card.
 *
 * The will do a hard reset of the card (possibly implemented as a
 * power cycle) and reinitialize the card to its default operational
 * state with no functions or interrupts enabled.
 *
 * @param uif device handle.
 *
 * @return SDIO_SUCCESS on success.
 * @return SDIO_ENOTSUPP if a hard reset is not possible.
 * @return Any other error if the reinitialization failed.
 */
int LIBSDIOAPI sdio_hard_reset(sdio_uif_t uif);

struct CsrSdioCardImpl;
typedef struct CsrSdioCardImpl CsrSdioCard;
