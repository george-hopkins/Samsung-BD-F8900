...
static void rxThreadFunc(void *handle)
{
    ...
    while (TRUE)
    {
        ...
        if (startRead)
        {
            ...
            readSuccess = ReadFile(comHandle[idx], rxData, numberToRead, &bytesRead,NULL);
            if (!readSuccess)
            {
                asyncRead = TRUE;
                 printf("read err\n");
...
static CsrBool init(CsrUint8 idx, CsrUint32 baud)
{
        ...
        (dataReadEvent[idx] == NULL))
    {
     ...
     wFileName = CsrUtf82Utf16String(instDevice[idx]);

    /*    create the file handle */

//    comHandle[idx] = CreateFile(instDevice[idx], GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
	comHandle[idx] = CreateFile(wFileName, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
...
CsrBool CsrUartDrvStop(void *handle)
{
    ...
    do
    {
        ...
        else
        {
            if (threadExitCode == STILL_ACTIVE)
            {
             //   printf("GetExitCodeThread aaaaaaaa!\n");
                Sleep(1);
            }
        }
    } while (threadExitCode == STILL_ACTIVE);
    ...
    const CsrUint8 *token)
{
    ...
    for (idx = 0; idx < NUM_SIMULTANEOUS_UARTS; idx++)
    {
        if (!instInUse[idx])
        {
            ...
            CsrStrCpy(instDevice[idx],device);
