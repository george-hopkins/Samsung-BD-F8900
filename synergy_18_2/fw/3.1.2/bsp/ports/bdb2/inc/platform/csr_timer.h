...
#define TIMER2_INVERT_ON()              BIT_SET(rTCON, 14)
#define TIMER2_INVERT_OFF()             BIT_CLEAR(rTCON, 14)
...
#define TIMER3_INVERT_ON()              BIT_SET(rTCON, 18)
#define TIMER3_INVERT_OFF()             BIT_CLEAR(rTCON, 18)
...
/*******************************************************************************
    ...
    DESCRIPTION
        ...
        will be limited by the current range setting of Timer3 plus one (default
        is TCNTB3=9999us). Furthermore, the value returned may be offset by a
        value equal to TCNTB3 + 1.

        Note: When using Nucleus GetTime will automatically adjust itself to the
        period of Timer3 (determined by the TCNTB3 register), but the counting
