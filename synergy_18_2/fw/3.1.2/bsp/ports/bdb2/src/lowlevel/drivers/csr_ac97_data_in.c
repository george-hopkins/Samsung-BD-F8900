...
#define AC97_MODE_OFF()     REG_SET(rAC_GLBCTRL, 11, 10, 0)
#define AC97_MODE_DMA()     REG_SET(rAC_GLBCTRL, 11, 10, 0x3)
...
{
    DMA_SOURCE_ADDRESS_SET(dma, &rAC_PCMDATA);
    DMA_SOURCE_CONTROL_SET(dma, 1, 1);
    DMA_DESTINATION_ADDRESS_SET(dma, data);
    DMA_DESTINATION_CONTROL_SET(dma, 0, 0, 0);
    DMA_CON_SET(dma, DMA_CON_HANDSHAKE | DMA_CON_SYNC_APB | DMA_CON_INT | DMA_CON_SINGLESERVICE | DMA_CON_HARDTRIGGER | DMA_CON_NOAUTORELOAD | DMA_CON_WORD, 4 + dma, length);
    DMA_ON(dma);
}
...
static void eventDMA(void)
{
    ...
    MMUInvalidateDCacheRegion((CsrUint32) pendingList->data, 4 * pendingList->length);
...
CsrInt32 AC97PcmInputStart(CsrUint8 dmaChannel)
{
    ...
    DMARegisterHandler(dma, eventDMA);

    return AC97_RESULT_SUCCESS;
}

CsrInt32 AC97PcmInputStop()
{
    if (state != AC97_STATE_DISABLE)
    {
        CsrInt32 i;
...
CsrInt32 AC97PcmInputPause(void)
{
    if (state == AC97_STATE_TRANSFER)
    {
        ...
        if (pendingList != NULL)
        {
            pendingList->current = DMA_CURRENT_SOURCE_GET(dma);
