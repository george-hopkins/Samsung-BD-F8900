    ...
    void *privArg)
{
    ...
    /* Restore whatever interrupt setting was set before. */
    intrLevel = NU_Local_Control_Interrupts(intrLevel);
...
static void csrAclBufBatchFull(void)
{
    ...
    for (s = subList; s != NULL; s = s->next)
    {
        CsrAclBufElmImpl **bq, **nbq;
        int i;

        nbq = CsrPmemZalloc(ACLBUF_BATCHS_IN_POOL * sizeof(CsrAclBufElmImpl *));
        bq = s->batchQueue;
        s->batchQueue = nbq;
        ...
        for (i = 0; i < ACLBUF_BATCHS_IN_POOL; i++)
        {
            if (bq[i] != NULL)
            {
                s->bufReady((CsrAclBufElm *) bq[i], s->privArg);
            }
        }
        CsrPmemFree(bq);
        intrLevel = NU_Local_Control_Interrupts(NU_DISABLE_INTERRUPTS);
    ...
    CsrUint16 size)
{
    ...
    {
        /* Make sure active batch is terminated. */
    ...
    CsrUint8 **data)
{
    ...
    {
        CsrInt32 dataSize;
        /* If new start packet before fragment complete, deliver fragments received so far */
        if (elm->isFragment)
        {
            s->freePool = elm->next;
            elm->next = NULL;
