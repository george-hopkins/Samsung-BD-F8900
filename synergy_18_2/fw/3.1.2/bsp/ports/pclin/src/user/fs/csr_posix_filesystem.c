...
//            int result;
            ...
            fileName = CsrPmemAlloc(CsrStrLen(dir->path) +
                CsrStrLen(dirent->d_name) + 1);
            CsrStrCpy(fileName, dir->path);
            CsrStrCat(fileName, dirent->d_name);
...
CsrDirHandle *CsrDirFindFirst(const CsrUtf8String *searchPattern, CsrDirFindStat *dirStat)
{
    ...
    if (dirname[0] == '/')
    {
        ...
        sz = CsrStrLen(dirname) + sizeof('/') + 1;

        dir->path = CsrPmemAlloc(sz);
        CsrStrCpy(dir->path, dirname);
        CsrStrNCat(dir->path, "/", sz - CsrStrLen(dir->path));
    }
    else
    {
        ...
        else
        {
            ...
            strncat(dir->path, "/", MAXPATHLEN - 1 - strlen(dir->path));
            strncat(dir->path, dirname, MAXPATHLEN - 1 - strlen(dir->path));
            strncat(dir->path, "/", MAXPATHLEN - 1 - strlen(dir->path));
        }
    }
