...
/*
 * Remove IP address from interface and
 * possibly kill DHCP client process.
 */
static CsrResult removeIp(CsrIpInstanceData *instanceData,
    struct netdev_t *netdev)
{
    struct rtattr *rta;
    int rc;
    ...
    if (netdev->dhcpPid != 0)
    {
        (void) kill(netdev->dhcpPid, SIGTERM);
        (void) waitpid(netdev->dhcpPid, NULL, 0);
        netdev->dhcpPid = 0;
    }
    ...
    req.ifa.ifa_index = netdev->ifd.ifHandle;
    ...
    else
    {
        ...
        ifr.ifr_ifindex = netdev->ifd.ifHandle;
...
static void updateNetdev(CsrIpInstanceData *instanceData,
    CsrIpIfconfigIfDetails *ifd,
    struct netdev_t *netdev)
{
        ...
        (CsrMemCmp(&ifd->mac, &netdev->ifd.mac, sizeof(ifd->mac)) != 0))
    {
        ...
        /*
         * If the interface is going down, remove
         * all configuration from it.
         */
        if (netdev->ifd.ifState == CSR_IP_IFCONFIG_IF_STATE_UP)
        {
            if ((ifd->ifState == CSR_IP_IFCONFIG_IF_STATE_MEDIA_DISCONNECTED) ||
                (ifd->ifState == CSR_IP_IFCONFIG_IF_STATE_DOWN))
            {
                (void) removeIp(instanceData, netdev);
            }
        }
             ...
             nh = NLMSG_NEXT(nh, len))
        {
            ...
            switch (nh->nlmsg_type)
            {
                case RTM_NEWADDR:
                {
                    ...
                    if (netdev != NULL)
                    {
                        updateNetdev(instanceData, &ifd, netdev);
                    }
                    /*
    ...
    CsrIpIfconfigDownReq *request)
{
    ...
    {
        rv = removeIp(instanceData, netdev);
    }
    ...
    else
    {
             ...
             netdev = netdev->next)
        {
            if (netdev->natEnabled)
            {
                ...
                sprintf(nextCmd, "-A POSTROUTING -o %s -j MASQUERADE\n", netdev->ifName);
            }
        }
        CsrStrCat(cmdBuf, "COMMIT\n");
...
void csrIpIfconfigDeferredProcess(CsrIpInstanceData *instanceData)
{
    ...
    do
    {
        ...
        /*
         * instanceData->ifconfigRequests has already been verified
         * to be non-NULL because of a THREAD_CMD_DEFERRED or at
         * the end of the loop.
         */

        (void) CsrMessageQueuePop(&instanceData->ifconfigRequests, &unused,
            &msg);
        ...
        CsrIpLock(instanceData);
    } while (instanceData->ifconfigRequests != NULL);
