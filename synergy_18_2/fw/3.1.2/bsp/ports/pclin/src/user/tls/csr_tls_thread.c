...
static void addSocket(CsrTlsThreadInstanceData *instanceData, int fd, fd_set *rset, fd_set *wset)
{
    ...
    if (rset)
    {
        CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "fd %d added to readset", fd));
        addDescSet(instanceData, fd, rset);
    }
    if (wset)
    {
        CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "fd %d added to writeset", fd));
        addDescSet(instanceData, fd, wset);
    }

    if (write(instanceData->addSocketPipe[1], &reap, 1) != 1)
    {
        CSR_LOG_TEXT_WARNING((CSR_TLS_IFACEQUEUE, 0, "Failed to write 0 to addSocketPipe"));
    }
}
...
static void removeSocket(CsrTlsThreadInstanceData *instanceData, int fd, fd_set *rset, fd_set *wset)
{
    ...
    if (rset)
    {
        CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "fd %d removed from readset", fd));
        removeDescSet(instanceData, fd, rset);
    }
    if (wset)
    {
        CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "fd %d removed from writeset", fd));
        removeDescSet(instanceData, fd, wset);
    }

    if (write(instanceData->addSocketPipe[1], &reap, 1) != 1)
    {
        CSR_LOG_TEXT_WARNING((CSR_TLS_IFACEQUEUE, 0, "Failed to write 1 to addSocketPipe"));
    }
}
...
static void sslInfoCallback(const SSL *s, int where, int ret)
{
    ...
    CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "%s: %d: %d", str, where, SSL_get_error(s, ret)));
}
...
static int sslCertVerifyCallback(int preverify_ok, X509_STORE_CTX *ctx)
{
    ...
    if (bp == NULL)
    {
        CSR_LOG_TEXT_WARNING((CSR_TLS_IFACEQUEUE, 0, "Could not allocate BIO mem buffer"));
        BIO_free(bp);
        return 0;
    }
    ...
    {
        CSR_LOG_TEXT_WARNING((CSR_TLS_IFACEQUEUE, 0, "Could not extract X509 certificate"));
        BIO_free(bp);
        return 0;
    }
    ...
    CSR_LOG_TEXT_BUFFER_INFO((CSR_TLS_IFACEQUEUE, 0, certLen, cert, "Server certificate verify callback, preverify: %u, err: %d", preverify_ok, err));
    ...
    CsrTlsSocketInst *sockInst)
{
    ...
    /* If certificate should be verified and break handshake process
        SSL_VERIFY_PEER should replace SSL_VERIFY_NONE. */
    ...
    CsrTlsSocketInst *sockInst)
{
    ...
    switch (res)
    {
        case SSL_ERROR_NONE:
        {
            CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0,
...
#endif
            CSR_LOG_TEXT_WARNING((CSR_TLS_IFACEQUEUE, 0,
...
static void readSSL(CsrTlsThreadInstanceData *instanceData, CsrTlsSocketInst *sockInst)
{
    ...
    do
    {
        ...
        switch (SSL_get_error(sockInst->ssl, res))
        {
            ...
            default:
            {
                CSR_LOG_TEXT_WARNING((CSR_TLS_IFACEQUEUE, 0, "RX: Could not recv data"));
                break;
            }
        }
...
static void writeSSL(CsrTlsThreadInstanceData *instanceData, CsrTlsSocketInst *sockInst)
{
    ...
    switch (error)
    {
        ...
        default:
        {
            CSR_LOG_TEXT_WARNING((CSR_TLS_IFACEQUEUE, 0, "TX: Could not send data, error %d", error));
...
static void handleSockets(CsrTlsThreadInstanceData *instanceData, fd_set *rset, fd_set *wset)
{
    ...
    for (sockno = 0; sockno <= instanceData->maxSocketHandle; sockno++)
    {
        ...
        CSR_LOG_TEXT_DEBUG((CSR_TLS_IFACEQUEUE, 0,
                            "socket %d, readset: %u, writeset: %u",
                            sockInst->socket,
                            FD_ISSET(sockInst->socket, rset),
                            FD_ISSET(sockInst->socket, wset)));

        CSR_LOG_TEXT_DEBUG((CSR_TLS_IFACEQUEUE, 0, "DEBUG: SocketInstance Handler:%d", sockInst));
        ...
        switch (sockInst->type)
        {
            ...
            default:
            {
                CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "socket thread unhandled socket type: %d", sockInst->type));
                break;
            }
        }
    }
...
static void *socketThreadFunction(void *arg)
{
    ...
    CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "socket thread started"));
    ...
    while (1)
    {
        ...
        CSR_TLS_ASSERT(ready >= 0, "select failed");

        if (FD_ISSET(instanceData->closeThreadPipe[0], &rset))
        {
            /* exit thread */
            CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "socket thread stopped"));
            return 0;
        }
        ...
        if (FD_ISSET(instanceData->addSocketPipe[0], &rset))
        {
            ...
            {
                CSR_LOG_TEXT_WARNING((CSR_TLS_IFACEQUEUE, 0, "Failed to read from addSocketPipe"));
            }
            ...
            {
                CSR_LOG_TEXT_DEBUG((CSR_TLS_IFACEQUEUE, 0, "socket thread rescheduled"));
            }
...
void CsrTlsThreadInit(CsrTlsThreadInstanceData *instanceData, CsrTlsInstanceData *taskInstanceData)
{
    ...
    CSR_TLS_ASSERT(pthread_mutex_init(&instanceData->instLock, NULL) == 0,
        "failed creating instance lock");
    CSR_TLS_ASSERT(pthread_create(&instanceData->tid,
            NULL, socketThreadFunction, (void *) instanceData) == 0,
        "failed creating ip thread");
}
...
void CsrTlsThreadDeinit(CsrTlsThreadInstanceData *instanceData)
{
    ...
    {
        CSR_LOG_TEXT_WARNING((CSR_TLS_IFACEQUEUE, 0, "Failed to write 1 to closeTreadPipe"));
    }
    ...
    CSR_TLS_ASSERT(close(instanceData->closeThreadPipe[0]) == 0,
        "close closeThreadPipe[0]");
    CSR_TLS_ASSERT(close(instanceData->closeThreadPipe[1]) == 0,
        "close closeThreadPipe[1]");
    CSR_TLS_ASSERT(close(instanceData->addSocketPipe[0]) == 0,
        "close addSocketPipe[0]");
    CSR_TLS_ASSERT(close(instanceData->addSocketPipe[1]) == 0,
        "close addSocketPipe[1]");
...
void CsrTlsSessionDestroyHandler(CsrTlsInstanceData *taskInstanceData)
{
    ...
    {
        CSR_LOG_TEXT_WARNING((taskInstanceData->qid, 0, "Faild to get session"));
    }
    pthread_mutex_unlock(&instanceData->instLock);
}
...
void CsrTlsConnectionCreateReqHandler(CsrTlsInstanceData *taskInstanceData)
{
    ...
    if (sock != -1)
    {
        ...
        {
            CSR_LOG_TEXT_WARNING((taskInstanceData->qid, 0,
...
void CsrTlsConnectionConnectReqHandler(CsrTlsInstanceData *taskInstanceData)
{
    ...
    if (sockInst != NULL)
    {
        ...
        if (res == 0)
        {
            CSR_LOG_TEXT_INFO((taskInstanceData->qid, 0,
                               "socket %d connected", sockInst->socket));
            sockInst->type = CSR_TLS_TYPE_TCP_SSL_CONNECT;
            createSSL(instanceData, sockInst);
            connectSSL(instanceData, sockInst);
...
void CsrTlsConnectionDisconnectReqHandler(CsrTlsInstanceData *taskInstanceData)
{
    ...
    {
        CSR_LOG_TEXT_WARNING((taskInstanceData->qid, 0, "failed getting socket inst"));
    }
    pthread_mutex_unlock(&instanceData->instLock);
}
...
void CsrTlsConnectionCertResHandler(CsrTlsInstanceData *taskInstanceData)
{
    ...
    CSR_LOG_TEXT_DEBUG((taskInstanceData->qid, 0, "DEBUG: certRes %d", sockInst));
