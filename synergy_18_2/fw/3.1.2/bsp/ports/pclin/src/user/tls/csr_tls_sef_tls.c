...
{
    CsrTlsSessionCreateHandler,
    CsrTlsSessionDestroyHandler,
    configCipherReqHandler,
    configCertClientReqHandler,
    configCertVerifyReqHandler,
    NULL,
    CsrTlsConnectionCreateReqHandler,
    CsrTlsConnectionBindReqHandler,
    CsrTlsConnectionDestroyReqHandler,
    CsrTlsConnectionConnectReqHandler,
    CsrTlsConnectionDisconnectReqHandler,
    CsrTlsConnectionDisconnectResHandler,
    CsrTlsConnectionCertResHandler,
    CsrTlsConnectionDataReqHandler,
    CsrTlsConnectionDataResHandler
};
    ...
    unsigned int maxPskLen)
{
    ...
    {
        CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "pskCLientCallback: Sesssions stored %d", globalCsrTlsInstanceData->sessions));
        ...
        for (count = 0; count < globalCsrTlsInstanceData->sessions && session; count++)
        {
            if (session->ctx == ssl->ctx)
            {
                ...
                CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "pskCLientCallback: Session found %d", count));
                break;
            }
    ...
    {
        CSR_LOG_TEXT_WARNING((CSR_TLS_IFACEQUEUE, 0, "pskCLientCallback: No globalCsrTlsInstanceData"));
    }
    ...
    if (session)
    {
        ...
        CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "pskCLientCallback: identity %s", identity));

        /*the identity-hint is not added/used here*/

        if ((unsigned int) session->pskKeyLength > maxPskLen)
        {
            CSR_LOG_TEXT_WARNING((CSR_TLS_IFACEQUEUE, 0, "pskCLientCallback: Stored psk key length larger than supported %d, %d", session->pskKeyLength, maxPskLen));
        }
        else
        {
            pskLen = (unsigned int) session->pskKeyLength;
            CsrMemCpy(psk, session->pskKey, pskLen);
            CSR_LOG_TEXT_INFO((CSR_TLS_IFACEQUEUE, 0, "pskCLientCallback: psk key of length %d copied to ssl store", pskLen));
        }
    }
...
csrTlsSession *sessionCreateHandler(CsrTlsInstanceData *instanceData, CsrSchedQid qid)
{
    if (instanceData->sessions < CSR_TLS_SESSIONLIMIT)
    {
        ...
        */
        ...
        CSR_LOG_TEXT_WARNING((instanceData->qid, 0,
...
static void configCipherReqHandler(CsrTlsInstanceData *instanceData)
{
         ...
         ii++)
    {
        ...
        {
            CSR_LOG_TEXT_ERROR((instanceData->qid, 0,
        ...
        {
            cipherList = CsrStrCat(cipherList, cipherTemp);
        }
    }
...
static void configCertClientReqHandler(CsrTlsInstanceData *instanceData)
{
    ...
    if (req->certificateType == CSR_TLS_CERTIFICATE)
    {
        ...
        {
            CSR_LOG_TEXT_INFO((instanceData->qid, 0,
...
static void configCertVerifyReqHandler(CsrTlsInstanceData *instanceData)
{
    ...
    *  need to do add them to the internal store in the ctx */
    ...
    if (!(inf = PEM_X509_INFO_read_bio(in, NULL, NULL, NULL)))
    {
        CSR_LOG_TEXT_ERROR((instanceData->qid, 0,
                            "BIO buffer do not contain any certificate"));
    ...
    count = 0;
    for (ii = 0; ii < sk_X509_INFO_num(inf); ii++)
    {
        ...
        if (itmp->x509)
        {
            ...
            {
                CSR_LOG_TEXT_ERROR((instanceData->qid, 0,
                                    "Certificate could not be added to X509 Store"));
            }
            else
            {
                count++;
    ...
    CSR_LOG_TEXT_INFO((instanceData->qid, 0,
                       "%d certificate added in certificate chain", count));
