...
#define SHUTDOWN_MESSAGE (WM_APP + 0)
...
static DWORD WINAPI schedThread(LPVOID schedInstance)
{
    CsrSched(schedInstance);
    PostThreadMessage(mainThreadId, SHUTDOWN_MESSAGE, 0, 0);
    ExitThread(0);
    return 0;
}
...
CsrResult CsrMain(void)
{
    ...
    schedThreadHandle = CreateThread(NULL, 0, schedThread, schedInstance, 0, NULL);
    ...
    /* Stop the scheduler */
    WaitForSingleObject(schedThreadHandle, INFINITE);
