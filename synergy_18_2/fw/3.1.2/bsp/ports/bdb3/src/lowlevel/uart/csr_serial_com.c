...
static CsrBool instInUse[UART_CHANNEL_COUNT] = {FALSE, FALSE, FALSE};
...
static CsrSchedBgint rxBgint[UART_CHANNEL_COUNT] = {CSR_SCHED_BGINT_INVALID, CSR_SCHED_BGINT_INVALID, CSR_SCHED_BGINT_INVALID};
static CsrUartDrvDataRx rxData[UART_CHANNEL_COUNT] = {NULL, NULL, NULL};
...
static CsrBool bgIntHisrCreated[UART_CHANNEL_COUNT] = {FALSE, FALSE, FALSE};
...
static void bgIntHisrActivate3(void)
{
    NU_Activate_HISR(&bgIntHisr[2]);
}

csrUartBgIntWrapper bgIntHisrActivate[UART_CHANNEL_COUNT] = {bgIntHisrActivate1, bgIntHisrActivate2, bgIntHisrActivate3};
...
static void csrUartBgIntWrapper3(void)
{
    CsrSchedBgintSet(rxBgint[2]);
}

csrUartBgIntWrapper bgIntWrapper[UART_CHANNEL_COUNT] = {csrUartBgIntWrapper1, csrUartBgIntWrapper2, csrUartBgIntWrapper3};
...
#ifdef CSR_INSTRUMENTED_PROFILING_SERVICE
        /* Measure the created HISR as the calling context */
        CsrIpsInsert(CSR_IPS_CONTEXT_THREAD(&bgIntHisr[idx]), CsrSchedIpsMeasurementsGet(CSR_SCHED_IPS_CONTEXT_CURRENT));
#endif
    ...
    const CsrUint8 *token)
{
    ...
    for (idx = 0; idx < UART_CHANNEL_COUNT; idx++)
    {
        if (!instInUse[idx])
        {
            ...
            else if (CsrStrNCmp(device, "uart2", UART_DEVICE_NAME_LENGTH) == 0)
            {
                instChannel[idx] = 2;
            }
            else
