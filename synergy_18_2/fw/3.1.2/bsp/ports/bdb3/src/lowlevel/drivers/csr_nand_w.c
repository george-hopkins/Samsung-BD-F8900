...
#define NAND_LOCK()                 BIT_SET(rNFCONT, 16)
#define NAND_UNLOCK()               BIT_CLEAR(rNFCONT, 16)
#define NAND_LOCK_TIGHT()           BIT_SET(rNFCONT, 17)
#define NAND_IS_LOCKED_TIGHT()      BIT_CHECK(rNFCONT, 17)
#define NAND_IS_LOCKED()            BIT_CHECK(rNFCONT, 16)
...
CsrInt32 NANDPageWrite(CsrUint32 page, CsrUint8 *buffer)
{
    ...
    if ((page / NAND_PAGESPERBLOCK) == 0)
    {
        /* Use 8bit ECC on block 0 */
        ecc8bit = TRUE;
    }
    ...
    {
        CsrInt32 j;
        CsrUint32 ecc[NAND_PAGESIZE / ECC_MAIN_8BIT_MSG_LEN][4];

        ECC_TYPE_8BIT();
        ECC_DIRECTION_ENCODE();

        for (i = 0; i < (NAND_PAGESIZE / ECC_MAIN_8BIT_MSG_LEN); i++)
        {
            ECC_MAIN_UNLOCK();  /* Unlock Main data area ECC generation */
            ECC_MAIN_RESET();   /* Initialize ECC decoder/encoder */

            for (j = 0; j < ECC_MAIN_8BIT_MSG_LEN; j++)
            {
                CHIP_ENABLE_ATOMIC();
                DATA_WRITE8(*buffer++);
                CHIP_DISABLE_ATOMIC();
            ...
            while (!ECC_ENCODE_DONE())
            {
            }

            ECC_ENCODE_DONE_CLEAR();
            ECC_STOP();
            ECC_MAIN_LOCK();

            /* Copy out the ECC values */
            ecc[i][0] = rNFM8ECC0;
            ecc[i][1] = rNFM8ECC1;
            ecc[i][2] = rNFM8ECC2;
            ecc[i][3] = rNFM8ECC3;
        ...
        /* Write the ECC values */
        for (i = 0; i < (NAND_PAGESIZE / ECC_MAIN_8BIT_MSG_LEN); i++)
        {
            buffer = (CsrUint8 *) ecc[i];
            for (j = 0; j < ECC_MAIN_8BIT_ECC_LEN; j++)
            {
                CHIP_ENABLE_ATOMIC();
                DATA_WRITE8(*buffer++);
                CHIP_DISABLE_ATOMIC();
    ...
    else
    {
        ...
        DATA_WRITE8(0xFF); /* Good block marker (only applicable for first or second page in block) */
        DATA_WRITE8(rNFMECC0 & 0xFF);           /* ECC0 */
        DATA_WRITE8((rNFMECC0 >> 8) & 0xFF);    /* ECC1 */
        DATA_WRITE8((rNFMECC0 >> 16) & 0xFF);   /* ECC2 */
        DATA_WRITE8((rNFMECC0 >> 24) & 0xFF);   /* ECC3 */
