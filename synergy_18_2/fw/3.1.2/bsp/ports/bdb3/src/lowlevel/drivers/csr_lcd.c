...
#define CLKVALUP    0x0         /* Update from CLKVAL_F always */
#define CLKVAL_F    0x3         /* 25MHz LCD clock */
#define VCLKFREE    0x0         /* Normal VCLK mode */
#define CLKDIR      0x1         /* Clock divided by CLKVAL_F */
#define CLKSEL_F    0x1         /* Source clock is CLKLCD from System Controller */
...
#define BURSTLEN    0x2         /* 4-word burst read */
#define BPPMODE_F   0xB         /* 24bpp (physical routing is 18bpp) */

/* Chip Select Output @ GPI0 */
#define XCS_SET()     GPIO_SET(GPIO_PORT_I, 0)
#define XCS_CLEAR()   GPIO_CLEAR(GPIO_PORT_I, 0)

/* Serial Clock Output @ GPI1 */
#define SCL_SET()     GPIO_SET(GPIO_PORT_I, 1)
#define SCL_CLEAR()   GPIO_CLEAR(GPIO_PORT_I, 1)
#define SCL_CLOCK()   {SCL_CLEAR(); \
                       DelayUs(100); \
                       SCL_SET(); \
                       DelayUs(100);}

/* Serial Data Input to LCD module @ GPI8 */
#define DIN_SET()     GPIO_SET(GPIO_PORT_I, 8)
#define DIN_CLEAR()   GPIO_CLEAR(GPIO_PORT_I, 8)
#define DIN_IN()      GPIO_I_FUNCTION_SET(8, GPIO_FUNCTION_INPUT)
#define DIN_OUT()     GPIO_I_FUNCTION_SET(8, GPIO_FUNCTION_OUTPUT)

/* Serial Data Output from LCD module @ GPI9 */
#define DOUT          GPIO_IN(GPIO_PORT_I, 9)
...
void LCDInitialise(CsrUint32 *fb0, CsrUint32 *fb1)
{
    ...
    if (fb0)
    {
        rVIDW00ADD0B0 = (CsrUint32) fb0;
        rVIDW00ADD1B0 = ((CsrUint32) fb0) + LCD_WIDTH * LCD_HEIGHT * LCD_PIXELSIZE - 1;
        LCDClear(fb0);
    }
    if (fb1)
    {
        rVIDW00ADD0B1 = (CsrUint32) fb1;
        rVIDW00ADD1B1 = ((CsrUint32) fb1) + LCD_WIDTH * LCD_HEIGHT * LCD_PIXELSIZE - 1;
        LCDClear(fb1);
    }

    rVIDW00ADD2 = LCD_WIDTH * LCD_PIXELSIZE;
    ...
    rVIDOSD0A = 0;
    rVIDOSD0B = LCD_WIDTH << 11 | LCD_HEIGHT;
    rVIDOSD0C = LCD_WIDTH * LCD_HEIGHT;
