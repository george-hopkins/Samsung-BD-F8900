...
#define LCD_OFF()           REG_SET(rVIDCON0, 1, 0, 0)
#define LCD_ON()            REG_SET(rVIDCON0, 1, 0, 3)

/* Frame synchronised on/off control of driving signals */
#define LCD_ON_FE()         REG_SET(rVIDCON0, 1, 0, 3)
#define LCD_OFF_FE()        BIT_CLEAR(rVIDCON0, 0)
#define LCD_IS_ACTIVE()     BIT_CHECK(rVIDCON0, 0)

/* Set/toggle the active frame buffer */
#define LCD_FB_SET(number)  {if (number) {BIT_SET(rWINCON0, 20)} else {BIT_CLEAR(rWINCON0, 20)}}
#define LCD_FB_TOGGLE()     BIT_TOGGLE(rWINCON0, 20)

/* Get active/inactive frame buffer number */
#define LCD_FB_ACTIVE_GET()             (BIT_CHECK(rWINCON0, 20) ? 1 : 0)
#define LCD_FB_INACTIVE_GET()           (BIT_CHECK(rWINCON0, 20) ? 0 : 1)
