...
/* The UARTReceive/UARTReceiveLine/UARTTransmit function as well as the
   interrupt handling is instrumented and measured separately for each UART
   channel. The other functions are not instrumented and execution time spend
   in calls will be attributed to the calling context. This function is not
   thread safe and should not be called concurrently with any of the
