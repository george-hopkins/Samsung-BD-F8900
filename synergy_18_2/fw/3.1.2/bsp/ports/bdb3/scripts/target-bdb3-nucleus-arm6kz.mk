...
ifeq ($(ARM_COMPILER),ADS)
    $(error ADS 1.2 is not supported on this target)
endif

# RVCT2.2
ifeq ($(ARM_COMPILER),2.2)
    $(error Realview ARM Compiler 2.2 is not supported on this target)
endif
...
ifeq ($(ARM_COMPILER_PATH),)
    $(error ARM_COMPILER=$(ARM_COMPILER) invalid, use: 3.0, 3.1, 4.0 or 4.1)
endif
...
SCATTER_FILE ?= $(BSP_ROOT)/scripts/bdb3.sct
...
# Hardware
ARM_CORE ?= ARM1176JZ-S
ARM_FPU  ?= SoftVFP+VFPv2

# Compiler flags
TARGET_CFLAGS += -c -g -O$(OPTIMIZE_LEVEL) --$(ARM_INSTRUCTION_SET) --cpu $(ARM_CORE) --fpu $(ARM_FPU) --no_unaligned_access --diag_suppress=9931,9933
# Assembler flags
TARGET_SFLAGS = -g --keep --cpu $(ARM_CORE) --fpu $(ARM_FPU) --apcs /interwork --no_unaligned_access
# Linker Flags
