...
/*! \brief Build and send an RFC_PORTNEG_REQ primitive to RFCOMM
    ...
    \params p_port_pars - Pointer to local port parameters. Must point to valid
...
/*! \brief Build and send an RFC_FC_REQ primitive to RFCOMM
    ...
    \param p_bd_addr - pointer to the bluetooth address of the remote device    \params
...
/*! \brief Build and send an RFC_DATAWRITE_REQ primitive to RFCOMM
    ...
    \param conn_id - Unique connection identifier
    \params payload_length - length of payload
    \params payload - mblk containing the payload
    \params rx_credits - Normally the host would be expected to set rx_credits to 0, however by
