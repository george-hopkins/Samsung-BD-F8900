 ...
 *
 *----------------------------------------------------------------------------*/
#define CsrBtHfgActivateReqSend(_phandle, _atMode, _numConnections, _serviceName, _supportedFeatures, _callConfig, _hfgConfig) { \
...
/*----------------------------------------------------------------------------*
 *  NAME
 *      CsrBtHfgC2cSfReqSend
 *
 *  DESCRIPTION
 *      Update the CSR-2-CSR supported features like text-to-speech,
 *      SMS notification etc.
 *
 *  PARAMETERS
 *      connectionId:            HFG connection index
 *      number:                  Parameter number to update
 *      value:                   New value of parameter
 *
 *----------------------------------------------------------------------------*/
#define CsrBtHfgC2cSfReqSend(_connectionId, _number, _value) {          \
        CsrBtHfgC2cSfReq *msg = (CsrBtHfgC2cSfReq*)CsrPmemAlloc(sizeof(CsrBtHfgC2cSfReq)); \
        msg->type = CSR_BT_HFG_C2C_SF_REQ;                              \
        msg->connectionId = _connectionId;                              \
        msg->number = _number;                                          \
        msg->value = _value;                                            \
        CsrBtHfgMsgTransport(msg);}
 ...
 *
 *----------------------------------------------------------------------------*/
#define CsrBtHfgC2cTxtReqSend(_connectionId, _text) {                   \
        CsrBtHfgC2cTxtReq *msg = (CsrBtHfgC2cTxtReq*)CsrPmemAlloc(sizeof(CsrBtHfgC2cTxtReq)); \
        msg->type = CSR_BT_HFG_C2C_TXT_REQ;                             \
        msg->connectionId = _connectionId;                              \
        msg->text = _text;                                              \
        CsrBtHfgMsgTransport(msg);}
 ...
 *  PARAMETERS
 *      connectionId:            HFG connection index
 *      smsIndex:                Index of the SMS for later retrieval
 *      number:                  Number of sending party
 *      name:                    Phonebook name of sending party
 *
 *----------------------------------------------------------------------------*/
#define CsrBtHfgC2cSmsArriveReqSend(_connectionId, _smsIndex, _number, _name) { \
        CsrBtHfgC2cSmsArriveReq *msg = (CsrBtHfgC2cSmsArriveReq*)CsrPmemAlloc(sizeof(CsrBtHfgC2cSmsArriveReq)); \
        msg->type = CSR_BT_HFG_C2C_SMS_ARRIVE_REQ;                      \
        msg->connectionId = _connectionId;                              \
        msg->smsIndex = _smsIndex;                                      \
        msg->number = _number;                                          \
        msg->name = _name;                                              \
        CsrBtHfgMsgTransport(msg);}
 ...
 *
 *  DESCRIPTION
 *      Send SMS body text to headset for text-2-speech
 *
 *  PARAMETERS
 *      connectionId:            HFG connection index
 *      smsText:                 Null-terminated ASCII text of SMS body.
 *----------------------------------------------------------------------------*/
#define CsrBtHfgC2cSmsTxtReqSend(_connectionId, _smsText) {             \
        CsrBtHfgC2cSmsTxtReq *msg = (CsrBtHfgC2cSmsTxtReq*)CsrPmemAlloc(sizeof(CsrBtHfgC2cSmsTxtReq)); \
        msg->type = CSR_BT_HFG_C2C_SMS_TXT_REQ;                         \
        msg->connectionId = _connectionId;                              \
        msg->smsText = _smsText;                                        \
        CsrBtHfgMsgTransport(msg);}
#endif
 ...
 *
 *----------------------------------------------------------------------------*/
#define CsrBtHfgGetC2CAdpcmLocalSupportedReqSend(){                     \
        CsrBtHfgGetC2cAdpcmLocalSupportedReq *msg = (CsrBtHfgGetC2cAdpcmLocalSupportedReq*)CsrPmemAlloc(sizeof(CsrBtHfgGetC2cAdpcmLocalSupportedReq)); \
        msg->type = CSR_BT_HFG_GET_C2C_ADPCM_LOCAL_SUPPORTED_REQ;       \
        CsrBtHfgMsgTransport(msg); }
/*----------------------------------------------------------------------------*
 *  NAME
