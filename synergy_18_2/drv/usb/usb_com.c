...
/* Device instance */
csr_dev_t *csr_dev[BT_USB_COUNT]= { NULL, };   /* Sunny@CSR : 2011.09.27 fixed for lockup during BT on without USB(BC8510) */
...
#define proc_file_name "driver/csr_btusb0"
...
#endif
{
    ...
    if((urb->status != 0) &&
       ...
       (urb->status != -ECONNRESET) &&
       (urb->status != -ESHUTDOWN) &&
       (urb->status != -ETIME))               // samsung error....
    {
...
int16_t usbTxCtrl(csr_dev_t *dv, void *data, uint16_t length)
{
    ...
    if(txctrl != NULL)
    {
    	mdelay(10); // According to CSR Engineer comment, add by carlos and nelly
        /* Setup packet */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,4,20)
...
#endif
{
    ...
    if(test_bit(DEVICE_CONNECTED, &(dv->flags)))
    {
        ...
        urb->dev = dv->dev;
        urb->actual_length = 0;
        err = URB_SUBMIT(urb, GFP_ATOMIC | GFP_DMA);
...
static int16_t usbRxIntr(csr_dev_t *dv)
{
    ...
    buf = kmalloc(MAX_HCI_EVENT_SIZE, GFP_ATOMIC | GFP_DMA);
    ...
    if((rxintr != NULL) && (buf != NULL))
    {
        ...
        err = URB_SUBMIT(rxintr, GFP_ATOMIC | GFP_DMA);
        if(err != 0)
        {
            URB_UNLINK(rxintr);
            usb_free_urb(rxintr);
...
static int16_t usbRxBulk(csr_dev_t *dv, uint8_t number)
{
    ...
    err = -ENOMEM;
    rxbulk = URB_ALLOC(0, GFP_ATOMIC | GFP_DMA);
    buf = kmalloc(MAX_HCI_ACL_SIZE, GFP_ATOMIC | GFP_DMA);
    ...
    if((rxbulk != NULL) && (buf != NULL))
    {
        ...
        err = URB_SUBMIT(rxbulk, GFP_ATOMIC | GFP_DMA);
        if(err != 0)
        {
            URB_UNLINK(rxbulk);
            usb_free_urb(rxbulk);
...
int csrUsbProbe(struct usb_interface *intf,
                const struct usb_device_id *passed_id)
#else
...
#endif
{
	...
	id = usb_match_id(intf, csr_device_table);
	if(!id)
	{
		printk("csr_bt_usb: Tried to attach to the incorrect device.\n");
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,0)
		return -ENODEV;
#else
		return NULL;
...
#ifdef BT_USB_DEBUG 
	printk("csr_bt_usb: Attached to the device VID: %04X, PID: %04X\n", id->idVendor, id->idProduct);
	printk("Device class: %02X, device subclass: %02X, device protocol: %02X\n", id->bDeviceClass, id->bDeviceSubClass, id->bDeviceProtocol);
	printk("Interface class: %02X, interface subclass: %02X, interface protocol: %02X\n", id->bInterfaceClass, id->bInterfaceSubClass, id->bInterfaceProtocol);
#endif
    ...
    proc_file = create_proc_read_entry(proc_file_name, 0444, NULL, NULL, intf);
    if (proc_file == NULL)
    {
    ...
    {
        printk("proc filesystem entry added /proc/%s\n", proc_file_name);
    }
...
int csrUsbSuspend(struct usb_interface *intf, pm_message_t state)
{
	...
	/* prohibit going to autosuspend when there is still connection */
	if((state.event & PM_EVENT_AUTO) && (intf->pm_usage_cnt > 0))
	{
		printk("bt_usb%u: attempt to autosuspend (PM message: 0x%04x) - busy\n", devno, state.event);
		return -EBUSY;
	}
