...
void commonLeBrowserInputPopupSet(CsrBtLeBrowserAppInstData *inst, CsrUint8 *dialogMessage, CsrUint8 keyMap)
{
    ...
    {
        dialogInputText = (CsrUint8 *)CsrPmemZalloc(sizeof(CsrUint8)*inst->dbElement->currentDescr->valueLength + 1);
        sprintf((void*)dialogInputText, "%s", inst->dbElement->currentDescr->value);
    }
    else
    {
        if((inst->writeMethod & CSR_BT_LE_BROWSER_FORMATTED_WRITE) == CSR_BT_LE_BROWSER_FORMATTED_WRITE)
        {
            ...
            {
                dialogInputText = (CsrUint8*)CsrPmemZalloc(sizeof(CsrUint8)*inst->dbElement->currentCharac->valueLength + 1);
                
                CsrMemCpy(dialogInputText, inst->dbElement->currentCharac->value, inst->dbElement->currentCharac->valueLength);
            }
            else
            {
                dialogInputText = '\0';
    ...
    if(inputStr != NULL)
    {
        ...
        switch(writeMethod)
        {
            ...
            case CSR_BT_LE_BROWSER_APP_WRITE_AUX_METHOD:
                {
                    CsrBtUuid *uuid = CsrBtGatt16BitTo128BitUuid(CSR_BT_GATT_APP_USER_DESCRIPTION_DESCR_UUID);
                    CsrBtGattAppDbCharacDescrElement *dElem = CSR_BT_GATT_APP_DB_FIND_CHARAC_DESCR_BY_UUID(inst->dbElement->currentCharac->descrList, 
                                                                                                           uuid);
                    CsrSize tmpLen = CsrStrLen((CsrCharString*)inputStr);
...
static void handleCsrUiDisplayGethandleCfm(CsrBtLeBrowserAppInstData *inst, CsrUiDisplayGethandleCfm *confirm)
{
    ...
    if((confirm->handle == inst->csrUiVar.hDescMenu) && (uuid == CSR_BT_GATT_APP_TEMP_MEASUREMENT_CHARAC_UUID)) 
    {
        CsrBtGattAppHandleLeBrowserCsrUiListDatabaseEntries(inst, CSR_BT_LE_BROWSER_CHARAC_DESCR_MENU_STATE);
    }
}
...
static void handleCsrUiIndicationKey(CsrBtLeBrowserAppInstData *inst, CsrUint16 key)
{
    ...
    switch(key)
    {
        ...
        case CSR_BT_LE_BROWSER_APP_WRITECOMMAND_KEY:
            {
                ...
                CsrUint8* popupMessage = (CsrUint8*)CsrPmemAlloc(sizeof(CsrUint8)*CSR_BT_LE_BROWSER_POPUPMSG_LEN); 

                CsrBtGattLeBrowserUpdateDbElement(inst, CSR_BT_GATT_LE_BROWSER_LVL_1);

                sprintf((void*)popupMessage, "Received Value Length: %X", inst->dbElement->currentCharac->valueLength);
        ...
        case CSR_BT_LE_BROWSER_APP_AUTH_WRITES_KEY:
            {
                ...
                CsrUint8* popupMessage = (CsrUint8*)CsrPmemAlloc(sizeof(CsrUint8)*25); /* Allocation assumes at most 9 bytes valueLength */
        ...
        case CSR_BT_LE_BROWSER_APP_EXT_RELIABLE_WRITE_KEY:
            {
                CsrUint8* popupMessage = (CsrUint8*)CsrPmemAlloc(sizeof("Input Value"));

                CsrBtGattLeBrowserUpdateDbElement(inst, CSR_BT_GATT_LE_BROWSER_LVL_1);

                sprintf((void*)popupMessage, "Input Value");
        ...
        case CSR_BT_LE_BROWSER_APP_EXT_WRITABLE_AUX_KEY:
            {
                CsrUint8* popupMessage = (CsrUint8*)CsrPmemAlloc(sizeof("Input User Description"));

                CsrBtGattLeBrowserUpdateDbElement(inst, CSR_BT_GATT_LE_BROWSER_LVL_2);

                sprintf((void*)popupMessage, "Input User Description");
