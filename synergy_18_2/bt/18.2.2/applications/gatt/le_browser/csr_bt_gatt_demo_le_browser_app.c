...
{
    char *buf;
    buf  = CsrPmemAlloc(CsrStrLen("LE Browser") + 1);

    sprintf(buf, "LE Browser");
        ...
        case CSR_BT_GATT_WRITE_CFM :
            {
                ...
                if(cfm->resultCode == CSR_BT_GATT_RESULT_SUCCESS)
                {
                    ...
                    else if((leBrowserData->writeMethod & CSR_BT_GATT_CHARAC_PROPERTIES_WRITE) == CSR_BT_GATT_CHARAC_PROPERTIES_WRITE)
                    {
                        ...
                        leBrowserData->dbElement->currentCharac->valueLength = tmpLen + 1;
                    ...
                    {
                        CsrBtUuid *currentUuid = CsrBtGatt16BitTo128BitUuid(CSR_BT_GATT_APP_USER_DESCRIPTION_DESCR_UUID);
                        CsrBtGattAppDbCharacDescrElement *dElem = CSR_BT_GATT_APP_DB_FIND_CHARAC_DESCR_BY_UUID(leBrowserData->dbElement->currentCharac->descrList, 
                                                                                                               currentUuid);
                        CsrSize tmpLen = CsrStrLen((CsrCharString*)leBrowserData->inputStr);
                        CsrPmemFree(dElem->value);
                        dElem->value = (CsrUint8*)CsrPmemAlloc(tmpLen + 1);
                        CsrMemCpy(dElem->value, leBrowserData->inputStr, tmpLen + 1);
                        dElem->valueLength = tmpLen + 1;
                        
                        /* Done writing */
                        leBrowserData->writeMethod = 0;
                        CsrPmemFree(leBrowserData->inputStr);
                        CsrPmemFree(currentUuid);
                    }
                    /*update UI*/
