...
void CsrBtThermcAppClearHandles(CsrBtThermcAppInstData *thermcData){
    ...
    thermcData->dbElement->tempCharac                    = NULL;
...
{
    char *buf;
    buf  = CsrPmemAlloc(CsrStrLen("Thermometer cli 0000")+1);

    sprintf(buf, "Thermometer cli %04X", instance->phandle);
    return buf;
}
        ...
        case CSR_BT_GATT_NOTIFICATION_IND :  
            {
                ...
                CsrBtGattAppDbPrimServiceElement *pElem  =  CSR_BT_GATT_APP_DB_FIND_PRIM_SERVICE_BY_UUID(thermcData->dbElement->dbPrimServices, CsrBtThermcAppUuid16ToUuid128(CSR_BT_GATT_APP_THERMOMETER_UUID));
