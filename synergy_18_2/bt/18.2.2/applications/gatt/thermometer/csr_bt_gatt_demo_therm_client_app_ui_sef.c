...
CsrUtf16String *commonThermcGetValueString(CsrBtThermcAppInstData *inst, CsrUint16 uuid)
{
    ...
    if(pElem != NULL)
    {
        ...
        case CSR_BT_GATT_APP_THERMOMETER_UUID:
            {
                CsrBtGattAppDbCharacElement *cElem = CSR_BT_GATT_APP_DB_FIND_CHARAC_BY_UUID(pElem->characList, 
                                                                                            CsrBtThermcAppUuid16ToUuid128(CSR_BT_GATT_APP_TEMP_MEASUREMENT_CHARAC_UUID));
                if(cElem != NULL && cElem->value != NULL)
                {
                    ...
                    ret = CsrPmemAlloc(25);
                    sprintf(ret, "%.2f%c Update: %s", temp, type,  (inst->tempNotif==TRUE)?"On ":"Off");
                }
                else 
                {
                    ret = CsrPmemAlloc(12);
                    sprintf(ret, "Update: %s", (inst->tempNotif==TRUE)?"On ":"Off");
                }
        ...
        case CSR_BT_GATT_APP_BATTERY_UUID:
            {
                CsrBtGattAppDbCharacElement *cElem = CSR_BT_GATT_APP_DB_FIND_CHARAC_BY_UUID(pElem->characList, 
                                                                                            CsrBtThermcAppUuid16ToUuid128(CSR_BT_GATT_APP_BATT_LEVEL_CHARAC_UUID));
                if(cElem != NULL && cElem->value != NULL)
                {
                    ret = CsrPmemAlloc(12);
...
void CsrBtGattAppHandleThermcCsrUiMenuUpdate(CsrBtThermcAppInstData *inst)
{
    ...
    if(inst->btConnId != 0xff)
    {
        ...
        CsrUiMenuSetitemReqSend(csrUiVar->hMainMenu, 
                                ...
                                commonThermcGetValueString(inst, CSR_BT_GATT_APP_THERMOMETER_UUID),
                                csrUiVar->hSk1Event, 
                                csrUiVar->hSk2Event, 
                                csrUiVar->hBackEvent, 
                                0);
        CsrUiMenuSetitemReqSend(csrUiVar->hMainMenu, 
                                ...
                                commonThermcGetValueString(inst, CSR_BT_GATT_APP_BATTERY_UUID),
                                csrUiVar->hSk1Event, 
                                csrUiVar->hSk2Event, 
                                csrUiVar->hBackEvent, 
                                0);
...
void CsrBtGattAppHandleThermcCsrUiShowBattInfo(CsrBtThermcAppInstData *inst)
{
    ...
    pElem  = CSR_BT_GATT_APP_DB_FIND_PRIM_SERVICE_BY_UUID(inst->dbElement->dbPrimServices,CsrBtThermcAppUuid16ToUuid128(CSR_BT_GATT_APP_BATTERY_UUID));
            ...
            case CSR_BT_THERMC_APP_WRITE_TEMPERATURE_KEY :
                {
                    ...
                    else
                    {
                        ...
                        CsrBtThermcAppStartWriteClientConfig(inst, CSR_BT_GATT_APP_THERMOMETER_UUID, CSR_BT_GATT_APP_TEMP_MEASUREMENT_CHARAC_UUID, val);
                    }
                    break;
                }
