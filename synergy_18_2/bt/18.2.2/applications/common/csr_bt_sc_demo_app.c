...
static void legacyPasskeyHandler(ScAppInstData_t *instData)
{
    ...
    if (key->key == RETURN_KEY)
    {
        ...
        {
            CsrBtScPasskeyResSend(TRUE, instData->deviceAddr,
...
static void sspPasskeyHandler(ScAppInstData_t *instData)
{
    ...
    {
        printf("\n");
        CsrBtScSspKeypressNotificationReqSend(instData->deviceAddr,
        ...
        else
        {
            ...
            CsrBtScSspPasskeyResSend(TRUE, instData->deviceAddr,
...
static void sspCompareHandler(ScAppInstData_t *instData)
{
    ...
    {
        CsrBtScSspCompareResSend(TRUE, instData->deviceAddr, TRUE, FALSE);
        printf("Y\n");
    }
...
static void sspNotificationHandler(ScAppInstData_t *instData)
{
    ...
    if (CSR_TOUPPER(key->key) == 'Y')
    {
        CsrBtScSspNotificationResSend(TRUE, instData->deviceAddr, TRUE, FALSE);
        printf("Y\n");
    }
    else if (CSR_TOUPPER(key->key) == 'N')
    {
...
{
    CsrBtScDebondReqSend(TESTQUEUE, instData->deviceAddr);
...
static void handleSetSecurityModeCfm(ScAppInstData_t *instData)
{
        ...
        prim->resultSupplier == CSR_BT_SUPPLIER_SC)
    {
        ...
        {
            /* Fetch new OOB data */
            CsrBtScReadLocalOobDataReqSend(TESTQUEUE);
        }
#endif
...
static void handleSecurityCancelPairing(ScAppInstData_t *instData)
{
    ...
    switch (key->key)
    {
        case 'C':
        case 'c':
            CsrBtScCancelBondReqSend(TESTQUEUE, instData->deviceAddr);
...
void handleScSspPassPrims(ScAppInstData_t *instData)
{
    ...
    switch (*prim)
    {
        ...
        case CSR_BT_SC_SSP_COMPARE_IND:
            {
                CsrBtScSspNotificationInd    *scNotifPrim;
                scNotifPrim = (CsrBtScSspNotificationInd *) instData->recvMsgP;

                instData->numericValue = scNotifPrim->numericValue;
                /* Fallthrough */
            }
...
static void handleScReadLocalOobDataCfm(ScAppInstData_t *instData)
{
        ...
        prim->resultSupplier == CSR_BT_SUPPLIER_SC)
    {
        ...
        instData->oobHashCLocal = prim->oobHashC;
        instData->oobRandRLocal = prim->oobRandR;
        prim->oobHashC = NULL;
        prim->oobRandR = NULL;
...
static void handleScLeSecurityInd(ScAppInstData_t *instData)
{
    ...
    printf("CsrBtScLeSecurityInd, %04x:%02x:%06x(%s), result 0x%04x, supplier 0x%04x\n",
