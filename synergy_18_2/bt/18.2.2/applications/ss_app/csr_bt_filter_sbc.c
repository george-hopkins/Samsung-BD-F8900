...
static CsrBool sbc_init(void **instance, struct av2filter_t *filter, void *unused);
...
void sbc_debug_config(sbc_instance_t *inst)
{
    ...
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "subbands..............%i\n", inst->subbands));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "blocks................%i\n", inst->blocks));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "sample_freq...........%i\n", inst->sample_freq));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "chan_mode.............%i (%c)\n", inst->channel_mode, ch_mode));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "alloc_method..........%i (%c)\n", inst->alloc_method, alloc_method));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "bitPool...............%i\n", inst->bitPool));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "channels..............%i\n", inst->channels));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "........................\n"));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "minBitpool............%i\n", inst->minBitpool));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "maxBitpool............%i\n", inst->maxBitpool));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "framesize.............%i\n", inst->framesize));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "frames_per_payload....%i\n", inst->frames_per_payload));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "bytes_per_encoding....%i\n", inst->bytes_per_encoding));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "remoteMinBitpool......%i\n", inst->remoteMinBitpool));
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "remoteMaxBitpool......%i\n", inst->remoteMaxBitpool));
}
#endif
...
//FILE	  *localPCMFile=NULL;

/* Initialise SBC filter */
static CsrBool sbc_init(void **instance, 
						struct av2filter_t *filter, void *unused)
{
    sbc_instance_t *inst;
    CsrUint8 bits;
    CsrUint16 bufsize;
    ...
    *instance = CsrPmemZalloc(sizeof(sbc_instance_t));
    ...
    inst->timestamp = 0;
	CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,"sbc_init\n"));
...
static CsrBool sbc_open(void **instance)
{
	...
	//localPCMFile=fopen("sbc_dump.sbc", "wb");

#ifdef DEBUG
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "open\n"));
#endif
    ...
    if(getPcmLocked())
    {
        ...
        {
            CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Error:  bitrate %u too high for mono, CSRMAX 266kbps.\n", targetBitrate));
           // exit(1);
        }
    }
    ...
    else
    {
        ...
        CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,SBC_PRN "invalid framesize\n"));
        return FALSE;
    }
...
*/
#ifdef DEBUG
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "close\n"));
#endif
  //  if(localPCMFile)
//	fclose(localPCMFile);
...
*/
#ifdef DEBUG
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "start\n"));
#endif
...
*/
#ifdef DEBUG
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "stop\n"));
#endif
                                 ...
                                 CsrUint32 length)
{
...
#ifdef DEBUG_VERBOSE
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "process_encode: sample length: %i, bytes_per_encoding: %i, estim_frames: %i\n",
           length,
           inst->bytes_per_encoding,
...
#endif
    {
        ...
        packet_ptr  = (CsrUint8 *)CsrPmemAlloc(packet_size);
...
#ifdef DEBUG_VERBOSE
        CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "process_encode: send %i frames (%i bytes) in packet size %i bytes, last framesize: %i\n",
               frames,
                                 ...
                                 CsrUint32 length)
{
    ...
    sbc_instance_t *inst;
	//if(localPCMFile)
	//fwrite(data,length,1,localPCMFile);
    ...
    do
    {
...
#ifdef DEBUG_VERBOSE
        CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "decode, bytes_left:%i, frame_size:%i\n", bytes_left, frame_size));
#endif
            ...
            for(block=0; ((block < inst->blocks) && (bufferProblem == FALSE)); block++)
            {
                for(subband=0; ((subband < inst->subbands) && (bufferProblem == FALSE)); subband++)
                {
                    for(chan=0; ((chan < inst->channels) && (bufferProblem == FALSE)); chan++)
                    {
                        ...
                        {
                            CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,SBC_PRN "SBC_DECODE_BUFFER_SIZE(%i) too small.\n", SBC_DECODE_BUFFER_SIZE));
                            bufferProblem = TRUE;
                        }
                    }
                }
        ...
        else
        {
            ...
            //CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,SBC_PRN "unable to decode SBC frame, discarding\n"));
            CsrPmemFree(data);
            return FALSE;
        }
...
#ifdef DEBUG_VERBOSE
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "done with this buffer, audio_len:%i, allocated %i\n",
                               ...
                               CsrUint8 *conf_len)
{
...
#ifdef DEBUG
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "get_config\n"));
#endif
    ...
    *conf_len = SBC_MEDIA_CODEC_SC_SIZE;
    ...
    switch(inst->sample_freq)
    {
        ...
        default:
            CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "invalid sample frequency!\n"));
            break;
    }
    ...
    switch(inst->channel_mode)
    {
        ...
        default:
            CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "invalid channel mode!\n"));
            break;
    }
#endif
    conf[4] = tmp;
    ...
    switch(inst->blocks)
    {
        ...
        default:
            CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "invalid block count number!\n"));
            break;
                                  ...
                                  CsrUint8 conf_len)
{
...
#ifdef DEBUG
    CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "set_config\n"));
#endif
    ...
    {
#ifdef DEBUG
        CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "set_config: unsupported config detected\n"));
#endif
                             ...
                             CsrUint8 *cap_len)
{
    ...
    *cap_len = SBC_MEDIA_CODEC_SC_SIZE + 2;
    cap =(CsrUint8 *) CsrPmemAlloc(*cap_len);
                                ...
                                CsrUint8 *conf_len)
{
    ...
    *conf_len = SBC_MEDIA_CODEC_SC_SIZE;
    ...
    if(getPcmLocked())
    {
        CsrUint8 tmp;
        CsrPmemFree(conf);
        CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,SBC_PRN "PCM settings are locked, returning current configuration\n"));
                     ...
                     CsrUint8 *num_options)
{
    ...
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"  0) SBC capabilities (0x%02X 0x%02X 0x%02X 0x%02X)\n",
           inst->sbc_capabilities[0],
           inst->sbc_capabilities[1],
           inst->sbc_capabilities[2],
           inst->sbc_capabilities[3]));
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"  1) Channel mode (%i)\n", inst->channel_mode));
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"  2) Allocation method (%i)\n", inst->alloc_method));
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"  3) PCM sample frequency (%i)\n", inst->sample_freq));
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"  4) PCM channels (%i)\n", inst->channels));
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"  5) SBC blocks (%i)\n", inst->blocks));
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"  6) SBC subbands (%i)\n", inst->subbands));
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"  7) Minimum bitpool (%i)\n", inst->minBitpool));
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"  8) Maximum bitpool (%i)\n", inst->maxBitpool));
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"  9) Maximum frame size (%i)\n", inst->maxFrameSize));
#endif
    *num_options = 10;
}
                        ...
                        char *value)
{
    ...
    if(i == 0)
    {
        CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Wrong selection or invalid format typed.\n"));
