...
#endif
extern av2filter_t filter_ss_decode; 
...
/* This is the default SINK filter chain */
	idx = 0;
	CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,"filtersInstall SBC_CODEC_TYPE\n"));

#ifdef USE_SBC
		instData->filters[idx++] = &filter_sbc_decode;
#endif
#ifdef USE_SBCFILE
		instData->filters[idx++] = &filter_sbcfile_writer;
#endif

#ifdef USE_WAVE
	instData->filters[idx++] = &filter_write_wave;
#endif
#ifdef USE_WINAUDIO
	instData->filters[idx++] = &filter_winplay;
#endif
#ifdef USE_ALSA
	instData->filters[idx++] = &filter_alsa_play;
#endif
#ifdef USE_FILEDUMP
	instData->filters[idx++] = &filter_filedump;
#endif
#ifdef USE_AVROUTER
	instData->filters[idx++] = &filter_avrouter_decode;
#endif

	instData->filters[idx++] = &av2_sink_filter;   /* Internal filter for stopping chain */
	...
	{
#ifdef DEBUG
		CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter combinations seems to be good\n"));
#endif
	}
	else
	{
		CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter combinations are invalid, check 'csr_bt_av2_filter.c' function filtersSetup()\n"));
	}
...
void filtersInstall(Av2AppInst *instData, CsrUint8 installedCodec)
{
   ...
   if( instData->installedCodec !=installedCodec)
   {
	/* This is the default SINK filter chain */
	    ...
	    {
           CSR_LOG_TEXT_DEBUG((CSR_SS_AP_LOG_ID,0,"filtersInstall SS_CODEC_TYPE\n"));
	    
	       instData->installedCodec=SS_CODEC_TYPE;
		   instData->filters[ idx++]= &filter_ss_decode;	
	    }
...
void filtersStartTimer(CsrTime interval)
{
    ...
    if(Av2Instance->filter_timer == 0)
    {
        ...
        CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Periodic timer started, interval %i usec\n", Av2Instance->filter_delay));
...
void filtersStopTimer()
{
    ...
    if(Av2Instance->filter_timer != 0)
    {
        ...
        Av2Instance->filter_interval = 0;
        Av2Instance->filter_lasttime = 0;
        CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Periodic timer stopped\n"));
    }
}
...
void filtersKill(Av2AppInst *instData)
{
    ...
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Deinitialising filters...\n"));
...
#ifdef DEBUG
            CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter %i: %15s (%s)\n",
...
#ifdef DEBUG
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filters killed\n"));
#endif
}
...
CsrBool filtersRun(Av2AppInst *instData, CsrUint8 op)
{
    ...
    name = "n/a";
#ifdef DEBUG	
	CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"filtersRun %d\n" ,op));
#endif
    for(i=0;
        ...
        switch(op)
        {
            case FILTER_INIT:
                ...
                if(*state == FILTER_ST_NOINIT)
                {
                    stateok = TRUE;
#ifdef DEBUG
                    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter init: Index %i: %s\n", i, name));
#endif
            ...
            case FILTER_DEINIT:
                ...
                if(*state == FILTER_ST_IDLE)
                {
                    stateok = TRUE;
#ifdef DEBUG
                    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter deinit: Index %i: %s\n", i, name));
#endif
            ...
            case FILTER_OPEN:
                   ...
                   (*state == FILTER_ST_OPEN))
                {
                    stateok = TRUE;
#ifdef DEBUG
                    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter open: Index %i: %s\n", i, name));
#endif
            ...
            case FILTER_CLOSE:
                ...
                if(*state == FILTER_ST_START)
                {
                    stateok = TRUE;
#ifdef DEBUG
                    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter halt: Index %i: %s\n", i, name));
#endif
                ...
                if(*state == FILTER_ST_OPEN)
                {
                    stateok = TRUE;
#ifdef DEBUG
                    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter close: Index %i: %s\n", i, name));
#endif
            ...
            case FILTER_START:
                ...
                if(*state == FILTER_ST_OPEN)
                {
                    stateok = TRUE;
#ifdef DEBUG
                    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter start: Index %i: %s\n", i, name));
#endif
            ...
            case FILTER_STOP:
                ...
                if(*state == FILTER_ST_START)
                {
                    stateok = TRUE;
#ifdef DEBUG
                    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter stop: Index %i: %s\n", i, name));
#endif
            ...
            default:
                CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Invalid opcode to filter helper\n"));
                result = FALSE;
                stateok = TRUE;
                state = NULL;
                break;
    ...
    if(!stateok)
    {
        CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter transition error: Index %i, opcode %i, state %i: %s\n",
               i, op, *state, name));
    }
    if(!result)
    {
        CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter function error: Index %i, opcode %i, state %i: %s\n",
               i, op, *state, name));
    }
    return result;
}
...
CsrBool filtersCheck(Av2AppInst *instData)
{
    ...
    {
#ifdef DEBUG
        CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Filter check: Index %i: %s : ",
        ...
        {
#ifdef DEBUG
            CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"producer "));
#endif
        ...
        {
#ifdef DEBUG
            CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"consumer "));
#endif
        ...
        {
#ifdef DEBUG
            CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"encoder "));
#endif
        ...
        {
#ifdef DEBUG
            CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"decoder "));
#endif
        ...
        {
#ifdef DEBUG
            CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"pass-through "));
#endif
        }
    ...
    if(fail)
    {
        CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Multiple producers/consumers/encoders/decoders detected!\n"));
    }
                          ...
                          CsrUint8 *length)
{
    ...
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Did not find a codec filter that supports 'get configuration'\n"));
    *length = 0;
    return NULL;
}
                             ...
                             CsrUint8 conf_len)
{
    ...
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Did not find a codec filter that supports 'set configuration'\n"));
                        ...
                        CsrUint8 *length)
{
    ...
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Did not find a codec filter that supports 'get capabilities'\n"));
    *length = 0;
    return NULL;
}
                           ...
                           CsrUint8 *conf_len)
{
    ...
    CSR_LOG_TEXT_CRITICAL((CSR_SS_AP_LOG_ID,0,"Did not find a codec filter that supports 'remote configuration'\n"));
    *conf_len = 0;
    return NULL;
}
