...
void startConnectAccept(hidhInstData_t *instData)
{
    ...
    else
    {
        ...
        if (!CsrBtBdAddrEq(&(instData->bdAddrList[instData->selectedDevice]), &zeroAddr))
        {
            sdpInfo = CsrPmemAlloc(sizeof(CsrBtHidhSdpInfo));
        ...
        case CSR_BT_HIDH_CONNECT_CFM:
            {
                ...
                {
                    printf("Establishment failed, remote device not responding. 0x%02X (supplier 0x%02X)\n", prim->resultCode, prim->resultSupplier);
                }
                playMenu(instData);
                        ...
                        case '3' :
                            {
                                ...
                                else
                                {
                                    ...
                                    if (!CsrBtBdAddrEq(&(instData->bdAddrList[instData->selectedDevice]), &zeroAddr))
                                    {
                                        Nvs_PutHidInfo(&(instData->bdAddrList[instData->selectedDevice]), NULL, NULL, 0, NULL);
                                        instData->numInList = instData->selectedDevice + 1;
                                        printf("Select from list of known devices:\n");
                                        printf(" %i) %04X:%02X:%06X\n",
                                               instData->selectedDevice,
