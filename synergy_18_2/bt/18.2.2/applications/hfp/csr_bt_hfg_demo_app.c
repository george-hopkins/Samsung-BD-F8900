...
void initHfgData(hfgInstance_t *inst)
{
    ...
    inst->current               = NO_IDX;
    inst->hfgSupportedFeatures  = CSR_BT_HFG_SUP_FEATURES;
    ...
    /* Clear C2C sf/indicators */
    for(i=0; i<CSR_BT_C2C_NUMBER_OF_INDICATORS; i++)
    {
        inst->c2cIndicators[i] = 0;
    }
    /* audioCodec Setup */
    inst->supportedAudioCodecMask      = CSR_BT_C2C_ADPCM_OFF_VALUE|CSR_BT_C2C_ADPCM_4BIT_VALUE|CSR_BT_C2C_ADPCM_2BIT_VALUE;
    inst->c2cIndicators[CSR_BT_C2C_ADPCM_IND] = CSR_BT_C2C_ADPCM_OFF_VALUE|CSR_BT_C2C_ADPCM_4BIT_VALUE|CSR_BT_C2C_ADPCM_2BIT_VALUE;
    inst->supportedSampleRate          = CSR_BT_C2C_SAMPLE_8KHZ_VALUE|CSR_BT_C2C_SAMPLE_16KHZ_VALUE;
    inst->c2cIndicators[CSR_BT_C2C_SAMPLE_RATE_IND] = CSR_BT_C2C_SAMPLE_8KHZ_VALUE|CSR_BT_C2C_SAMPLE_16KHZ_VALUE;
...
void CsrBtHfgAppInit(void **gash)
{
    ...
    *gash = (void*)CsrPmemZalloc(sizeof(hfgInstance_t));
    ...
    CsrBtHfgC2cSfReqSend(CSR_BT_HFG_CONNECTION_ALL ,6, 7);
    CsrBtHfgC2cSfReqSend(CSR_BT_HFG_CONNECTION_ALL ,7, 1);
#endif
...
void CsrBtHfgAppHandler(void * * gash)
{
    ...
    switch (event)
    {
        ...
        case CSR_BT_CM_PRIM:
        {
            ...
            if (*cmPrim == CSR_BT_CM_REGISTER_CFM)
            {
                ...
                {/* Success */
                    ...
                    {
                        sprintf(buf, "Reg succeess\n");
                    }
                    else
                    {
                        sprintf(buf, "Wrong srv channel: %d\n", prim->serverChannel);
                    }
                    CsrBtCmUnRegisterReqSend(prim->serverChannel);
                }
                else
                {/* Error */
                    sprintf(buf, "Reg failed; error: 0x%X\n", prim->resultCode);
                }
        ...
        default:
        {
            sprintf(buf, "Unknown primitive received: 0x%04x,\n", event);
