...
void CsrBtHfgDiscTimeout(CsrUint16 mi, void *mv)
{
    hfgInstance_t *inst;
    Connection_t *con;
    ...
    sprintf(buf, "Disconnect Timer has expired. It looks like the Headset is in bad state!!");
...
void hfgActivate(hfgInstance_t *inst)
{
    ...
    {
        CsrBtHfgActivateReqSend(inst->phandle,
                            ...
                            CsrBtHfgConnection type)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                sprintf(buf,"Connection already established to %04X:%02X:%06X\n",con->address.nap,con->address.uap,con->address.lap);
                CsrBtHfgSetDialog(inst, CSR_BT_HFG_ALREADY_CON_DIALOG_UI, TEXT_FAILED_UCS2,
                                                CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
                /* Show the CSR_BT_HFG_DEFAULT_INFO_UI on the display                   */
                CsrBtHfgShowUi(inst, CSR_BT_HFG_ALREADY_CON_DIALOG_UI, CSR_UI_INPUTMODE_AUTO, CSR_BT_HFG_STD_PRIO);
            ...
            else
            {
                ...
                sprintf(buf,"Connecting to %04X:%02X:%06X with connection type '%s'...\n",
                        con->address.nap,
                        con->address.uap,
                        con->address.lap,
...
void startCancelConnect(hfgInstance_t *inst)
{
    ...
    else
    {
        ...
        sprintf(buf,"Cancelling connection attempt...\n");
...
void startDisconnecting(hfgInstance_t *inst)
{
    ...
    else
    {
        ...
        inst->discGuard = CsrSchedTimerSet(HFG_DISC_GAURD,
                                       CsrBtHfgDiscTimeout,
                                       0,
                                       (void*)inst);
...
void toggleHfgConnections(hfgInstance_t *inst)
{
    ...
    {
        sprintf(buf, "No active connections found...\n");
        CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_FAILED_UCS2,
                                                                CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
    }
    /* Show the CSR_BT_HFG_DEFAULT_INFO_UI on the display                   */
...
void handleAtCmdChoiceMenu(hfgInstance_t *inst, CsrUint8 key)
{
    ...
    if (idx != 0xFF)
    {
        ...
        sprintf(buf, "Sent HFG_CONFIG_SINGLE_ATCMD_REQ!!");
    }
    else
    {/* Done: configure all */
        ...
        sprintf(buf, "Sent HFG_CONFIG_ATCMD_HANDLING _REQ");
    }
...
void toggleAudio(hfgInstance_t *inst)
{
    ...
    else
    {
        ...
        sprintf(buf, "Sent audio '%s' request...\n", (INVERT(con->audioOn) ? "on" : "off"));
        CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_PLEASE_WAIT_UCS2,
                                                                CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
        inputMode = CSR_UI_INPUTMODE_BLOCK;
        inst->dialogShow = TRUE;
...
void startOutgoingCall(hfgInstance_t *inst, char *ringNumber)
{
    ...
    else
    {
        ...
        if (con != NULL)
        {
            ...
            {
                CsrBtHfgAudioConnectReqSend(con->index,PCM_SLOT, PCM_SLOT_REALLOCATE);
                sprintf(buf, "Call active, request to open audio...\n");
            }
            else
            {
                sprintf(buf, "Call active, audio already open...\n");
            }
        }
        else
        {
            sprintf(buf, "Outgoing call started from AG, no HF-connection exists yet...\n");
        }
        ...
        else
        {
            ...
            if(con != NULL)
            {
                CsrBtHfgCallWaitingReqSend(con->index,
                                      ...
                                      (CsrCharString *) StringDup((char*)inst->name),
                                      NUMBER_TYPE_INT);
                sprintf(buf, "Sent call waiting notification...\n");
                CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_SUCCESS_UCS2,
                                                                        CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
            }
...
void startAnswer(hfgInstance_t *inst)
{
    ...
    else
    {
        ...
        if(con != NULL)
        {
            ...
            {
                CsrBtHfgAudioConnectReqSend(con->index, PCM_SLOT, PCM_SLOT_REALLOCATE);
                sprintf(buf, "Requested audio on for answered call...\n");
                inputMode = CSR_UI_INPUTMODE_BLOCK;
                inst->dialogShow = TRUE;
                dataReady = TRUE;
            }
        ...
        if(!dataReady)
        {
            sprintf(buf, "Sent 'call-active' status indicator and 'no-call-setup' setup indicator...\n");
        }
        else
        {
...
void startReject(hfgInstance_t *inst)
{
    ...
    if(con != NULL)
    {
        ...
        if(con->audioOn)
        {
            ...
            sprintf(buf, "Sent Audio Disconnect Request!!\n");
            CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_PLEASE_WAIT_UCS2,
                                                                    CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
            inputMode = CSR_UI_INPUTMODE_BLOCK;
            inst->dialogShow = TRUE;
    ...
    else
    {
        ...
        if(call != NO_CALL)
        {
            sprintf(buf, "Waiting call has been dropped (index %i)\n", call);
            CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_SUCCESS_UCS2,
                                                                    CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
    ...
    if(call == NO_CALL)
    {
        sprintf(buf, "No active/incoming call to reject\n");
        CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_FAILED_UCS2,
                                                                CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
    }
    CsrBtHfgShowUi(inst, CSR_BT_HFG_DEFAULT_INFO_UI, inputMode, CSR_BT_HFG_STD_PRIO);
...
void startPutOnHold(hfgInstance_t *inst)
{
    ...
    if(call != NO_CALL)
    {
        ...
        if(call != NO_CALL)
        {
            ...
            sprintf(buf, "Call handling command: Incoming call is now on hold (index %i)\n", call);
            CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_SUCCESS_UCS2,
                                                                    CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
        ...
        {
            sprintf(buf, "No incoming calls to put on hold\n");
            CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_FAILED_UCS2,
                                                                    CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
        }
    }
...
void startAnswerCallOnHold(hfgInstance_t *inst)
{
    ...
    if(call != NO_CALL)
    {
        ...
        if(call != NO_CALL)
        {
            ...
            sprintf(buf, "Call handling command: Answer call on hold (index %i)\n", call);
            CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_FAILED_UCS2,
                                                                    CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
        ...
        {
            sprintf(buf, "No held calls to answer\n");
            CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_FAILED_UCS2,
                                                                    CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
        }
    }
...
void startRejectCallOnHold(hfgInstance_t *inst)
{
    ...
    if(call != NO_CALL)
    {
        ...
        if(call != NO_CALL)
        {
            ...
            sprintf(buf, "Call handling command: Held call is now Rejected (index %i)\n", call);
            CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_FAILED_UCS2,
                                                                    CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
        }
        else
        {
        sprintf(buf, "No held calls to reject\n");
        CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_FAILED_UCS2,
                                                                CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
        }
    }
...
/* Return C2C indicator for remote/local */
static char *settingC2c(hfgInstance_t *inst, CsrUint8 ind)
{
    CsrBool local;
    CsrBool remote;
    static char *loc  = "local";
    static char *rem  = "remote";
    static char *both = "local + remote";
    static char *none = "n/a";

    local = FALSE;
    remote = FALSE;

    if(inst->c2cIndicators[ind] != 0)
    {
        local = TRUE;
    }
    if(inst->current != NO_IDX)
    {
        if(inst->conInst[inst->current].c2cIndicators[ind] != 0)
        {
            remote = TRUE;
        }
    }
...
void displayFeatures(hfgInstance_t *inst)
{
    ...
    sprintf((char*)buf, "Locally supported HFG features:\n Supported features bitmask:(0x%08x)\n Three way calling: (%s)\n \
    ...
    if(con != NULL)
    {
    sprintf((char*)buf1, "Locally activated HFG features:\n Echo/noise reduction(%s)\n Voice recognition (%s)\n \
    ...
    CsrStrCat(buf,buf1);
    sprintf((char*)buf1, "CSR-2-CSR supported features/indicators:\n Name notifications (%s)\n \
        Text-to-speech support (%s)\n SMS notification/text support   (%s)\n \
        Battery indications (%s)\n Power status indications (%s)\n",
        settingC2c(inst, CSR_BT_C2C_NAME_IND),
        settingC2c(inst, CSR_BT_C2C_TXT_IND),
        settingC2c(inst, CSR_BT_C2C_SMS_IND),
        settingC2c(inst, CSR_BT_C2C_BAT_IND),
        settingC2c(inst, CSR_BT_C2C_PWR_IND));
    CsrStrCat(buf,buf1);
        ...
        case CSR_BT_HFG_CL_CHANGE_ACTIVE_KEY:
            {
                ...
                sprintf(buf, "Call status changed to Active!!");
                break;
            }
        ...
        case CSR_BT_HFG_CL_TOGGLE_DIR_KEY:
            {
                ...
                sprintf(buf, "Direction is Toggled!!");
                break;
            }
        case CSR_BT_HFG_CL_TRAVERSE_STATUS_KEY:
            {
                ...
                updateCievCallParameters(inst);
                sprintf(buf, "Status Traverse(active/held/dial/alert/incoming/wait) is done!!");
                break;
            }
        case CSR_BT_HFG_CL_TOGGLE_MULTIPARTY_KEY:
            {
                ...
                sprintf(buf, "Multiparty status is Toggled!!");
                break;
            }
        default:
            {
...
void toggleRoam(hfgInstance_t *inst)
{
    ...
    sprintf(buf, "Sent roam '%s' status indicator...\n",
...
void toggleCellRegStatus(hfgInstance_t *inst)
{
    ...
    CsrBtHfgStatusIndicatorSetReqSend(CSR_BT_HFG_CONNECTION_ALL,
                                 ...
                                 (CsrUint8)(inst->regStatus
                                           ? CSR_BT_SERVICE_PRESENT_VALUE
                                           : CSR_BT_NO_SERVICE_VALUE));
    sprintf(buf, "Sent cellular registration '%s' status indicator...\n",
...
void toggleSignal(hfgInstance_t *inst, CsrBool inc)
{
    ...
    CsrBtHfgStatusIndicatorSetReqSend(CSR_BT_HFG_CONNECTION_ALL,
                                 ...
                                 inst->signal);
    sprintf(buf, "Send signal strength indicator with value %i...\n", inst->signal);
    CsrBtHfgSetDialog(inst, CSR_BT_HFG_DEFAULT_INFO_UI, TEXT_SUCCESS_UCS2,
                                                        CONVERT_TEXT_STRING_2_UCS2(buf), TEXT_OK_UCS2, NULL);
    CsrBtHfgShowUi(inst, CSR_BT_HFG_DEFAULT_INFO_UI, CSR_UI_INPUTMODE_AUTO, CSR_BT_HFG_STD_PRIO);
...
void toggleBattery(hfgInstance_t *inst, CsrBool inc)
{
    ...
    CsrBtHfgStatusIndicatorSetReqSend(CSR_BT_HFG_CONNECTION_ALL,
                                 ...
                                 inst->battery);
    sprintf(buf, "Send battery charge indicator with value %i...\n", inst->battery);
...
void toggleMic(hfgInstance_t *inst, CsrBool inc)
{
    ...
    {
        sprintf(buf, "No active connection selected\n");
        CsrStrCpy(result, TEXT_FAILED_STRING);
    }
    else
    {
        if (con->hfSupportedFeatures & CSR_BT_HF_SUPPORT_REMOTE_VOLUME_CONTROL)
        {
            ...
            CsrBtHfgMicGainReqSend(con->index,
                              con->micGain);
            sprintf(buf, "Send microphone gain value %i...\n", con->micGain);
            CsrStrCpy(result, TEXT_SUCCESS_STRING);
        }
        else
        {
            sprintf(buf, "Remote volume control not activated on HF side.\n");
            CsrStrCpy(result, TEXT_FAILED_STRING);
        }
    }
    ...
    else
    {
        if (con->hfSupportedFeatures & CSR_BT_HF_SUPPORT_REMOTE_VOLUME_CONTROL)
        {
            ...
            CsrBtHfgSpeakerGainReqSend(con->index,
                                  con->speakerGain);
            sprintf(buf, "Send speaker gain value %i...\n", con->speakerGain);
            CsrStrCpy(result, TEXT_SUCCESS_STRING);
        }
        else
        {
...
void toggleInband(hfgInstance_t *inst, CsrBool on)
{
    ...
    else
    {
        ...
        CsrBtHfgInbandRingingReqSend(con->index,
                                con->inbandRinging);
        sprintf(buf, "Sent inband ringing '%s' setting...\n",
...
void toggleVoiceRecog(hfgInstance_t *inst, CsrBool on)
{
    ...
    else
    {
        if (con->hfSupportedFeatures & CSR_BT_HF_SUPPORT_VOICE_RECOGNITION)
        {
            ...
            sprintf(buf, "Sent voice recognition '%s' setting...\n",
