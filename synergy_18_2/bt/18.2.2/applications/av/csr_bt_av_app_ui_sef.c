        ...
        case CSR_BT_AV_CHANGE_FILENAME_KEY:
        {
            ...
            else
            {
                CsrCharString displayString[500];
                      #ifndef _WIN32_WCE
                sprintf((char*)displayString,"Change File Name - Current File Name is :%s\n",
...
{ /* This function handles the SK1 action event from the CSR_BT_AV_ESTABLISH_CONN_MEANU_UI  */
    ...
    switch (key)
    {
...
#ifdef CSR_USE_DSPM
        case CSR_BT_AV_CHOOSE_ENCODER_KEY:
        {
            if((inst->avCon[inst->currentConnection].state == DaAvStateAvConnected)||
                (inst->avCon[inst->currentConnection].state == DaAvStateAvConnectedOpen))
            {/* Show menu to choose the on-chip encoder */
                CsrBtAvShowUi(inst, CSR_BT_AV_CHOOSE_ONCHIP_ENCODER_MENU_UI, CSR_UI_INPUTMODE_AUTO, CSR_BT_AV_STD_PRIO);
            }
            else
            {
                if (inst->avCon[inst->currentConnection].state == DaAvStateAvConnectedStreaming)
                {
                    CsrBtAvSetDialog(inst, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
                                      CONVERT_TEXT_STRING_2_UCS2("Streaming - to change the encoder close the stream first \n") , TEXT_OK_UCS2, NULL);
                }
                else
                {
                    CsrBtAvSetDialog(inst, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
        ...
        case CSR_BT_AV_CHOOSE_AUDIO_INPUT_KEY:
        {
            if(inst->avCon[inst->currentConnection].state == DaAvStateAvConnected)
            {/* Show menu to choose the on-chip encoder */
                CsrBtAvShowUi(inst, CSR_BT_AV_CHOOSE_ONCHIP_INPUT_MENU_UI, CSR_UI_INPUTMODE_AUTO, CSR_BT_AV_STD_PRIO);
            }
            else
            {
                if (inst->avCon[inst->currentConnection].state == DaAvStateAvConnectedStreaming)
...
#endif
#ifdef CSR_USE_DSPM
         CsrUiMenuAdditemReqSend(displayHandle, CSR_UI_LAST, CSR_BT_AV_CHOOSE_ENCODER_KEY, CSR_UI_ICON_NONE,
                          TEXT_AV_CHOOSE_ENCODER_UCS2, NULL, sk1Handle,
                          CSR_UI_DEFAULTACTION, prim->handle, CSR_UI_DEFAULTACTION);

         CsrUiMenuAdditemReqSend(displayHandle, CSR_UI_LAST, CSR_BT_AV_CHOOSE_AUDIO_INPUT_KEY, CSR_UI_ICON_NONE,
                          TEXT_AV_CHOOSE_ENCODER_INPUT_UCS2, NULL, sk1Handle,
                          CSR_UI_DEFAULTACTION, prim->handle, CSR_UI_DEFAULTACTION);
#endif         
...
{ /* This function handles the SK1 action event from the CSR_BT_AV_ACTIVATE_MENU_UI  */
    ...
    switch (key)
    {
        ...
        {
                  #ifndef _WIN32_WCE
            sprintf((char*)displayString, "\t\tDetails for connection: %d\n\t\tAV connection ID:%d\n\t\tAV In use:%s\n\t\tStream handle:%d\n\t\tRemote seid:%d\n",
    ...
    else
    {
        ...
        { /* CSR_BT_AV_CREATE_BACK_EVENT                                           */
            ...
            CsrUiMenuAdditemReqSend(displayHandle, CSR_UI_LAST, CSR_BT_AV_SINK_MODE_KEY,
                            ...
                            CSR_UI_DEFAULTACTION, prim->handle, CSR_UI_DEFAULTACTION);
#ifdef CSR_USE_DSPM
            /* Create CSR_BT_AV_CHOOSE_ONCHIP_ENCODER_MENU_UI                        */
            csrUiVar->uiIndex = CSR_BT_AV_CHOOSE_ONCHIP_ENCODER_MENU_UI;
#else
...
#ifdef CSR_USE_DSPM
static void appUiKeyHandlereAvChoseEncoderInputMenu(void * instData,
                   CsrUieHandle displayHandle, CsrUieHandle eventHandle, CsrUint16 key)
{ /* This function handles the SK1 action event from the CSR_BT_AV_CON_MODE_MENU_UI  */
    av2instance_t * inst = instData;
    CSR_UNUSED(eventHandle);
    ...
    switch (key)
    {
        case CSR_BT_AV_CHOOSE_CODEC_AUDIO_INPUT_KEY:
        {
            inst->nextEndPointType = CSR_DSPM_CP_TYPE_CODEC;
            
            if ((inst->avCon[inst->currentConnection].state == DaAvStateAvConnectedStreaming)||
                (inst->avCon[inst->currentConnection].state == DaAvStateAvConnectedOpen))
            {
                CsrBtAvSetDialog(inst, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
                                  CONVERT_TEXT_STRING_2_UCS2("Not possible to change the encoder input when a stream is open. The changes will affect the next stream \n") , TEXT_OK_UCS2, NULL);
            }
            else
            {
               inst->avCon[inst->currentConnection].avDspmInst.endPointType = CSR_DSPM_CP_TYPE_CODEC;

               CsrBtAvSetDialog(inst, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
                                TEXT_AV_CODEC_ENDPOINT_SELECTED_UCS2 , TEXT_OK_UCS2, NULL);
            }
            break;
        }
        case CSR_BT_AV_CHOOSE_PCM_AUDIO_INPUT_KEY:
        {
            inst->nextEndPointType = CSR_DSPM_CP_TYPE_PCM;
            if ((inst->avCon[inst->currentConnection].state == DaAvStateAvConnectedStreaming)||
                (inst->avCon[inst->currentConnection].state == DaAvStateAvConnectedOpen))
            {
                CsrBtAvSetDialog(inst, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
            ...
            {
               inst->avCon[inst->currentConnection].avDspmInst.endPointType = CSR_DSPM_CP_TYPE_PCM;

               CsrBtAvSetDialog(inst, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
                                TEXT_AV_PCM_ENDPOINT_SELECTED_UCS2 , TEXT_OK_UCS2, NULL);
            }
            break;
        }
        default : /*                                         */
        {
            CsrGeneralException("CSR_BT_AV", 0, ((CsrUiUieCreateCfm *) (inst->recvMsgP))->type,
                        "AV Choose Encoder Input Menu Key \n");
            break;
        }
    }
...
static void appCreateAvChooseOnchipInputMenuUi(av2instance_t * inst)
{
    CsrBtAVCsrUiType    * csrUiVar  = &(inst->csrUiVar);
    CsrUiUieCreateCfm * prim     = (CsrUiUieCreateCfm *) inst->recvMsgP;
    ...
    if (prim->elementType == CSR_UI_UIETYPE_MENU)
    {
        csrUiVar->eventState = CSR_BT_AV_CREATE_SK1_EVENT;
        csrUiVar->displayesHandlers[csrUiVar->uiIndex].displayHandle       = prim->handle;
        csrUiVar->displayesHandlers[csrUiVar->uiIndex].sk1EventHandleFunc  = appUiKeyHandlereAvChoseEncoderInputMenu;
        csrUiVar->displayesHandlers[csrUiVar->uiIndex].backEventHandleFunc = appUiKeyHandlerHideMics;

        /* Set the menu on the CSR_BT_AV_CHOOSE_ONCHIP_INPUT_MENU_UI */
        CsrUiMenuSetReqSend(prim->handle, TEXT_AV_CHOOSE_ENCODER_INPUT_UCS2, TEXT_SELECT_UCS2, NULL);
    ...
    else
    {
        ...
        { /* CSR_BT_AV_CREATE_BACK_EVENT                                           */
            ...
            /* Updated the CSR_BT_AV_CHOOSE_ONCHIP_INPUT_MENU_UI to include all the supported
               client applications                                              */
            CsrUiMenuAdditemReqSend(displayHandle, CSR_UI_FIRST, CSR_BT_AV_CHOOSE_CODEC_AUDIO_INPUT_KEY,
                            CSR_UI_ICON_NONE, TEXT_AV_CHOOSE_ENCODER_CODEC_UCS2, NULL, sk1Handle,
                            CSR_UI_DEFAULTACTION, prim->handle, CSR_UI_DEFAULTACTION);

            CsrUiMenuAdditemReqSend(displayHandle, CSR_UI_LAST, CSR_BT_AV_CHOOSE_PCM_AUDIO_INPUT_KEY,
                            CSR_UI_ICON_NONE, TEXT_AV_CHOOSE_ENCODER_PCM_UCS2, NULL, sk1Handle,
                            CSR_UI_DEFAULTACTION, prim->handle, CSR_UI_DEFAULTACTION);
...
/* Static functions to create the CSR_BT_AV_CHOOSE_ONCHIP_ENCODER_MENU_UI and handle key events
   releated to this UI                                                          */
static void appUiKeyHandlereAvChoseEncoderMenu(void * instData,
                   CsrUieHandle displayHandle, CsrUieHandle eventHandle, CsrUint16 key)
{ /* This function handles the SK1 action event from the CSR_BT_AV_CON_MODE_MENU_UI  */
    av2instance_t * inst = instData;
    CSR_UNUSED(eventHandle);
    ...
    switch (key)
    {
        case CSR_BT_AV_CHOOSE_SBC_ENCODER_KEY:
        {
            inst->nextEncoderToUse = CSR_DSPM_CAPABILITY_SBC_ENCODER;
            if ((inst->avCon[inst->currentConnection].state == DaAvStateAvConnectedStreaming)||
                (inst->avCon[inst->currentConnection].state == DaAvStateAvConnectedOpen))
            {
                CsrBtAvSetDialog(inst, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
                                  CONVERT_TEXT_STRING_2_UCS2("Not possible to change the encoder when a stream is open. The changes will affect the next stream \n") , TEXT_OK_UCS2, NULL);
            }
            else
            {
               inst->avCon[inst->currentConnection].avDspmInst.encoderToUse = CSR_DSPM_CAPABILITY_SBC_ENCODER;

               CsrBtAvSetDialog(inst, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
                                TEXT_AV_SBC_ENCODER_SELECTED_UCS2 , TEXT_OK_UCS2, NULL);
            }
            break;
        }
        case CSR_BT_AV_CHOOSE_APTX_ENCODER_KEY:
        {
            inst->nextEncoderToUse = CSR_DSPM_CAPABILITY_APTX_ENCODER;
            if ((inst->avCon[inst->currentConnection].state == DaAvStateAvConnectedStreaming)||
                (inst->avCon[inst->currentConnection].state == DaAvStateAvConnectedOpen))
            {
                CsrBtAvSetDialog(inst, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
            ...
            {
               inst->avCon[inst->currentConnection].avDspmInst.encoderToUse = CSR_DSPM_CAPABILITY_APTX_ENCODER;

               CsrBtAvSetDialog(inst, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
                                TEXT_AV_APTX_ENCODER_SELECTED_UCS2 , TEXT_OK_UCS2, NULL);
            }
            break;
        }
        default : /*                                         */
...
static void appCreateAvChooseOnchipEncoderMenuUi(av2instance_t * inst)
{ 
    ...
    if (prim->elementType == CSR_UI_UIETYPE_MENU)
    {
        csrUiVar->eventState = CSR_BT_AV_CREATE_SK1_EVENT;
        csrUiVar->displayesHandlers[csrUiVar->uiIndex].displayHandle       = prim->handle;
        csrUiVar->displayesHandlers[csrUiVar->uiIndex].sk1EventHandleFunc  = appUiKeyHandlereAvChoseEncoderMenu;
        csrUiVar->displayesHandlers[csrUiVar->uiIndex].backEventHandleFunc = appUiKeyHandlerHideMics;

        /* Set the menu on the CSR_BT_AV_CHOOSE_ONCHIP_ENCODER_MENU_UI */
        CsrUiMenuSetReqSend(prim->handle, TEXT_AV_CHOOSE_ENCODER_UCS2, TEXT_SELECT_UCS2, NULL);
    ...
    else
    {
        ...
        { /* CSR_BT_AV_CREATE_BACK_EVENT                                           */
            ...
            /* Updated the CSR_BT_AV_CHOOSE_ONCHIP_ENCODER_MENU_UI to include all the supported
               client applications                                              */
            CsrUiMenuAdditemReqSend(displayHandle, CSR_UI_FIRST, CSR_BT_AV_CHOOSE_SBC_ENCODER_KEY,
                            CSR_UI_ICON_NONE, TEXT_AV_CHOOSE_SBC_ENCODER_UCS2, NULL, sk1Handle,
                            CSR_UI_DEFAULTACTION, prim->handle, CSR_UI_DEFAULTACTION);

            CsrUiMenuAdditemReqSend(displayHandle, CSR_UI_LAST, CSR_BT_AV_CHOOSE_APTX_ENCODER_KEY,
                            CSR_UI_ICON_NONE, TEXT_AV_CHOOSE_APTX_ENCODER_UCS2, NULL, sk1Handle,
                            CSR_UI_DEFAULTACTION, prim->handle, CSR_UI_DEFAULTACTION);
            
            /* Create  CSR_BT_AV_MAIN_MENU_UI                                        */
            csrUiVar->uiIndex = CSR_BT_AV_CHOOSE_ONCHIP_INPUT_MENU_UI;
            CsrUiUieCreateReqSend(CsrSchedTaskQueueGet(), CSR_UI_UIETYPE_MENU);
        }
    }
}
...
#ifdef CSR_USE_DSPM
    appCreateAvChooseOnchipEncoderMenuUi,                 /* CSR_BT_AV_CHOOSE_ONCHIP_ENCODER_MENU_UI */
    appCreateAvChooseOnchipInputMenuUi,                   /* CSR_BT_AV_CHOOSE_ONCHIP_INPUT_MENU_UI */
#endif
