...
void handleAvConnectInd(av2instance_t *instData, CsrBtAvConnectInd *prim)
{
    ...
    if (conId < MAX_CONNECTIONS)
    {
...
#ifdef CSR_USE_DSPM
        {
            ...
            CsrMemSet(&(instData->avCon[conId].avDspmInst), 0 ,sizeof(AvDspmType));
            instData->avCon[conId].avDspmInst.remoteCid       = CSR_BT_AV2_UNKNOWN_ID;
            instData->avCon[conId].avDspmInst.aclHandle       = CSR_BT_AV2_UNKNOWN_ID;
            instData->avCon[conId].avDspmInst.operatorEncoder = CSR_BT_AV2_UNKNOWN_ID;
            instData->avCon[conId].avDspmInst.num_tokens      = CSR_BT_AV2_DSPM_DEFAULT_NUM_TOKENS;
            instData->avCon[conId].avDspmInst.encoderToUse    = instData->nextEncoderToUse;
        ...
        CsrAppBacklogReqSend(TECH_BT, PROFILE_AV, FALSE,"Connection established (id: %d)\n", conId);
                       ...
                       #ifndef _WIN32_WCE
        sprintf((char*)displayString, "\t\tConnection established (id: %d)\n",conId);
#endif
...
void handleAvConnectCfm(av2instance_t *instData, CsrBtAvConnectCfm *prim)
{
        ...
        prim->avResultCode == CSR_BT_RESULT_CODE_AV_SUCCESS)
    {
        ...
        {
            /* Get channel info and disable low power mode negotiation started from the local device */
            CsrMemSet(&(instData->avCon[instData->currentConnection].avDspmInst), 0 ,sizeof(AvDspmType));

            instData->avCon[instData->currentConnection].avDspmInst.remoteCid       = CSR_BT_AV2_UNKNOWN_ID;
            instData->avCon[instData->currentConnection].avDspmInst.aclHandle       = CSR_BT_AV2_UNKNOWN_ID;
            instData->avCon[instData->currentConnection].avDspmInst.operatorEncoder = CSR_BT_AV2_UNKNOWN_ID;
            instData->avCon[instData->currentConnection].avDspmInst.num_tokens      = CSR_BT_AV2_DSPM_DEFAULT_NUM_TOKENS;
            instData->avCon[instData->currentConnection].avDspmInst.encoderToUse    = instData->nextEncoderToUse;
            CsrBtAvLpNegConfigReqSend(FALSE);
        }
#endif
...
void handleAvGetCapabilitiesCfm(av2instance_t *instData, CsrBtAvGetCapabilitiesCfm *prim)
{
            ...
            if(servCap != NULL)
            {
                ...
                if(conf_len > 0)
                {
                    ...
                    {/* Now send delay report */
                        CsrBtAvDelayReportReqSend(50,instData->avCon[instData->currentConnection].connectionId,ASSIGN_TLABEL);
                    }
                    found = TRUE;
                }
                else
    ...
    else
    {
        ...
        CsrBtAvHideUi(instData, CSR_BT_AV_OPEN_STREAM_SPECIFIC_DIALOG_UI);
      #ifndef _WIN32_WCE
        sprintf((char*)displayString,"Get capabilities rejected, error code 0x%04x (0x%04x): %s\n",
                    prim->avResultCode,
               prim->avResultSupplier,
               getAvErrorString(prim->avResultSupplier, prim->avResultCode));
      #endif
...
void handleAvGetCapabilitiesInd(av2instance_t *instData, CsrBtAvGetCapabilitiesInd *prim)
{
    ...
    if ((caps) &&
        (conIndex < MAX_CONNECTIONS) &&
        (instData->localSeid[instData->avCon[conIndex].localSeidIndex] == prim->acpSeid))
    {
...
void handleAvSetConfigurationCfm(av2instance_t *instData, CsrBtAvSetConfigurationCfm *prim)
{
        ...
        prim->avResultCode == CSR_BT_RESULT_CODE_AV_SUCCESS)
    {
        ...
        else
        {
            CsrAppBacklogReqSend(TECH_BT, PROFILE_AV, FALSE,"Unable to configure stream end-point (filter open error)\n");
                  #ifndef _WIN32_WCE
            sprintf((char*)displayString,"Unable to configure stream end-point (filter open error)\n");
                  #endif
            CsrBtAvHideUi(instData, CSR_BT_AV_OPEN_STREAM_SPECIFIC_DIALOG_UI);
            CsrBtAvSetDialog(instData, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
                                           CONVERT_TEXT_STRING_2_UCS2(displayString) , TEXT_OK_UCS2, NULL);
    ...
    else
    {
        CsrAppBacklogReqSend(TECH_BT, PROFILE_AV, FALSE,"Set configuration rejected, error code 0x%x (0x%04x): %s\n",
               ...
               getAvErrorString(prim->avResultSupplier, prim->avResultCode));
      #ifndef _WIN32_WCE
        sprintf((char*)displayString,"Set configuration rejected, error code 0x%x (0x%04x): %s\n",
               prim->avResultCode,
               prim->avResultSupplier,
               getAvErrorString(prim->avResultSupplier, prim->avResultCode));
#endif
...
void handleAvSetConfigurationInd(av2instance_t *instData, CsrBtAvSetConfigurationInd *prim)
{
    ...
    if (conIndex < MAX_CONNECTIONS)
    {
        ...
        if(instData->localSeid[instData->avCon[conIndex].localSeidIndex] == prim->acpSeid)
        {
    ...
    CsrAppBacklogReqSend(TECH_BT, PROFILE_AV, FALSE,"Configuration has been rejected (Error code: 0x%x)\n", result);
          #ifndef _WIN32_WCE
    sprintf((char*)displayString,"Configuration has been rejected (Error code: 0x%x)\n",
               result);
          #endif
    CsrBtAvSetDialog(instData, CSR_BT_AV_DEFAULT_INFO_UI, NULL,
                                       CONVERT_TEXT_STRING_2_UCS2(displayString) , TEXT_OK_UCS2, NULL);
...
void handleAvStartCfm(av2instance_t *instData, CsrBtAvStartCfm *prim)
{
    ...
    else
    {
        CsrAppBacklogReqSend(TECH_BT, PROFILE_AV, FALSE,"Start stream rejected, error code 0x%04x (0x%04x): %s\n",
               ...
               getAvErrorString(prim->avResultSupplier, prim->avResultCode));
              #ifndef _WIN32_WCE
        sprintf((char*)displayString,"Error: Start stream rejected, error code 0x%04x (0x%04x): %s\n",
               prim->avResultCode,
               prim->avResultSupplier,
               getAvErrorString(prim->avResultSupplier, prim->avResultCode));
              #endif
...
void handleAvStartInd(av2instance_t *instData, CsrBtAvStartInd *prim)
{
    ...
    for (i=0; i<prim->listLength; i++)
    {
        ...
        if(shandle == instData->avCon[conIndex].streamHandle)
        {
...
void handleAvCloseCfm(av2instance_t *instData, CsrBtAvCloseCfm *prim)
{
    ...
    else
    {
        CsrAppBacklogReqSend(TECH_BT, PROFILE_AV, FALSE,"Close stream rejected, error code 0x%04x (0x%04x): %s\n",
               ...
               getAvErrorString(prim->avResultSupplier, prim->avResultCode));
              #ifndef _WIN32_WCE
        sprintf((char*)displayString,"Close stream rejected, error code 0x%04x (0x%04x): %s\n",
               prim->avResultCode,
               prim->avResultSupplier,
               getAvErrorString(prim->avResultSupplier, prim->avResultCode));
#endif
...
void handleAvCloseInd(av2instance_t *instData, CsrBtAvCloseInd *prim)
{
    ...
    conIndex = getIndexFromShandle(instData, prim->shandle);
    if(prim->shandle == instData->avCon[conIndex].streamHandle)
    {
...
void handleAvSuspendCfm(av2instance_t *instData, CsrBtAvSuspendCfm *prim)
{
    ...
    else
    {
        ...
        CsrAppBacklogReqSend(TECH_BT, PROFILE_AV, FALSE,"Suspend stream rejected, error code 0x%04x (0x%04x): %s\n",
               ...
               getAvErrorString(prim->avResultSupplier, prim->avResultCode));
              #ifndef _WIN32_WCE
        sprintf((char*)displayString,"Start stream rejected, error code 0x%04x (0x%04x): %s\n",
               prim->avResultCode,
               prim->avResultSupplier,
               getAvErrorString(prim->avResultSupplier, prim->avResultCode));
              #endif
...
void handleAvGetConfigurationCfm(av2instance_t *instData, CsrBtAvGetConfigurationCfm *prim)
{
        ...
        prim->avResultCode == CSR_BT_RESULT_CODE_AV_SUCCESS)
    {
        ...
        CsrAppBacklogReqSend(TECH_BT, PROFILE_AV, FALSE,"Remote device reported the current stream configuration:\n");
              #ifndef _WIN32_WCE
        sprintf((char*)displayString,"Remote device reported the current stream configuration:\n");
              #endif
        index = 0;
        servCap = prim->servCapData;
    ...
    else
    {
        ...
        CsrAppBacklogReqSend(TECH_BT, PROFILE_AV, FALSE,"Get stream configuration rejected, error code 0x%04x (0x%04x): %s\n",
               ...
               getAvErrorString(prim->avResultSupplier, prim->avResultCode));
              #ifndef _WIN32_WCE
        sprintf((char*)displayString,"Get stream configuration rejected, error code 0x%04x (0x%04x): %s\n",
               prim->avResultCode,
               prim->avResultSupplier,
               getAvErrorString(prim->avResultSupplier, prim->avResultCode));
#endif
...
void handleAvGetConfigurationInd(av2instance_t *instData, CsrBtAvGetConfigurationInd *prim)
{
    ...
    conIndex = getIndexFromShandle(instData, prim->shandle);
    if(instData->localSeidInUse[instData->avCon[conIndex].localSeidIndex] != 0)
    {
        CsrUint8 *conf,*ptr;
        CsrUint8 conf_len;
