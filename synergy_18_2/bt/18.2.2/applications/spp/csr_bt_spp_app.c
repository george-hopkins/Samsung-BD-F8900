...
{
    char *buf;
    buf = CsrPmemAlloc(CsrStrLen("SPP instance xxxx")+1);
   #ifndef _WIN32_WCE
    sprintf(buf, "SPP instance %04X", instance);
   #endif
    return buf;
}
            ...
            switch(*type)
            {
                ...
                { /* Only received by extra spp demo app instances */
                    ...
                    sppData->queueId = prim->queueId;
                       #ifndef _WIN32_WCE
                    sprintf(sppData->sppInstance.comPortName,"COM%d",prim->instance);
                    sprintf(sppData->sppInstance.outputFileName,"CSR_BT_SPP%02d.txt",prim->instance);
                    #endif
...
void CsrBtSppAppHandleSppPrim(CsrBtSppAppGlobalInstData *sppData)
{
    ...
    switch (*primType)
    {
        ...
        case CSR_BT_SPP_GET_INSTANCES_QID_CFM:
        {
            ...
            CsrAppRegisterReqSend(sppData->phandle, TECH_BT, PROFILE_NAME(sppData->profileName));
               #ifndef _WIN32_WCE
            sprintf(sppData->sppInstance.comPortName,"COM%d",0);
            sprintf(sppData->sppInstance.outputFileName,"CSR_BT_SPP%02d.txt",0);
#endif
            CsrBtSppAppHandleSppSetMainMenuHeader(sppData);
