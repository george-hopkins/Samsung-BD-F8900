...
CsrUint32 HidDriver_open()
{
    ...
    fd = open("/dev/csrhid", O_WRONLY);
    
    if (fd < 0) {
        printf("<%s (Line: %i)> Failed opening /dev/bluetoothinput",
                                     __FUNCTION__, __LINE__);
    }    
#ifdef HIDH_DEBUG    
    else
            printf("HIDH: HidDriver_open Success \n");
#endif       
    return fd;
}
...
int HidDriver_close(int fd)
{
    ...
    if (result < 0) {
        printf("<%s (Line: %i)> Failed Closing /dev/bluetoothinput", __FUNCTION__, __LINE__);
        printf("HIDH: HidDriver_close failed with %d \n", fd);   
    }
#ifdef HIDH_DEBUG    
    else
            printf("HIDH: HidDriver_close Success \n");
#endif    
...
void initInstanceData(hidhInstData_t *instData)
{
    ...
    {
        instData->bdAddrList[i].lap = 0xA40002;
        instData->bdAddrList[i].nap = 0x32;
        instData->bdAddrList[i].uap = 0XE811;
    }
    instData->selectedDevice = 0;
        ...
        case CSR_BT_HIDH_DATA_IND:
            {
                ...
                if( prim->reportType != CSR_BT_HIDH_OTHER)
                {
					...
					if (send_data == NULL)
					{
					   printf("CSR_BT_HIDH_STATUS_IND FAIL: report_desc CsrPmemAlloc Fail");
					   return;
					}
					...
					send_data->deviceIndex = Nvs_FindInList(&instData->bdAddrList[instData->selectedDevice]);;
					memcpy(send_data->report, (CsrUint8*)(prim->data+1), prim->dataLen );
...
//				CsrUint8		*desc;
                    ...
                    prim->resultSupplier == CSR_BT_SUPPLIER_HIDH)
                {
					...
					memcpy(send_desc->report, (CsrUint8*)(prim->descriptor + 8), report_desc_len );
        ...
        case CSR_BT_HIDH_DISCONNECT_IND:
            {
                CHANGE_STATE(HIDH_STATE_IDLE);
				printf("CSR_BT_HIDH_DISCONNECT_IND instData->fd %d",instData->fd);
				if (instData->fd >= 0)
				{
            ...
            case HIDH_STATE_CONNECTED:
                {
                    switch(theChar)
                    {
                        ...
                        case 'E':
                            {
                                CsrUint16            reportLen = 0, i;
                                CsrUint8            *reportData;
                                struct CsrBtHidReport *report = NULL;
