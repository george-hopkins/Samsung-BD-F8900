...
static CsrBool winplay_init(void **instance, struct av2filter_t *filter, void  *unused);
...
static void winplay_error_print(DWORD err, char *str)
{
    ...
    CL_API_LOGPrintf(WINPLAY_PRN "%s, error: code 0x%08X, windows error '%s'\n",
           str, err, ext);
}
...
static void winplay_new_buffer(winplay_instance_t *inst)
{
    ...
    {
        inst->buffer = (CsrUint8 * )inst->header[inst->buffer_next].lpData;
        inst->length = 0;
    }
...
static DWORD WINAPI winplay_proc(LPVOID arg)
{
            ...
            case MM_WOM_DONE:
                {
                    ...
                    {
#ifdef DEBUG_VERBOSE
                        CL_API_LOGPrintf(WINPLAY_PRN "MM_WOM_DONE received in winplay started state\n");
#endif
                        ...
                        {
#ifdef DEBUG
                            CL_API_LOGPrintf(WINPLAY_PRN "Winplayer is Paused\n");
#endif
                    ...
                    {
#ifdef DEBUG
                        CL_API_LOGPrintf(WINPLAY_PRN "MM_WOM_DONE received, but winplay NOT started\n");
#endif
                    }
                    LeaveCriticalSection(&winplay_mutex);
                    break;
...
#ifdef DEBUG
                    CL_API_LOGPrintf(WINPLAY_PRN "close thread\n");
#endif
                    return 0;
                }
            default:
    ...
    CL_API_LOGPrintf(WINPLAY_PRN "message loop ended\n");
    return 0;
}
...
static CsrBool winplay_init(void **instance, struct av2filter_t *filter, void *unused)
{
    ...
    if(WinplayInst == NULL)
    {
        *instance = CsrPmemZalloc(sizeof(winplay_instance_t));
        WinplayInst = (winplay_instance_t *)*instance;
    }
    inst = WinplayInst;
...
static CsrBool winplay_open(void **instance)
{
...
#ifdef DEBUG
            CL_API_LOGPrintf(WINPLAY_PRN "Opening winplay with channels:%i bits:%i samplerate:%i\n",
                   inst->channels,
                   inst->bits,
                   inst->sample_freq);
#endif
    ...
    if(!inst->thread)
    {
        CL_API_LOGPrintf(WINPLAY_PRN "wave playback thread creation failed, code 0x%08X\n", GetLastError());
        inst->state = WINPLAY_IDLE;
        return FALSE;
    }
    if(!SetThreadPriority(inst->thread, THREAD_PRIORITY_ABOVE_NORMAL))
    {
        CL_API_LOGPrintf(WINPLAY_PRN "could not set high priority for playback thread\n");
    }
    ...
    {
        CL_API_LOGPrintf(WINPLAY_PRN "Problem allocating the header buffers %i\n", (WAVE_OUT_BUFFER_SIZE * AUDIO_OUT_BUFFERS));
    }
    ...
    for(i=0; i<AUDIO_OUT_BUFFERS; i++)
    {
        ...
        {
            CL_API_LOGPrintf(WINPLAY_PRN "error preparing wave header %d, code 0x%08X\n", i, err);
            inst->state = WINPLAY_IDLE;
            return FALSE;
        }
    }
...
static CsrBool winplay_close(void **instance)
{
    ...
    if(err != 0)
    {
        CL_API_LOGPrintf(WINPLAY_PRN "error closing playback handle, code 0x%08X\n", err);
    }
    else
    {
        /* Free buffer */
    ...
    if(inst->thread != NULL)
    {
        ...
        {
#ifdef DEBUG
            CL_API_LOGPrintf(WINPLAY_PRN "playback thread still running, code 0x%08X, terminating it\n", err);
#endif
            TerminateThread(inst->thread, err);
#ifdef DEBUG
            CL_API_LOGPrintf(WINPLAY_PRN "thread termination code was 0x%08X\n", err);
#endif
        }
        inst->thread = NULL;
    }
...
static CsrBool winplay_start(void **instance)
{
    ...
    CL_API_LOGPrintf(WINPLAY_PRN "PCM settings (winplay_start) are channels:%i bits:%i samplerate:%i\n",
           channels,
           bits,
           sample_freq);
    ...
    {
        CL_API_LOGPrintf(WINPLAY_PRN "PCM settings (winplay_start) have changed, reconfiguring\n");
                              ...
                              CsrUint32      length)
{
    ...
    if(inst->length >= PLAYBACK_THRESHOLD)
    {
        ...
        /* Wave out the data */
#ifdef DEBUG
        CL_API_LOGPrintf(WINPLAY_PRN "play buffer, buffer_next:%i, length:%i, buffer_ready:%i\n",
        ...
        if(inst->playing == FALSE)
        {
            ...
            {
#ifdef DEBUG
                CL_API_LOGPrintf(WINPLAY_PRN "Winplayer is restarted\n");
#endif
                inst->playing = TRUE;
                waveOutRestart(inst->handle);
            }
            ...
            {
#ifdef DEBUG
                CL_API_LOGPrintf(WINPLAY_PRN "Winplayer is not restarted due to lack of ready buffers\n");
#endif
            }
        }
        LeaveCriticalSection(&winplay_mutex);
...
static void winplay_menu(void **instance,
                         CsrUint8 *num_options)
{
    CL_API_LOGPrintf("  Windows playback filter does not support menu\n");
    *num_options = 0;
}
