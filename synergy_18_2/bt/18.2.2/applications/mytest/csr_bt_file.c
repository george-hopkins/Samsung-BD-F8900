...
DIR *CsrBtOpenDir(const char *name)
{
    ...
    if(name && name[0])
    {
        ...
        if((dir = (DIR *)CsrPmemAlloc(sizeof *dir)) != 0 &&
           (dir->name =(char *) CsrPmemAlloc(base_length + CsrStrLen(all) + 1)) != 0)
        {
            CsrStrCat(CsrStrCpy(dir->name, name), all);
...
CsrResult CsrBtCopy(const CsrCharString *srcName,const CsrCharString *destName)
{
    ...
    if (dirCopy)
    {
        ...
        dirPath = (CsrCharString *)CsrPmemZalloc(CSR_BT_MAX_PATH_LENGTH);
        CsrStrCat(dirPath,srcName);
        ...
        else
        {
            ...
            while((file = CsrBtReadDir(dir)) != NULL)
            {
                ...
                {
                    newSrc = (CsrCharString *)CsrPmemZalloc(CSR_BT_MAX_PATH_LENGTH);
                    newDest = (CsrCharString *)CsrPmemZalloc(CSR_BT_MAX_PATH_LENGTH);
                    
                    CsrStrCat(newSrc,srcName);
                    CsrStrCat(newSrc,PATH_SEP);
                    CsrStrCat(newSrc,file->d_name);
                    
                    CsrStrCat(newDest,destName);
                    CsrStrCat(newDest,PATH_SEP);
                    CsrStrCat(newDest,file->d_name);
    ...
    else
    {
        ...
        buffer = (CsrUint8 *)CsrPmemAlloc(CSR_BT_COPY_BUFFER_SIZE);
