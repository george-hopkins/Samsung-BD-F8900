...
void sbc_debug_config(sbc_instance_t *inst)
{
    ...
    CL_API_LOGPrintf(SBC_PRN "subbands..............%i\n", inst->subbands);
    CL_API_LOGPrintf(SBC_PRN "blocks................%i\n", inst->blocks);
    CL_API_LOGPrintf(SBC_PRN "sample_freq...........%i\n", inst->sample_freq);
    CL_API_LOGPrintf(SBC_PRN "chan_mode.............%i (%c)\n", inst->channel_mode, ch_mode);
    CL_API_LOGPrintf(SBC_PRN "alloc_method..........%i (%c)\n", inst->alloc_method, alloc_method);
    CL_API_LOGPrintf(SBC_PRN "bitPool...............%i\n", inst->bitPool);
    CL_API_LOGPrintf(SBC_PRN "channels..............%i\n", inst->channels);
    CL_API_LOGPrintf(SBC_PRN "........................\n");
    CL_API_LOGPrintf(SBC_PRN "minBitpool............%i\n", inst->minBitpool);
    CL_API_LOGPrintf(SBC_PRN "maxBitpool............%i\n", inst->maxBitpool);
    CL_API_LOGPrintf(SBC_PRN "framesize.............%i\n", inst->framesize);
    CL_API_LOGPrintf(SBC_PRN "frames_per_payload....%i\n", inst->frames_per_payload);
    CL_API_LOGPrintf(SBC_PRN "bytes_per_encoding....%i\n", inst->bytes_per_encoding);
    CL_API_LOGPrintf(SBC_PRN "remoteMinBitpool......%i\n", inst->remoteMinBitpool);
    CL_API_LOGPrintf(SBC_PRN "remoteMaxBitpool......%i\n", inst->remoteMaxBitpool);
}
#endif
...
static CsrBool sbc_open(void **instance)
{
    ...
    if(getPcmLocked())
    {
        ...
        {
           // CL_API_LOGPrintf("Error:  bitrate %u too high for mono, CSRMAX 266kbps.\n", targetBitrate);
           // exit(1);
        }
    }
    ...
    else
    {
        ...
        //CL_API_LOGPrintf(SBC_PRN "invalid framesize\n");
        return FALSE;
    }
...
#ifdef DEBUG
    CL_API_LOGPrintf(SBC_PRN "close\n");
#endif
...
#ifdef DEBUG
    CL_API_LOGPrintf(SBC_PRN "start\n");
#endif
...
#ifdef DEBUG_VERBOSE
    CL_API_LOGPrintf(SBC_PRN "process_encode: sample length: %i, bytes_per_encoding: %i, estim_frames: %i\n",
           length,
           inst->bytes_per_encoding,
...
#ifdef DEBUG_VERBOSE
        CL_API_LOGPrintf(SBC_PRN "process_encode: send %i frames (%i bytes) in packet size %i bytes, last framesize: %i\n",
               frames,
                                 ...
                                 CsrUint32 length)
{
    ...
    do
    {
...
#ifdef DEBUG_VERBOSE
        CL_API_LOGPrintf(SBC_PRN "decode, bytes_left:%i, frame_size:%i\n", bytes_left, frame_size);
#endif
            ...
            for(block=0; ((block < inst->blocks) && (bufferProblem == FALSE)); block++)
            {
                for(subband=0; ((subband < inst->subbands) && (bufferProblem == FALSE)); subband++)
                {
                    for(chan=0; ((chan < inst->channels) && (bufferProblem == FALSE)); chan++)
                    {
                        ...
                        {
                            //CL_API_LOGPrintf(SBC_PRN "SBC_DECODE_BUFFER_SIZE(%i) too small.\n", SBC_DECODE_BUFFER_SIZE);
                            bufferProblem = TRUE;
                        }
                    }
                }
        ...
        else
        {
            ...
            //CL_API_LOGPrintf(SBC_PRN "unable to decode SBC frame, discarding\n");
            CsrPmemFree(data);
            return FALSE;
        }
...
#ifdef DEBUG_VERBOSE
    CL_API_LOGPrintf(SBC_PRN "done with this buffer, audio_len:%i, allocated %i\n",
                               ...
                               CsrUint8 *conf_len)
{
...
#ifdef DEBUG
    CL_API_LOGPrintf(SBC_PRN "get_config\n");
#endif
    ...
    *conf_len = SBC_MEDIA_CODEC_SC_SIZE;
    ...
    switch(inst->sample_freq)
    {
        ...
        default:
//            CL_API_LOGPrintf(SBC_PRN "invalid sample frequency!\n");
            break;
    }
    ...
    switch(inst->channel_mode)
    {
        ...
        default:
            CL_API_LOGPrintf(SBC_PRN "invalid channel mode!\n");
            break;
    }
#endif
    conf[4] = tmp;
    ...
    switch(inst->blocks)
    {
        ...
        default:
            //CL_API_LOGPrintf(SBC_PRN "invalid block count number!\n");
            break;
                                  ...
                                  CsrUint8 conf_len)
{
...
#ifdef DEBUG
    CL_API_LOGPrintf(SBC_PRN "set_config\n");
#endif
    ...
    {
#ifdef DEBUG
        CL_API_LOGPrintf(SBC_PRN "set_config: unsupported config detected\n");
#endif
                                ...
                                CsrUint8 *conf_len)
{
    ...
    *conf_len = SBC_MEDIA_CODEC_SC_SIZE;
    ...
    if(getPcmLocked())
    {
        CsrUint8 tmp;
        CsrPmemFree(conf);
//        CL_API_LOGPrintf(SBC_PRN "PCM settings are locked, returning current configuration\n");
                     ...
                     CsrUint8 *num_options)
{
    ...
    CL_API_LOGPrintf("  0) SBC capabilities (0x%02X 0x%02X 0x%02X 0x%02X)\n",
           inst->sbc_capabilities[0],
           inst->sbc_capabilities[1],
           inst->sbc_capabilities[2],
           inst->sbc_capabilities[3]);
    CL_API_LOGPrintf("  1) Channel mode (%i)\n", inst->channel_mode);
    CL_API_LOGPrintf("  2) Allocation method (%i)\n", inst->alloc_method);
    CL_API_LOGPrintf("  3) PCM sample frequency (%i)\n", inst->sample_freq);
    CL_API_LOGPrintf("  4) PCM channels (%i)\n", inst->channels);
    CL_API_LOGPrintf("  5) SBC blocks (%i)\n", inst->blocks);
    CL_API_LOGPrintf("  6) SBC subbands (%i)\n", inst->subbands);
    CL_API_LOGPrintf("  7) Minimum bitpool (%i)\n", inst->minBitpool);
    CL_API_LOGPrintf("  8) Maximum bitpool (%i)\n", inst->maxBitpool);
    CL_API_LOGPrintf("  9) Maximum frame size (%i)\n", inst->maxFrameSize);
#endif
    *num_options = 10;
}
