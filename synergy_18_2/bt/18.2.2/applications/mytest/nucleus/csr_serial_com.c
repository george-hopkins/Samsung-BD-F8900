    ...
    const CsrUint8 *token)
{
    ...
    for (idx = 0; idx < UART_CHANNEL_COUNT; idx++)
    {
        if (!instInUse[idx])
        {
            ...
            {
                instChannel[idx] = 0;//UART_PORT0;
            }
            else if (CsrStrNCmp(device, "uart1", UART_DEVICE_NAME_LENGTH) == 0)
            {
                instChannel[idx] = 1;//UART_PORT1;
            }
			else if (CsrStrNCmp(device, "uart2", UART_DEVICE_NAME_LENGTH) == 0)
            {
                instChannel[idx] = 2;//UART_PORT2;
            }
			else if (CsrStrNCmp(device, "uart3", UART_DEVICE_NAME_LENGTH) == 0)
            {
                instChannel[idx] = 3;//UART_PORT3;
            }
            else
            {
                instInUse[idx] = FALSE;
