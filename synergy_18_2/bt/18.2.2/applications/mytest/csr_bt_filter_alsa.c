...
static CsrBool alsa_setup(void **instance,
                         CsrUint8 index,
                         char *value);
static char* alsa_return_filename(void **instance);
...
static int alsa_xrun(alsa_instance_t *inst, int err)
{
    ...
    switch(err)
    {
        case -EAGAIN:
            if((inst->errors % 10) == 0)
            {
                CL_API_LOGPrintf(ALSA_PRN "device has more data, error count %i\n", inst->errors);
            }
        ...
        case -EBADFD:
            if((inst->errors % 10) == 0)
            {
                CL_API_LOGPrintf(ALSA_PRN "device not in right state, error count %i\n", inst->errors);
            }
        ...
        case -EPIPE:
            if((inst->errors % 10) == 0)
            {
                CL_API_LOGPrintf(ALSA_PRN "device over/under-run, error count %i\n", inst->errors);
            }
            snd_pcm_prepare(inst->handle);
        ...
        case -ESTRPIPE:
            if((inst->errors % 10) == 0)
            {
                CL_API_LOGPrintf(ALSA_PRN "device suspend error, error count %i\n", inst->errors);
            }
            snd_pcm_prepare(inst->handle);
            break;
        ...
        default:
            if((inst->errors % 10) == 0)
            {
                CL_API_LOGPrintf(ALSA_PRN "playback thread, unknown error, code %i, error count %i\n",
                       res, inst->errors);
            }
            res = err;
            snd_pcm_prepare(inst->handle);
...
static void *alsa_thread_play(void *instance)
{
...
#ifdef DEBUG
    CL_API_LOGPrintf(ALSA_PRN "thread begin...\n");
#endif
    ...
    while(inst->run)
    {
        ...
        {
            CL_API_LOGPrintf(ALSA_PRN "playback device did not consume all frames, sent %i, consumed %i\n",
                   frames, res);
            break;
        }
    }
...
#ifdef DEBUG
    CL_API_LOGPrintf(ALSA_PRN "thread end...\n");
#endif
...
static void *alsa_thread_capture(void *instance)
{
    ...
    while(inst->run)
    {
        ...
        cap = snd_pcm_readi(inst->handle,
                            ...
                            READ_FRAMES);
#ifdef VERBOSE_DEBUG
        CL_API_LOGPrintf(ALSA_PRN "read %i frames\n", (int)cap);
#endif
...
{
    alsa_instance_t *inst;
	Av2AppInst *av2inst_ins=(Av2AppInst *)av2inst;
    CsrUint8 i;
    *instance = CsrPmemZalloc(sizeof(alsa_instance_t));
    ...
    if(inst->params == NULL)
    {
        CL_API_LOGPrintf(ALSA_PRN "could not allocate ALSA hw_params structure\n");
        return FALSE;
    }
#ifdef DEBUG
    else
    {
        ...
        CL_API_LOGPrintf(ALSA_PRN "allocated hw_params structure\n");
    }
#endif

    /* Set defaults */
    inst->device = (CsrCharString *)CsrStrDup(av2inst_ins->alsa_device_name);
    inst->filter = filter;
...
#ifdef DEBUG
    CL_API_LOGPrintf(ALSA_PRN "alsa filter initialized\n");
#endif
...
static CsrBool alsa_open(void **instance)
{
...
#ifdef DEBUG
    CL_API_LOGPrintf(ALSA_PRN "opening alsa filter\n");
#endif
    ...
    if(inst->handle != 0)
    {
        CL_API_LOGPrintf(ALSA_PRN "filter already open - reopening\n");
    ...
    if(result && (snd_pcm_open(&(inst->handle),
                               ...
                               0) < 0))
    {
        CL_API_LOGPrintf(ALSA_PRN "could not open PCM device '%s'\n", inst->device);
        result = FALSE;
    }
#ifdef VERBOSE_DEBUG
    else
    {
        CL_API_LOGPrintf(ALSA_PRN "pcm device opened, 0x%08x\n", inst->handle);
        CL_API_LOGPrintf(ALSA_PRN "pcm device params, 0x%08x\n", inst->params);
    }
#endif
    ...
    if(result && (snd_pcm_hw_params_any(inst->handle,
                                        inst->params) < 0))
    {
        CL_API_LOGPrintf(ALSA_PRN "could not get hardware parameters\n");
        result = FALSE;
    }
#ifdef VERBOSE_DEBUG
    else
    {
        CL_API_LOGPrintf(ALSA_PRN "got hw parameters\n");
    }
#endif
    ...
    {
        CL_API_LOGPrintf(ALSA_PRN "could not set access mode\n");
        result = FALSE;
    }
#ifdef VERBOSE_DEBUG
    else
    {
        CL_API_LOGPrintf(ALSA_PRN "access mode set\n");
    }
#endif
    ...
    {
#ifndef __BIG_ENDIAN__
        CL_API_LOGPrintf(ALSA_PRN "could not set PCM format (signed 16bit little endian)\n");
#else
        CL_API_LOGPrintf(ALSA_PRN "could not set PCM format (signed 16bit big endian)\n");
#endif
        result = FALSE;
    }
    ...
    if(result && (snd_pcm_hw_params_set_rate_near(inst->handle,
                                                  ...
                                                  0) < 0))
    {
        CL_API_LOGPrintf(ALSA_PRN "could not set sample rate\n");
        result = FALSE;
    }
#ifdef VERBOSE_DEBUG
    else
    {
        CL_API_LOGPrintf(ALSA_PRN "sample rate set\n");
    }
#endif

    if(result && (tmp != inst->sample_rate))
    {
        CL_API_LOGPrintf(ALSA_PRN "sample rate is not supported by the hardware\n");
        result = FALSE;
    }
#ifdef VERBOSE_DEBUG
    else
    {
        CL_API_LOGPrintf(ALSA_PRN "actual sample rate matches requested rate\n");
    }
#endif
    ...
    if(result && (snd_pcm_hw_params_set_channels(inst->handle,
                                                 ...
                                                 inst->channels) < 0))
    {
        CL_API_LOGPrintf(ALSA_PRN "could not set number of channels\n");
        result = FALSE;
    }
#ifdef VERBOSE_DEBUG
    else
    {
        CL_API_LOGPrintf(ALSA_PRN "number of channels set\n");
    }
#endif
    ...
    if(result && (snd_pcm_hw_params_set_periods(inst->handle,
                                                ...
                                                0) < 0))
    {
        CL_API_LOGPrintf(ALSA_PRN "could not set number of periods\n");
        result = FALSE;
    }
#ifdef VERBOSE_DEBUG
    else
    {
        CL_API_LOGPrintf(ALSA_PRN "periods set\n");
    }
#endif
    ...
    if(result && (snd_pcm_hw_params_set_buffer_size(inst->handle,
                                                    ...
                                                    tmp) < 0))
    {
        CL_API_LOGPrintf(ALSA_PRN "could not set buffer size (%i)\n", tmp);
        result = FALSE;
    }
#ifdef VERBOSE_DEBUG
    else
    {
        CL_API_LOGPrintf(ALSA_PRN "buffer size set\n");
    }
#endif
    ...
    if(result && (snd_pcm_hw_params(inst->handle,
                                    inst->params) < 0))
    {
        CL_API_LOGPrintf(ALSA_PRN "could not set hw parameters\n");
        result = FALSE;
    }
#ifdef VERBOSE_DEBUG
    else
    {
        CL_API_LOGPrintf(ALSA_PRN "parameters applied\n");
    }
#endif
...
static CsrBool alsa_close(void **instance)
{
    ...
    if(inst->handle != 0)
    {
        ...
        if(code < 0)
        {
            CL_API_LOGPrintf(ALSA_PRN "error closing PCM handle, code %i\n", code);
            result = FALSE;
        }
    }
...
static CsrBool alsa_start(void **instance)
{
    ...
    /* Prepare the handle */
#ifdef DEBUG
    CL_API_LOGPrintf(ALSA_PRN "preparing pcm\n");
#endif
    code = snd_pcm_prepare(inst->handle);
    if(code < 0)
    {
        CL_API_LOGPrintf(ALSA_PRN "error preparing PCM handle, code %i\n", code);
        result = FALSE;
    }
    else
    {
#ifdef DEBUG
        CL_API_LOGPrintf(ALSA_PRN "starting thread\n");
#endif
        inst->errors = 0;
        inst->run = TRUE;
        ...
        if(inst->filter->f_type & FILTER_CONSUMER)
        {
            ...
            if(code != 0)
            {
                CL_API_LOGPrintf(ALSA_PRN "could not start playback thread, code %i\n", code);
                result = FALSE;
            }
            else
            {
                result = TRUE;
#ifdef DEBUG
                CL_API_LOGPrintf(ALSA_PRN "playback thread started\n");
#endif
        ...
        else
        {
            ...
            if(code != 0)
            {
                CL_API_LOGPrintf(ALSA_PRN "could not start capture thread, code %i\n", code);
                result = FALSE;
            }
            else
            {
                result = TRUE;
#ifdef DEBUG
                CL_API_LOGPrintf(ALSA_PRN "capture thread started\n");
#endif
            }
        }
    }
...
#ifdef DEBUG
    CL_API_LOGPrintf(ALSA_PRN "PCM device now streaming\n");
#endif
...
static CsrBool alsa_stop(void **instance)
{
    ...
    if(inst->thread != 0)
    {
        ...
        if(code != 0)
        {
            CL_API_LOGPrintf(ALSA_PRN "error stopping thread, code %i\n", code);
        }
        inst->thread = 0;
    }
    else
    {
        CL_API_LOGPrintf(ALSA_PRN "thread not running\n");
    }
    ...
    if(code < 0)
    {
        CL_API_LOGPrintf(ALSA_PRN "error draining (stopping) PCM handle, code %i\n", code);
        result = FALSE;
    }
    else
    {
                                   ...
                                   CsrUint32 length)
{
    ...
    while(result)
    {
        ...
        if(result)
        {
            ...
            {
                CL_API_LOGPrintf(ALSA_PRN "delivering %i bytes\n", inst->capsize);
...
#ifdef VERBOSE_DEBUG
            CL_API_LOGPrintf(ALSA_PRN "process done send\n");
#endif
        }
    }
    ...
    /* Done */
    {
        ...
        if(foo > 10)
        {
            CL_API_LOGPrintf(ALSA_PRN "capture count %i\n", inst->count);
            foo = 0;
        }
    }
    return result;
...
{
    /* Not supported */
    CL_API_LOGPrintf("  ALSA playback/capture does not support menu options\n");
    *num_options = 0;
}
