...
static void updateSearchingUi(CsrBtGapInstData * inst,
                                     CsrBtDeviceAddr deviceAddr, CsrBtClassOfDevice deviceClass,
                                     CsrCharString *friendlyName, CsrUint32 deviceStatus)
{
    CsrUint16         * codLabel;
    CsrCharString     * deviceName;
    CsrUint16         codIcon = CSR_UI_ICON_NONE;
    CsrCharString     deviceNameType[CSR_BT_MAX_FRIENDLY_NAME_LEN + sizeof("[BR|LE] ")];

    /* Convert the friendly name of the remote device from utf8 to ucs2         */
    deviceName = CsrBtGapReturnCharStringDeviceName(deviceAddr, friendlyName);

#ifdef CSR_BT_LE_ENABLE
    if(CSR_BT_SD_STATUS_RADIO_LE & deviceStatus && CSR_BT_SD_STATUS_RADIO_BREDR & deviceStatus)
    {
        CsrSnprintf(deviceNameType, sizeof(deviceNameType), "[LE|BR] %s", deviceName);
    }
    else if (CSR_BT_SD_STATUS_RADIO_LE & deviceStatus)
    {
        CsrSnprintf(deviceNameType, sizeof(deviceNameType), "[LE] %s", deviceName);
    }
    else if (CSR_BT_SD_STATUS_RADIO_BREDR & deviceStatus)
    {
        CsrSnprintf(deviceNameType, sizeof(deviceNameType), "[BR] %s", deviceName);
    }
    else
#endif
    {
        CsrSnprintf(deviceNameType, sizeof(deviceNameType), "%s", deviceName);
    }

    /* Decode the Class of Device bit mask, to a Icon and a sublabel            */
    /* codLabel   = AppCodDecodeDeviceClass(deviceClass, &codIcon);*/
    codLabel = CsrBtGapDeviceAddr2UnicodeString(deviceAddr, deviceStatus);
...
void appAddDevicesToSearchingUI(CsrBtGapInstData * inst, CsrBtDeviceAddr deviceAddr,
                                                ...
                                                CsrUint32 deviceStatus)
{
    ...
    if (CsrBtGapDeviceAddrPresentInKeyList(inst->searchList, deviceAddr,
                                        &key, &secLevel))
    { /* The device is allready present on the UI, just ignore it                           */
        ;
    }
    else
    {
        ...
        updateSearchingUi(inst, deviceAddr, deviceClass, friendlyName, deviceStatus);

        /* Insert the deviceAddr and the key in the searchList                              */
        CsrBtGapInsertDeviceInKeyList(&(inst->searchList), deviceAddr, friendlyName,
    ...
    )
{
    ...
    { /* The applicatoin is in the middle of a search procedure                           */
        ...
        if ((inst->scVar.proximityPairing) && (inst->proximityPairingHandle != 0xFFFF))
        {
            ...
            /* Insert the deviceAddr and the key in the searchList                              */
            CsrBtGapInsertDeviceInKeyList(&(inst->searchList), deviceAddr, (char *)friendlyName,
                                          deviceStatus, inst->devicesInSearchList, authorised);
            inst->devicesInSearchList++;
        ...
        else
        {
            ...
            appAddDevicesToSearchingUI(inst, deviceAddr, deviceClass, (CsrCharString *) friendlyName, deviceStatus);
        }
