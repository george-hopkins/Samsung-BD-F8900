...
#ifdef CSR_USE_DSPM
#include "csr_bt_dspm_patch_build_id_8241.h"
#include "csr_bt_dspm_patch_build_id_8295.h"
...
#define PROFILE_GAP CsrUtf8StrDup("GAP")
...
#ifdef CSR_USE_DSPM
static CsrUint8* previnPatchHandler(CsrUint16 buildId, CsrUint16 configBlockIndex, CsrUint16 *configBlockCount, CsrUint16 *configBlockLength)
{
    CsrUint8 *configBlock = NULL;
    CsrUint8 *tmpConfigBlock = NULL;

    switch (buildId)
    {
        case 8241:
        {
            *configBlockLength = CSR_BT_DSPM_PATCH_BUILD_ID_8241_BLOCK_LENGTH(configBlockIndex);
            configBlock = tmpConfigBlock = CsrPmemAlloc(*configBlockLength);
            CSR_ADD_UINT16_TO_XAP(tmpConfigBlock, CSR_BT_DSPM_PATCH_BUILD_ID_8241_MAX_INDEX);
            CsrMemCpy(tmpConfigBlock, CsrBtDspmPatchBuildId8241[configBlockIndex], *configBlockLength - sizeof(CsrUint16));
            break;
        }
        case 8295:
        {
            *configBlockLength = CSR_BT_DSPM_PATCH_BUILD_ID_8295_BLOCK_LENGTH(configBlockIndex);
            configBlock = tmpConfigBlock = CsrPmemAlloc(*configBlockLength);
            CSR_ADD_UINT16_TO_XAP(tmpConfigBlock, CSR_BT_DSPM_PATCH_BUILD_ID_8295_MAX_INDEX);
            CsrMemCpy(tmpConfigBlock, CsrBtDspmPatchBuildId8295[configBlockIndex], *configBlockLength - sizeof(CsrUint16));
            break;
        }
        default:
        {
...
#ifdef CSR_USE_DSPM
    /* Download of previn patches if they exist */
    CsrDspmConfigAcquireFunctionRegister(previnPatchHandler);
#endif
}
