...
extern void * CsrSbcMemMove ( void * source, CsrUint8 bands );
...
static void putbits(SbcHandle_t *hdl, const CsrUint32 n, const CsrUint32 x)
{
    SbcControl_t *SbcPtrAlias = hdl->sbc;
    CsrUint8* framePtrAlias = SbcPtrAlias->frame;
    CsrUint32 partial_posn_variable = SbcPtrAlias->partial_posn;
    CsrUint32 partial_variable = SbcPtrAlias->partial;

    partial_posn_variable = (CsrUint16) (partial_posn_variable - n);
    partial_variable |= x << partial_posn_variable;

    if (partial_posn_variable <= 16)
    {
        *framePtrAlias = ((CsrUint8) ((partial_variable>>24) & 0xff));
        *(framePtrAlias+1) = ((CsrUint8) ((partial_variable>>16) & 0xff));
        SbcPtrAlias->frame = (framePtrAlias + 2);
        partial_variable <<= 16;
        partial_posn_variable += 16;
    }
    SbcPtrAlias->partial_posn = partial_posn_variable;
    SbcPtrAlias->partial = partial_variable;    
...
{
    CsrUint8 ch, sb, b;
    CsrUint32 num_blocks = hdl->sbc->blocks;
...
static void jointEncode(SbcHandle_t *hdl)
{
    ...
    for (b=0; b < SBCPtrAlias->blocks; b++)
    {
        for (sb=0; sb < SBCPtrAlias->subbands-1; sb++)
        {
            /*
    ...
    calcScaleFactors1(hdl, 2, (CsrUint8)(SBCPtrAlias->subbands-1), sb_sample_js, scale_factor_js);
...
static CsrUint16 build_sbc_frame(SbcHandle_t *hdl)
{
    ...
    CsrUint32 x[CSR_SBC_MAX_BLOCKS][CSR_SBC_MAX_CHANNELS][CSR_SBC_MAX_SUBBANDS];
    CsrUint32 ch, sb, b;
    CsrUint8 *frame_crc;/* = hdl->sbc->frame+3;*/
    CsrUint8 *frame_start;/* = hdl->sbc->frame;        */
    SbcControl_t *SbcPtrAlias = hdl->sbc;

    frame_start = SbcPtrAlias->frame;
    frame_crc = frame_start + 3;
    ...
    *frame_crc = SBC_CRC_INIT;
    ...
    putbits_with_crc(hdl, 2, SbcPtrAlias->sample_freq, frame_crc);
    putbits_with_crc(hdl, 2, (CsrUint16)((SbcPtrAlias->blocks - 4)>>2), frame_crc);
    putbits_with_crc(hdl, 2, SbcPtrAlias->ch_mode, frame_crc);
    putbits_with_crc(hdl, 1, SbcPtrAlias->alloc_method, frame_crc);
    putbits_with_crc(hdl, 1, (CsrUint16)(SbcPtrAlias->subbands == 8 ? 1 : 0), frame_crc);
    putbits_with_crc(hdl, 8, SbcPtrAlias->bitpool, frame_crc);
    flush_putbits(hdl);

    SbcPtrAlias->frame++; /* skip over CRC*/

    /* add joint stereo information if used*/
    if (SbcPtrAlias->ch_mode == CSR_SBC_JOINT_STEREO)
    {
        putbits_with_crc(hdl, SbcPtrAlias->subbands, SbcPtrAlias->joint, frame_crc);
    }

    /* add scale factors*/
    for (ch=0; ch < SbcPtrAlias->channels; ch++)
    {
        for (sb=0; sb < SbcPtrAlias->subbands; sb++)
        {
    ...
    for (b=0; b < SbcPtrAlias->blocks; b++)
        for (ch=0; ch < SbcPtrAlias->channels; ch++)
            for (sb=0; sb < SbcPtrAlias->subbands; sb++)
            {
                const CsrUint32 n = hdl->bits[ch][sb];
                if (n != 0)
    ...
    /* do padding*/
    flush_putbits(hdl);
    return (CsrUint16) (SbcPtrAlias->frame - frame_start);
}
                  ...
                  CsrUint8 bitpool)
{
    ...
    {
        for(ii=0; ii < (CSR_SBC_MAX_WINDOW/2); ii++)
        {
            /*hdl->audio_window1[i][ii] = 0xAA00*(i+1)+ii;*/
            /*hdl->audio_window2[i][ii] = 0xBB00*(i+1)+ii+90;*/
            hdl->audio_window1[i][ii] = 0;
            hdl->audio_window2[i][ii] = 0;
        }
    }
                  ...
                  CsrUint8 *frame, CsrUint8 bit_pool)
{
    ...
    for (blk=0; blk < hdl->sbc->blocks; blk++)
    {
        for (ch=0; ch < hdl->sbc->channels; ch++)
        {
            if (blk & 1)
            {
                /* advance the sample window2 by 1 sample*/
                /*Important! Pointer Addresses to CsrSbcMemMove must be word-aligned!! Otherwise a runtime error will raise.*/
                CsrSbcMemMove(hdl->audio_window2[ch],hdl->sbc->subbands);
                /* pointer where to start to input next audio samples*/
                ptrDest = hdl->audio_window2[ch];
            }
            else
            {
                /* advance the sample window1 by 1 sample*/
                /*Important! Pointer Addresses to CsrSbcMemMove must be word-aligned!! Otherwise a runtime error will raise.*/
                CsrSbcMemMove(hdl->audio_window1[ch],hdl->sbc->subbands);
                /* pointer where to start to input next audio samples*/
                ptrDest = hdl->audio_window1[ch];
            }
        ...
        if (blk & 1)
        {
            if(hdl->sbc->subbands == 4)
                polyPhaseAnalysisSB4(hdl, hdl->sbc->channels + (blk<<8), hdl->audio_window2,hdl->audio_window1);
            else
                polyPhaseAnalysisSB8(hdl, hdl->sbc->channels + (blk<<8), hdl->audio_window2,hdl->audio_window1);
        }
        else
        {
            if(hdl->sbc->subbands == 4)
                polyPhaseAnalysisSB4(hdl, hdl->sbc->channels + (blk<<8), hdl->audio_window1,hdl->audio_window2);
            else
                polyPhaseAnalysisSB8(hdl, hdl->sbc->channels + (blk<<8), hdl->audio_window1,hdl->audio_window2);
        }
...
/*
    for (i=0;i<16;i++)
    {
        for (ii=0;ii<8;ii++)
        {
            linear[i*8+ii]=hdl->sb_sample[i][0][ii];
        }
    }
    */
