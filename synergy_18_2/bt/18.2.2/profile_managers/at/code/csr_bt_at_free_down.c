/*****************************************************************************

            (c) Cambridge Silicon Radio Limited 2010
            All rights reserved and confidential information of CSR

            Refer to LICENSE.txt included with this source for details
            on the license terms.

*****************************************************************************/

/* Note: this is an auto-generated file. */

#ifndef EXCLUDE_CSR_BT_AT_MODULE
#include "csr_synergy.h"
#include "csr_types.h"
#include "csr_pmem.h"
#include "csr_mblk.h"
#include "csr_bt_autogen.h"
#include "csr_bt_at_lib.h"
#include "csr_bt_at_prim.h"

void CsrBtAtFreeDownstreamMessageContents(CsrUint16 eventClass, void *message)
{
    if (eventClass == CSR_BT_AT_PRIM)
    {
        CsrBtAtPrim *prim = (CsrBtAtPrim *) message;
        switch (*prim)
        {
#ifndef EXCLUDE_CSR_BT_AT_DG_ATC_UNKNOWN_EXTENDED_CMD_RES
            case CSR_BT_AT_DG_ATC_UNKNOWN_EXTENDED_CMD_RES:
            {
                CsrBtAtDgAtcUnknownExtendedCmdRes *p = message;
                CsrPmemFree(p->payload);
                p->payload = NULL;
                break;
            }
#endif /* EXCLUDE_CSR_BT_AT_DG_ATC_UNKNOWN_EXTENDED_CMD_RES */
#ifndef EXCLUDE_CSR_BT_AT_DG_DATA_REQ
            case CSR_BT_AT_DG_DATA_REQ:
            {
                CsrBtAtDgDataReq *p = message;
                CsrPmemFree(p->payload);
                p->payload = NULL;
                break;
            }
#endif /* EXCLUDE_CSR_BT_AT_DG_DATA_REQ */
#ifndef EXCLUDE_CSR_BT_AT_DG_ATC_GMR_RES
            case CSR_BT_AT_DG_ATC_GMR_RES:
            {
                CsrBtAtDgAtcGmrRes *p = message;
                CsrPmemFree(p->payload);
                p->payload = NULL;
                break;
            }
#endif /* EXCLUDE_CSR_BT_AT_DG_ATC_GMR_RES */
#ifndef EXCLUDE_CSR_BT_AT_DG_ATC_GMM_RES
            case CSR_BT_AT_DG_ATC_GMM_RES:
            {
                CsrBtAtDgAtcGmmRes *p = message;
                CsrPmemFree(p->payload);
                p->payload = NULL;
                break;
            }
#endif /* EXCLUDE_CSR_BT_AT_DG_ATC_GMM_RES */
#ifndef EXCLUDE_CSR_BT_AT_DG_ATC_GCAP_RES
            case CSR_BT_AT_DG_ATC_GCAP_RES:
            {
                CsrBtAtDgAtcGcapRes *p = message;
                CsrPmemFree(p->payload);
                p->payload = NULL;
                break;
            }
#endif /* EXCLUDE_CSR_BT_AT_DG_ATC_GCAP_RES */
#ifndef EXCLUDE_CSR_BT_AT_DG_ATC_GMI_RES
            case CSR_BT_AT_DG_ATC_GMI_RES:
            {
                CsrBtAtDgAtcGmiRes *p = message;
                CsrPmemFree(p->payload);
                p->payload = NULL;
                break;
            }
#endif /* EXCLUDE_CSR_BT_AT_DG_ATC_GMI_RES */
#ifndef EXCLUDE_CSR_BT_AT_DG_ATC_CONNECT_TEXT_RES
            case CSR_BT_AT_DG_ATC_CONNECT_TEXT_RES:
            {
                CsrBtAtDgAtcConnectTextRes *p = message;
                CsrPmemFree(p->payload);
                p->payload = NULL;
                break;
            }
#endif /* EXCLUDE_CSR_BT_AT_DG_ATC_CONNECT_TEXT_RES */
            default:
            {
                break;
            }
        } /* End switch */
    } /* End if */
    else
    {
        /* Unknown primitive type, exception handling */
    }
}
#endif /* EXCLUDE_CSR_BT_AT_MODULE */
