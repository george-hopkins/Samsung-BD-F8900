...
#define CSR_BT_MAPC_PRIM_DOWNSTREAM_HIGHEST                              (0x0017 + CSR_BT_MAPC_PRIM_DOWNSTREAM_LOWEST)
...
#define CSR_BT_MAPC_PRIM_UPSTREAM_HIGHEST                                (0x0017 + CSR_BT_MAPC_PRIM_UPSTREAM_LOWEST)
...
typedef struct
{
    ...
    CsrBool             mnsController;                  /* USE WITH CARE: If there is more MSE instances in the peer MSE device
                                                           this flag should only be set as TRUE for the first Notification
                                                           registration (ie. where "enableNotifications == TRUE") made to an MSE
                                                           instance in the peer MSE device. This is because the specification
                                                           only allows for one MNS channel even though there's multiple MAS channels
                                                           to the same device. This MNS registration will be valid until the MNS session
                                                           is discontinued by either the peer MSE device (in which case the task which sent
                                                           this signal mnsController==TRUE will receive a CSR_BT_REGISTRATION_NOTIFICATION_OFF_IND)
                                                           or when the task which sent this signal with mnsController==TRUE disconnects
                                                           its MAS session. If either of the above happens the registration notification
                                                           for all other tasks which sent this signal with mnsController==FALSE and which is
                                                           connected to the same MSE device will become void and they would have to reregister
                                                           if they continue to wish to receive notification. Also the task which sends this
                                                           signal with mnsController==TRUE and enableNofitications==TRUE should be capable of
                                                           handling continuing notifications even though it uses this signal to deregister for
                                                           notifications. The latter can only happen in case other tasks connected other MSE
                                                           instances on the same MSE device has also registered for notifications with
                                                           mnsController==FALSE and enableNotifications==TRUE */
