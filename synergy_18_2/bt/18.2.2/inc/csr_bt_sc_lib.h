 ...
 *----------------------------------------------------------------------------*/
#define CsrBtScPasskeyResSendEx(_accept, _theDeviceAddr, _thePasskeyLength, _thePasskey, _addDevice, _authorised, _addressType) { \
        CsrBtScPasskeyRes *msg__ = (CsrBtScPasskeyRes *) CsrPmemAlloc(sizeof(CsrBtScPasskeyRes)); \
        msg__->type = CSR_BT_SC_PASSKEY_RES;                            \
        msg__->deviceAddr = _theDeviceAddr;                             \
        msg__->addDevice = _addDevice;                                  \
        msg__->authorised = _authorised;                                \
        msg__->addressType = _addressType;                              \
        CsrMemSet(msg__->passKey, '\0', CSR_BT_PASSKEY_MAX_LEN);        \
        if (_accept == FALSE || _thePasskeyLength > 16)                 \
        {                                                               \
            msg__->accept = FALSE;                                      \
            msg__->passKeyLength = 0;                                   \
        }                                                               \
        else                                                            \
        {                                                               \
            msg__->accept = TRUE;                                       \
            msg__->passKeyLength = _thePasskeyLength;                   \
            CsrMemCpy(msg__->passKey, _thePasskey, _thePasskeyLength);  \
        }                                                               \
        CsrBtScMsgTransport(msg__);}
...
#define CsrBtScSetAuthRequirementsReqSend(_authRequirements) CsrBtScSetAuthRequirementsReqSendEx(_authRequirements,CSR_BT_SC_LE_AUTH_IGNORE)
...
#define CsrBtScReadLocalOobDataReqSend(_thePhandle) {                   \
        CsrBtScReadLocalOobDataReq *msg__ = (CsrBtScReadLocalOobDataReq *) CsrPmemAlloc(sizeof(CsrBtScReadLocalOobDataReq)); \
 ...
 *----------------------------------------------------------------------------*/
#if CSR_BT_BT_VERSION >= CSR_BT_BLUETOOTH_VERSION_2P1
#define CsrBtScSspKeypressNotificationReqSend(_deviceAddr, _notificationType) { \
        CsrBtScSspKeypressNotificationReq *msg__ = (CsrBtScSspKeypressNotificationReq *) CsrPmemAlloc(sizeof(CsrBtScSspKeypressNotificationReq)); \
        msg__->type = CSR_BT_SC_SSP_KEYPRESS_NOTIFICATION_REQ;          \
        msg__->deviceAddr = _deviceAddr;                                \
        msg__->notificationType = _notificationType;                    \
        msg__->addressType = CSR_BT_ADDR_PUBLIC;                        \
        CsrBtScMsgTransport(msg__);}
#else
#define CsrBtScSspKeypressNotificationReqSend(_deviceAddr, _notificationType)
 ...
 *      authorised:     TRUE if authorisation is automatic granted, FALSE if not
 *----------------------------------------------------------------------------*/
#define CsrBtScSspPairingResSendEx(_deviceAddr, _accept, _addDevice, _authorised, _addressType) { \
...
#define CsrBtScSspPairingResSend(_deviceAddr, _accept, _addDevice, _authorised) CsrBtScSspPairingResSendEx(_deviceAddr, _accept, _addDevice, _authorised, CSR_BT_ADDR_PUBLIC)

#define CsrBtScSspPairingNegResSend(_d)                 \
    CsrBtScSspPairingResSend(_d, FALSE, FALSE, FALSE)
#define CsrBtScSspPairingNegResSendEx(_d, _at)                  \
    CsrBtScSspPairingResSendEx(_d, FALSE, FALSE, FALSE, _at)

#ifdef CSR_BT_INSTALL_SC_SET_EVENT_MASK
/*----------------------------------------------------------------------------*
 *  NAME
 *      CsrBtScSetEventMaskReqSend
 *
 *  DESCRIPTION
 *      Function to build and send a SC_SET_EVENT_MASK message to the SC message
 *      queue.
 *
 *  PARAMETERS
...
#define CsrBtScSetEventMaskReqSend(_eventMask) {                        \
        CsrBtScSetEventMaskReq *msg__ = (CsrBtScSetEventMaskReq *) CsrPmemAlloc(sizeof(CsrBtScSetEventMaskReq)); \
 ...
 *    PARAMETERS
 *      deviceAddr:         address of device which should be notified
 *      oobHashC:           SSP hash parameter
 *      oobRandR:           SSP random value
 *----------------------------------------------------------------------------*/
#define CsrBtScAddRemoteOobDataReqSend(_deviceAddr, _oobHashC, _oobRandR) { \
        CsrBtScAddRemoteOobDataReq *msg__ = (CsrBtScAddRemoteOobDataReq *) CsrPmemAlloc(sizeof(CsrBtScAddRemoteOobDataReq)); \
        msg__->type = CSR_BT_SC_ADD_REMOTE_OOB_DATA_REQ;                \
        msg__->deviceAddr = _deviceAddr;                                \
        CsrMemCpy(msg__->oobHashC, _oobHashC, SIZE_OOB_DATA);           \
        CsrMemCpy(msg__->oobRandR, _oobRandR, SIZE_OOB_DATA);           \
        CsrBtScMsgTransport(msg__);}
#endif
