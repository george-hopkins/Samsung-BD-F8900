...
/* Deprecated: use 'CsrBtAvrcpCtLibExtGfiMpGet' instead; it also retrieves the media player Id */
...
#define CsrBtAvrcpCtLibElementsAttributeGet(maxData, attIndex, itemsLen, items, attribId, charset, attLen, att) { \
        CsrBool result;                                                 \
        result=CsrBtAvrcpCtLibGfiMediaAttributeGet(maxData, attIndex, itemsLen, items, attribId, charset, attLen, att);}
...
/*----------------------------------------------------------------------------*
 *  NAME
 *      CsrBtAvrcpCtSetSSVolumeReqSend
 *
 *  DESCRIPTION
 *
 *
 ...
 *
 *----------------------------------------------------------------------------*/
#define CsrBtAvrcpCtSetSSVolumeReqSend(_phandle, _connId, _volume,_isMute) {      \
        CsrBtAvrcpCtSetSSVolumeReq *msg = pnew(CsrBtAvrcpCtSetSSVolumeReq); \
        msg->type          = CSR_BT_AVRCP_CT_SET_SS_VOLUME_REQ;            \
        msg->phandle       = _phandle;                                  \
        msg->connectionId  = _connId;                                   \
        msg->volume        = _volume;									\
		msg->isMute        = _isMute;									\
