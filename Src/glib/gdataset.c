		     ...
		     GDataset	   *dataset)
{
  ...
  if (new_data == NULL) /* remove */
      ...
      if (d)
	{
      ...
      if (d == NULL)
	{
	  d = g_malloc (sizeof(GData) + sizeof(GData) % 8);
	  d->len = 0;
	  d->alloc = 1;
	}
	...
	{
	  d->alloc = d->alloc * 2;
	  align_bytes = sizeof(GData) + (d->alloc - 1) * sizeof(GDataElt);
	  align_bytes  = align_bytes + align_bytes % 8;
	  d = g_realloc (d, align_bytes);
	}
      if (old_d != d)
