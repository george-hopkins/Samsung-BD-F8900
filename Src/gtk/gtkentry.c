...
struct _GtkEntryPrivate
{
  ...
  guint         cursor_disable          : 1; /* Cursor enable or Disable FROM USER LEVEL for entry box by SISC  */
		...
		cairo_t   *cr)
{
  ...
  if (gtk_entry_get_display_mode (entry) != DISPLAY_BLANK &&
      ...
      priv->selection_bound == priv->current_pos && priv->cursor_visible && !priv->cursor_disable)
		       ...
		       CursorType type)
{
  ...
  if (!block)
    {
      PangoDirection dir1 = priv->resolved_dir;
    ...
    PangoRectangle strong_x, weak_x;
    PangoRectangle *x1, *x2;

    g_return_if_fail (cr != NULL);
    g_return_if_fail (PANGO_IS_LAYOUT (layout));
    g_return_if_fail (cursor_index >= 0);
    
    pango_layout_get_cursor_pos (layout, cursor_index, &strong_x, &weak_x);
    if (split_cursor)
    {
        x1 = &strong_x;
        if (strong_x.x != weak_x.x || strong_x.y != weak_x.y)
        {
            dir2 = (dir1 == PANGO_DIRECTION_LTR) ? PANGO_DIRECTION_RTL : PANGO_DIRECTION_LTR;
            x2 = &weak_x;
        }
    }
    else
    {
        if (keymap_direction == dir1)
            x1 = &strong_x;
        else
            x1 = &weak_x;
    }
    ...
    cursor_location.x = x + PANGO_PIXELS (x1->x);
    cursor_location.y =    y + PANGO_PIXELS (x1->y);
    cursor_location.height =  PANGO_PIXELS (x1->height);
      ...
      if (dir2 != PANGO_DIRECTION_NEUTRAL)
        {
#ifdef USE_PANGO_LAYOUT
        cursor_location.x = x + PANGO_PIXELS (x2->x);
        cursor_location.y = y + PANGO_PIXELS (x2->y);
        cursor_location.height =  PANGO_PIXELS (x2->height);
#else
...
void gtk_entry_hide_blink_cursor(GtkEntry *entry, gboolean blink)
{
        GtkWidget *widget;
        widget = GTK_WIDGET (entry);
        if(blink)
        {
                g_object_set(gtk_widget_get_settings(widget), "gtk-cursor-blink", TRUE, NULL);
                entry->priv->cursor_disable=TRUE; // disable cursor
        }
        else
        {
                g_object_set(gtk_widget_get_settings(widget), "gtk-cursor-blink", FALSE, NULL);
                entry->priv->cursor_disable=FALSE; //enable cursor
