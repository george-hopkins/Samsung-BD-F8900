...
int _get_group_idx( struct mm_struct *mm, unsigned long flags,
		struct file *file, unsigned long start, unsigned long end , unsigned char is_stack) 
{
	...
	/* data : (file && (rw-p) && exec file) */
        else if ( file && (((flags & VM_WRITE) == VM_WRITE) || ((flags & VM_READ) == VM_READ))
        //else if ( file && ((flags & VMAG_RWS_MASK) == (VM_WRITE|VM_READ))
...
#if 0 
	/* thread stack : (rwxp) && (~brk) */
	else if ((flags & VMAG_RWXS_MASK) == (VM_READ|VM_WRITE|VM_EXEC)
			&& (start > mm->brk || end < mm->start_brk)) { 
        printk ( "Landed here for VMA start:end : %p %p\n" , start,end); 
		return VMAG_STACK;
	}
#endif // 0
...
int inline get_group_idx(struct vm_area_struct *vma) {
	return _get_group_idx( vma->vm_mm, vma->vm_flags, vma->vm_file, 
                        vma->vm_start, vma->vm_end , vma->is_stack);
}
...
//    vma->started_rss_profiling = 1 ; 
    	vma->started_rss_profiling += value ; 
}
...
#else
int _get_group_idx( struct mm_struct *mm, unsigned long flags, struct file *file, unsigned long start, unsigned long end, unsigned char is_stack ) { return 0; }
...
#ifdef CONFIG_RSS_INFO
	if ( 0 == new_below ) { 
		new->started_rss_profiling = 1 ; 
	} 
	else { 
		new->started_rss_profiling = 0 ; 
	} 
#endif 
