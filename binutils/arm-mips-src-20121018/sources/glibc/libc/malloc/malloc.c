...
#define public_vd_mALLINFo	dlvd_mallinfo
...
#define public_vd_mALLINFo	__libc_vd_mallinfo
...
#define public_vd_mALLINFo	vd_mallinfo
...
struct mallinfo public_vd_mALLINFo(void);
#else
struct mallinfo public_mALLINFo();
struct mallinfo public_vd_mALLINFo();
#endif
...
static struct trim_res mTRIMESt(mstate av, struct mallinfo *m);
...
static struct trim_res mTRIMESt();
...
struct malloc_chunk {
  ...
  INTERNAL_SIZE_T      fr_count;  /* Fragmentation brought by this mmapped chunk */
...
struct malloc_par {
  ...
  INTERNAL_SIZE_T  mmap_wasted_mem; /* Memory that is wasted by mmaping whole pages */
  INTERNAL_SIZE_T  mmap_fragm_chunks; /* Number of chunks that contain wasted memory */
  ...
  */
  ...
  try_mmap:
	...
	*/
	...
	if(size - (nb+SIZE_SZ) > 0){
	  mp_.mmap_wasted_mem += size - (nb + SIZE_SZ);
	  mp_.mmap_fragm_chunks++;
	  p->fr_count = size - (nb + SIZE_SZ);
	}
#ifdef NO_THREADS
	sum += av->system_mem;
...
#endif
{
  ...
  if(p->fr_count){
    if(mp_.mmap_wasted_mem - p->fr_count > 0)
	    mp_.mmap_wasted_mem -= p->fr_count;
    if(mp_.mmap_fragm_chunks > 0)
	    mp_.mmap_fragm_chunks--;
  }
...
struct mallinfo public_mALLINFo()
{
  ...
  struct mallinfo m_sum;
  int i;
  mstate av;
  memset(&m_sum,0,sizeof(struct mallinfo));

  if(__malloc_initialized < 0)
    ptmalloc_init ();

  for (i = 0, av = &main_arena;; i++) {

    (void)mutex_lock(&av->mutex);

    m = mALLINFo(av);
    m_sum.arena += m.arena;
    m_sum.ordblks += m.ordblks;
    m_sum.uordblks += m.uordblks;
    m_sum.fordblks += m.fordblks;
    m_sum.keepcost += m.keepcost;
    m_sum.hblks = m.hblks;
    m_sum.hblkhd = m.hblkhd;
...
{
  struct mallinfo m;
  struct trim_res tres;
  unsigned long g_trim = 0, g_used = 0, g_total = 0, g_full = 0;

/*
 * VDLinux glibc patch, show all arena info, hakbong.lee, 2010-07-15
 */
   unsigned int i;
   ...
   for (i = 0, av = &main_arena;; i++) {
       fprintf(stderr, "################################\n");
       fprintf(stderr, "System Memory in this arena (%u) :: %d\n",i ,av->system_mem);
       (void)mutex_lock(&av->mutex);
       m = mALLINFo(av);
       tres = mTRIMESt (av, &m);
       g_used += tres.used;
       g_total += tres.total;
       g_full += tres.full;
       fprintf(stderr, "arena          = %10u\n", m.arena);
       fprintf(stderr, "ordblks        = %10u\n", m.ordblks);
       fprintf(stderr, "smblks         = %10u\n", m.smblks);
       fprintf(stderr, "hblks          = %10u\n", m.hblks);
       fprintf(stderr, "hblkhd         = %10u\n", m.hblkhd);
       fprintf(stderr, "usmblks        = %10u\n", m.usmblks);
       fprintf(stderr, "fsmblks        = %10u\n", m.fsmblks);
       fprintf(stderr, "uordblks       = %10u\n", m.uordblks);
       fprintf(stderr, "fordblks       = %10u\n", m.fordblks);
       fprintf(stderr, "keepcost       = %10u\n", m.keepcost);
       fprintf(stderr, "fragmentation  = %10u/%10u (4k/4k pages = %10u)\n", tres.used, tres.total, tres.full);
       (void)mutex_unlock(&av->mutex);

       /* Other arenas can't be trimmed because of mmap allocation */
       if(av == &main_arena){
           fprintf(stderr, "malloc_trim    = %10u\n", tres.trim);
       }
       fprintf(stderr, "Max size of this arena :: %d\n", av->max_system_mem);
       fprintf(stderr, "################################\n");
       av = av->next;
       if (av == &main_arena){
           break;
  ...
  fprintf(stderr, "Total fragmentation =       %10lu / %10lu (4k/4k pages = %u)\n", g_used, g_total, g_full);
  return m;
}
...
/*
  ------------------------------ malloc_trimest ---------------------------
*/
struct trim_res mTRIMESt(mstate av, struct mallinfo *m)
{
  /* Ensure initialization/consolidation */
  malloc_consolidate (av);
  ...
  const size_t ps = mp_.pagesize;
  const size_t psm1 = ps - 1;
  const int page_max_cnt = (1024*1024*1024)/malloc_getpagesize; /* 1Gb of pages */


  unsigned int i, fragm_pgs = 0, page_cnt;
  int psindex = bin_index (ps);

#define BITS_PER_LONG	(CHAR_BIT*sizeof(unsigned long))

/* Retrieves the value in bit i, use as logical value (0 / !0) */
#define GET_BIT(array, i) (array[i / BITS_PER_LONG] & (1UL << (i % BITS_PER_LONG)))

/* Sets bit i to true*/
#define SET_BIT(array, i) (array[i / BITS_PER_LONG] |= (1UL << (i % BITS_PER_LONG)))

/* Sets bit i to false */
#define CLEAR_BIT(array, i) (array[i / BITS_PER_LONG] &= ~(1UL << (i % BITS_PER_LONG)))


#define ARRAY_SZ (page_max_cnt / (CHAR_BIT*sizeof(unsigned long)) + 1)
  unsigned long page_array[ARRAY_SZ];
...
#define BX_(x)         ((x) - (((x)>>1)&0x77777777) \
  ...
  char* max_address = (char*)(av->top) + chunksize(av->top);
  char* min_address = max_address - av->system_mem;
  page_cnt = (max_address-min_address)/ps; /* Really used pages at the moment */

  struct trim_res result = {.trim = 0, .used = 0, .full = 0, .total = 0};
  if(mp_.mmap_fragm_chunks*ps > mp_.mmap_wasted_mem)
	  result.used = mp_.mmap_fragm_chunks*ps - mp_.mmap_wasted_mem;
  result.total = mp_.mmap_fragm_chunks*ps;
  result.full = (mp_.mmapped_mem - result.total)/ps;
  if(!result.full || !result.total) /* Sanity check */
	  result.used = 0;
  memset(page_array, 0, ARRAY_SZ * sizeof(unsigned long));
  if(!m->arena) /* Nothing to do here, if no memory was allocated from heap */
    return result;
/* Traverse all freed chunks in bins */
  for (i = 1; i < NBINS; ++i)
	{
	...
	for (mchunkptr p = last (bin); p != bin; p = p->bk)
		...
		size -= paligned_mem - (char *) p;
                ...
                result.trim += (size & ~ psm1);
	      }
	    }
	    /* Count how many bytes are used in this page,
	     * increase number of fragmented pages. For last page, count fragmentation
	     * separately.
	     */
	    char *paligned = (char *) ((uintptr_t) p & ~psm1);
	    if(paligned >= min_address && paligned < max_address){
	      char *end = (char *) (uintptr_t)p+chunksize(p);
	      char *ealigned = (char *) ((uintptr_t)end & ~psm1);
	      int pg_idx = ((uintptr_t)paligned - (uintptr_t)min_address)/ps;
	      assert(pg_idx <= page_max_cnt);
	      if(chunksize(p) > psm1){/* Large chunk, could occupy several pages, count only fragmented space */
		if(p > paligned){ /* Start is not aligned */
		   int over_start = (uintptr_t)p - (uintptr_t)paligned;
		   result.used += over_start;
		   SET_BIT(page_array, pg_idx);
		 }
		 if(end > ealigned){ /* End is not aligned */
		    int over_end = ps - ((uintptr_t)end - (uintptr_t)ealigned);
		    result.used += over_end;
		    SET_BIT(page_array, (ealigned - min_address)/ps);
		 }


	      }
	      else {
		/* If start and end are laying in the same page */
		if(ealigned == paligned)
		  result.used += ps - size;
		else { /* Count FR for both pages */
		  assert(paligned + ps == ealigned);
		  int over_start = ps - ((uintptr_t)p - (uintptr_t)paligned);
		  int over_end = ps - ((uintptr_t)end - (uintptr_t)ealigned);
		  SET_BIT(page_array, (ealigned - min_address)/ps);
		  result.used += over_start + over_end;
		}
		/* If this page is new, remember it as fragmented */
		SET_BIT(page_array, pg_idx);
	      }
	    }
	    else {
	      /* Something is wrong - chunk not lies within heap boundaries */
	    }
	  }
    } /* End of for() */

  /* Align topmost chunk size for proper accounting */
  result.trim += m->keepcost & ~psm1;
  /* Count topmost chunk head fragmentation */
  char *ptr = av->top;
  /*
   * If ptr is not aligned to page boundary, the page where topmost chunk starts
   * is fragmented.
   */
  char *ptr_aligned = (char *) ((uintptr_t)ptr & ~psm1);

  assert((ptr_aligned - min_address)/ps <= page_cnt);
  if(m->keepcost && chunksize(av->top)){
	  SET_BIT(page_array, (ptr_aligned - min_address)/ps);
    int df = (uintptr_t)ptr - (uintptr_t)ptr_aligned;

    if(m->arena > ps || m->ordblks == 1){
      result.used += df;
    }
    else{
      result.used -= ps - df;
  ...
  /* Count all used pages */
  for(i = 0; i < ((max_address - min_address)/ps) / CHAR_BIT + 1; i++){
	if(page_array[i]){
		fragm_pgs += BITCOUNT(page_array[i]);
	}
  }
  if((((uintptr_t)m->arena - (uintptr_t)m->keepcost)/ps + 1) > fragm_pgs){
	  result.full += (m->uordblks - result.used)/ps;
  }
  /* Count total fragmented pages size */
  result.total += fragm_pgs * ps;
  return result;
}
...
strong_alias (__libc_vd_mallinfo, __vd_mallinfo)
weak_alias (__libc_vd_mallinfo, vd_mallinfo)
