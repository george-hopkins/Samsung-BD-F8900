...
static int show_pss(struct seq_file *m, void *v)
{
	struct proc_maps_private *priv = m->private;
	struct task_struct *task = priv->task;
	struct vm_area_struct *vma = v;
...
//	show_map_vma(m, vma);
	...
	if(!vma->vm_next) { /* latest vma sequence */
		seq_printf(m,
				" =====================================================\n"
				"                VMSize    Rss Shared Private    Pss\n" 
				"  Process Cur: %7lu %6lu %6lu  %6lu %6lu kB\n"
				"     Code Cur: %7lu %6lu %6lu  %6lu %6lu kB\n"
				"     Data Cur: %7lu %6lu %6lu  %6lu %6lu kB\n"
				"    Stack Cur: %7lu %6lu %6lu  %6lu %6lu kB\n"
				"    Other Cur: %7lu %6lu %6lu  %6lu %6lu kB\n",
				vmem.total, rss.total, shared.total, private.total, pss.total,
				vmem.vmag[0], rss.vmag[0], shared.vmag[0], private.vmag[0], pss.vmag[0],
				vmem.vmag[1], rss.vmag[1], shared.vmag[1], private.vmag[1], pss.vmag[1],
				vmem.vmag[2], rss.vmag[2], shared.vmag[2], private.vmag[2], pss.vmag[2],
				vmem.vmag[3], rss.vmag[3], shared.vmag[3], private.vmag[3], pss.vmag[3]
			  );
...
static const struct seq_operations proc_pid_pss_op = {
	.start	= m_start,
	.next	= m_next,
	.stop	= m_stop,
	.show	= show_pss
...
static int pss_open(struct inode *inode, struct file *file)
{
	return do_maps_open(inode, file, &proc_pid_pss_op);
}

const struct file_operations proc_pid_pss_operations = {
        .open           = pss_open,
        .read           = seq_read,
        .llseek         = seq_lseek,
        .release        = seq_release_private,
