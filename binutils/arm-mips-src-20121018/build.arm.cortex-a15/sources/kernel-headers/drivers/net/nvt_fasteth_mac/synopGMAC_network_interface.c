...
#if defined CONFIG_NVT_NT72558 || defined CONFIG_NVT_NT72568
...
s32 synopGMAC_linux_open(struct net_device *netdev)
{
	...
	while (count--)
	{        
		PhyId=synopGMAC_get_phy_id(gmacdev);
		TR0("%d: Phy ID %x\n",count, PhyId);
		//printk("%d: Phy ID %x\n",count, PhyId);

#ifdef CONFIG_NVT_MICREL_PHY_SUPPORT
		if ( ( PhyId == MICREL_PHY_ID1) || (PhyId == MICREL_PHY_ID2) )
#elif defined CONFIG_NVT_RTL_PHY_SUPPORT
		if ( PhyId == RTL8201E_PHY_ID ) 
#endif		
		{
#ifdef CONFIG_NVT_MICREL_PHY_SUPPORT			
...
#ifdef CONFIG_NVT_CMD_OPTION_SET_ETHER_ADDR

/*Derived from alchemy/common/prom.c */
static inline unsigned char str2hex(unsigned char c)
{
    if (c >= '0' && c <= '9')
       return c - '0';
...
#endif /*CONFIG_CMD_OPTION_SET_ETHER_ADDR*/
...
#define NULL_MAC_ADDRESS {0x00, 0x00, 0x00, 0x00, 0x00, 0x00}
	...
	char *ethaddr_str=NULL;
        char tmp_ea[6] = NULL_MAC_ADDRESS;
        int i;
	unsigned char num;

	ethaddr_str = strstr(boot_command_line, "ethaddr=");
        if (ethaddr_str)
	{
		ethaddr_str += strlen("ethaddr=");
               ...
               for (i = 0; i < 6; i++) 
               {
                      if ((*ethaddr_str == '.') || (*ethaddr_str == ':'))
                        ethaddr_str++;

                      num  = str2hex(*ethaddr_str++) << 4;
                      num |= str2hex(*ethaddr_str++);

                      tmp_ea[i] = num;
               }

	       memcpy(Mac_Addr, tmp_ea, sizeof(Mac_Addr));
...
#ifdef CONFIG_NVT_PRINT_ETHER_HW_ADDRES
	printk("[NTKETHMAC] : Ethernet MAC : ");
	for (i=0; i<6; i++)
	{	
		printk("%02x",(unsigned char) Mac_Addr[i]);
		if (i<5) printk(":");
                else printk("\n"); 
	}

#endif // CONFIG_NVT_PRINT_ETHER_HW_ADDRES
