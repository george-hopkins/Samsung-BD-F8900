...
(ppl_const_Polyhedron_t pset) try {
  const C_Polyhedron& ppset
    = *static_cast<const C_Polyhedron*>(to_const(pset));
...
(ppl_const_Polyhedron_t pset) try {
  const NNC_Polyhedron& ppset
    = *static_cast<const NNC_Polyhedron*>(to_const(pset));
  return termination_test_MS(ppset);
}
CATCH_ALL
...
(ppl_const_Polyhedron_t pset_before, ppl_const_Polyhedron_t pset_after) try {
  const C_Polyhedron& ppset_before
    = *static_cast<const C_Polyhedron*>(to_const(pset_before));
  const C_Polyhedron& ppset_after
    = *static_cast<const C_Polyhedron*>(to_const(pset_after));
...
(ppl_const_Polyhedron_t pset_before, ppl_const_Polyhedron_t pset_after) try {
  const NNC_Polyhedron& ppset_before
    = *static_cast<const NNC_Polyhedron*>(to_const(pset_before));
  const NNC_Polyhedron& ppset_after
    = *static_cast<const NNC_Polyhedron*>(to_const(pset_after));
  return termination_test_MS_2(ppset_before, ppset_after);
}
CATCH_ALL
...
(ppl_const_Polyhedron_t pset_before,
