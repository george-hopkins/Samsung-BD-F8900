...
(ppl_const_Grid_t pset) try {
  const Grid& ppset
    = *static_cast<const Grid*>(to_const(pset));
...
(ppl_const_Grid_t pset_before, ppl_const_Grid_t pset_after) try {
  const Grid& ppset_before
    = *static_cast<const Grid*>(to_const(pset_before));
  const Grid& ppset_after
    = *static_cast<const Grid*>(to_const(pset_after));
