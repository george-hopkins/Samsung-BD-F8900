...
ptp_usb_sendreq (PTPParams* params, PTPContainer* req)
{
	...
	if (ret!=PTP_RC_OK) {
		ret = PTP_ERROR_IO;
		//MPTP_Print("[ERROR]ptp_usb_sendreq %d\n",ret);
/*		ptp_error (params,
...
#else
		/* It is possible to not allocate the memory if it is already allocated */
		if (*data == NULL)
		{
			/* allocate memory for data */
			*data=(unsigned char*)calloc(len,1);				//g++ typecast vishal			
			if(NULL == *data)
			{
				ret = PTP_ERROR_IO;
				break;
...
* @param	Params (in) pointer to the  globle structure PTPParams
* @param	blkCnt (in)	pointer to the General container for Protocol  
* @param	data (out) double pointer to the data buffer get from the device
* @return	uint16_t (out) returns the protocol respose code
*/
uint16_t ptp_usb_getdata_NS (PTPParams* params, PTPContainer* ptp,
                unsigned char **data, uint32_t offset, uint32_t maxbytes)
{
       static uint16_t returnCode16;
       static PTPUSBBulkContainer usbdata;
       
	//int count=0, i=0;				
	//unsigned char arr[1024];
	//unsigned char arr1[8192];
	//unsigned int number=0;		
	//unsigned int rnumber=0;		
	//unsigned int temp=0;
	static uint32_t len;
	uint32_t lengthOfDataToRead;
	uint16_t err=0;
	...
	{
		len = 0;
		lengthOfDataToRead = 0;			
		
              returnCode16 = params->read_func((unsigned char *)&usbdata,
                                						sizeof(usbdata), params->data, &readlen);

	  	//printf("\nParam2=%d; Param3=%d\n", offset, maxbytes);

		if(returnCode16 == PTP_RC_OK)
		{
			//printf("\n----------  PTP_RC_OK\n");	
			//printf("USB Data Type = %x\n", usbdata.type);
			//printf("USB Data Code = %x\n", usbdata.code);
		}
              if (returnCode16 != PTP_RC_OK) 
		{
			returnCode16 = PTP_ERROR_IO;
			printf("\n---------- ERROR: returnCode16 != AL_ERR_IO\n");	
			err = PTP_PARTREAD_ERR;
			printf("err=%d\n",err);
			return err;
		} 
		else if (dtoh16(usbdata.type) != PTP_USB_CONTAINER_DATA) 
		{
			printf("\nusbdata.type=0x%x where as it should be = 0x%x\n", usbdata.type, PTP_USB_CONTAINER_DATA);
			printf("\nERROR is not PTP_USB_CONTAINER_DATA\n");	
//DFUNC
			printf ("\n************* Get Data ERROR *************\n");
			returnCode16 = PTP_ERROR_DATA_EXPECTED;
			err = PTP_PARTREAD_ERR;
			printf("err=%d\n", err);
			return err;
		}
		...
		{
			printf("\n IT enter into  (!=blkCnt->Code)  condition  \r\n");
			returnCode16 = dtoh16(usbdata.code);
			err = PTP_PARTREAD_ERR;
			return err;
		}
		len = dtoh32(usbdata.length) - PTP_USB_BULK_HDR_LEN;
		...
		{
			err = PTP_PARTREAD_ERR;
			printf(" memory ont allocated for buf in test application \n");
			return err;
		}
		...
		{
			err = PTP_PARTREAD_ERR;
			printf(" if (len+PTP_USB_BULK_HDR_LEN<=sizeof(usbBlkCnt))  \n");
			return err;
		}
		
		if(maxbytes >= len)
		{
			lengthOfDataToRead = len-PTP_USB_BULK_PAYLOAD_LEN;
			//printf("if(maxbytes >= len)\n");
			err = PTP_PARTREAD_FINISHED;
		}
		else
		{
			lengthOfDataToRead = maxbytes - PTP_USB_BULK_PAYLOAD_LEN;
			lengthOfDataToRead -= lengthOfDataToRead % 512;
			err = PTP_PARTREAD_CONTINUE;
			if(lengthOfDataToRead == 0)
				return err;
		}
		
		//printf("if::before read func 1st time; lengthOfDataToRead=%d\n", lengthOfDataToRead);
		returnCode16 = params->read_func(((unsigned char *)(*data)) + PTP_USB_BULK_PAYLOAD_LEN,
							                        lengthOfDataToRead,
							                        params->data, &readlen);
		
		params->getObjRetSize = lengthOfDataToRead + PTP_USB_BULK_PAYLOAD_LEN;
		//printf("if::after read func 1st time Params->getObjRetSize=%d\n", params->getObjRetSize);
		if (returnCode16 != PTP_RC_OK) 
		{
		        returnCode16 = PTP_ERROR_IO;
		        err = PTP_PARTREAD_ERR;
	...
	else
	{
		...
		if((offset+ maxbytes) >= len)
		{
			lengthOfDataToRead = len-offset;
			err = PTP_PARTREAD_FINISHED;
		}
		else
		{
			lengthOfDataToRead = maxbytes;
			err = PTP_PARTREAD_CONTINUE;
			lengthOfDataToRead -= lengthOfDataToRead % 512;	
		}	

		//printf("before read func cont time\n");
		//printf("else:: lengthOfDataToRead=%d \n", lengthOfDataToRead);
		returnCode16 = params->read_func(((unsigned char *)(*data)),
								                lengthOfDataToRead,
								                params->data, &readlen);
		params->getObjRetSize = lengthOfDataToRead + offset;
		//printf("else::params->getObjRetSize=%d\n", params->getObjRetSize);
		//printf("after read func cont time");
		if (returnCode16 != PTP_RC_OK) 
		{
			returnCode16 = PTP_ERROR_IO;
			err = PTP_PARTREAD_ERR;
...
/*		ptp_error (params,
		"PTP: request code 0x%04x getting resp error 0x%04x",
			resp->Code, ret);*/
//MPTP_Debug_Print("Sheetal - ptp_usb_getresp : failed\n");
		return ret;
	}
	...
	/*FIX FOR FUJI CAMERA*/
	/*Check for Return Length from Camera as it is found that*/
	/*sometimes Fuji Camera returns 12 Bytes*/
	if(resp->Param1  == GET_NUM_OBJ && rlen <= PTP_USB_BULK_HDR_LEN)
	{
		resp->Param1 = 0;
		//printf("  &&&&&&&&&&&&&&&&&&&&& Receive len is less %d \n", rlen);
		return ret;
	}
#endif	
...
//MPTP_Debug_Print("Sheetal - ptp_usb_getresp : success\n");
	return ret;
}
		...
		unsigned int *recvlen)
{
	...
	switch (flags&PTP_DP_DATA_MASK) {
		case PTP_DP_SENDDATA:	//DFUNC
			CHECK_PTP_RC(params->senddata_func(params, ptp,
				(unsigned char*)data, sendlen));	//g++ typecast vishal
			//MPTP_Debug_Print("Sheetal - ptp_transaction : send data\n");
			break;
		case PTP_DP_GETDATA:	//DFUNC
			CHECK_PTP_RC(params->getdata_func(params, ptp,
				(unsigned char**)data, recvlen));
			//MPTP_Debug_Print("Sheetal - ptp_transaction : get data\n");
			break;
		case PTP_DP_NODATA:
			//MPTP_Debug_Print("Sheetal - ptp_transaction : no data\n");
			break;
		default:
			//MPTP_Debug_Print("Sheetal - ptp_transaction : default\n");
		return PTP_ERROR_BADPARAM;
	}
	
#ifdef PTP_THREAD_SAFE	
	if((PTP_DP_NODATA == flags) && (GET_NUM_OBJ == sendlen))
	{
		ptp->Param1= GET_NUM_OBJ;
	}
#endif	
	/* get response */
...
/**
* @brief	Performs PTP transaction for devices not supporting GET_PARTIAL_OBJECT
* @param	params(in) pointer to the global structure PTPParams
* @param	ptp (in) pointer to the General container for Protocol  request/response/event 
* @param	flags (in) Data phase transaction flag
* @param	sendlen (in) lenth of the data to be send
* @param	data (out)	double pointer to the data to be  read from  USB
* @return	uint16_t (out) returns the protocol respose code
*/
static uint16_t ptp_transaction_NS (PTPParams* params, PTPContainer* ptp, 
			uint16_t flags, uint32_t sendlen, char** data, 
			uint32_t offset, uint32_t maxbytes)
{
	...
	flags=PTP_DP_GETDATA;
	sendlen=0;


	ptp->Transaction_ID = params->transaction_id++;
	ptp->SessionID = params->session_id;
	//printf("ptp_transaction_NS ~START\n");
	
	if(offset >0)
	{
		//printf("\nptp_transaction_NS offset > 0 case:\n");
		//printf("Get Data\n");
		if ((result = ptp_usb_getdata_NS(params, ptp,
					(uint8_t**)data, offset, maxbytes)) == PTP_PARTREAD_ERR) 
		{ 
			/*16April - brackets added*/
			printf("\n ptp_usb_getdata_NS second time read \n");
			printf("~ ING\n");
			return result;
		}
		//printf("Get Data Returned Success\n");
		if(result == PTP_PARTREAD_FINISHED)
		{
			if (params->getresp_func(params, ptp) != PTP_RC_OK) 
				result=PTP_PARTREAD_ERR;
	...
	else
	{
		...
		//printf("\nptp_transaction_NS offset > 0 case:\n");
		//printf("\nsendreq_func\n");
		result1 = params->sendreq_func (params, ptp);
		if (result1 != PTP_RC_OK) 
		{
			printf("~ ING\n");
			result = PTP_PARTREAD_ERR;
			printf("\n sendreq_func error \n");
		...
		//printf("Get Data\n");
		 ...
		 result= ptp_usb_getdata_NS(params, ptp, (uint8_t**)data, offset, maxbytes);
		if (result == PTP_PARTREAD_ERR)
		{
			printf("~ ING\n");
			printf("\n AlUsbGetData_NS first time error \n");
			printf("\nERROR_N_DEBUG CP4\n");
			return result;
		}
		if(result == PTP_PARTREAD_FINISHED) /*16April - bug fix - getPartial file less than 1MB*/
		{
			if (params->getresp_func(params, ptp) != PTP_RC_OK) 
				result = PTP_PARTREAD_ERR;
			printf("\n getresp_func \n");
...
ptp_getdeviceinfo (PTPParams* params, PTPDeviceInfo* deviceinfo)
{
	...
	ptp.Nparam=0;
	len=0;
	ret=ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, &di, (unsigned int*)&len);
...
ptp_opensession (PTPParams* params, uint32_t session)
{
	...
	//MPTP_Debug_Print("Sheetal - ptp_opensession : %d\n", ret);
...
ptp_getstorageids (PTPParams* params, PTPStorageIDs* storageids)
{
	...
	ptp.Nparam=0;
	len=0;
	ret=ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, &sids, (unsigned int*)&len);
			...
			PTPStorageInfo* storageinfo)
{
	...
	ptp.Nparam=1;
	len=0;
	ret=ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, &si, (unsigned int*)&len);
			...
			PTPObjectHandles* objecthandles)
{
	...
	ptp.Nparam=3;
	len=0;
	ret=ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, &oh, &len);		// /*(unsigned int*)*/
			...
			PTPObjectInfo* objectinfo)
{
	...
	ptp.Nparam=1;
	len=0;
	ret=ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, &oi,(unsigned int*) &len);
	if (ret == PTP_RC_OK) ptp_unpack_OI(params, oi, objectinfo, len);
	PTPFreeMem(oi);
	return ret;
}

/* 20060513 : Added by Lee Seung Hoon */
/**
 * ptp_GetNumObjects:
 * params:	PTPParams*
 *		storageId			- strorage id
 *		objectformatcode	- object type
 *		objecthandle		- Object handle
 *		nNum			- number of object
 *
 ...
 * Return values: Some PTP_RC_* code.
 **/
uint16_t ptp_GetNumObjects (PTPParams* params, uint32_t storageId,
                        uint32_t objectformatcode, uint32_t objecthandle, uint32_t* nNum)
{       
        uint16_t ret;
        PTPContainer ptp;
        ...
        ptp.Param1=storageId;
        ptp.Param2=objectformatcode;
        ptp.Param3=objecthandle;
        ptp.Nparam=3;
#ifdef PTP_THREAD_SAFE        
        ret=ptp_transaction(params, &ptp, PTP_DP_NODATA, GET_NUM_OBJ, NULL, NULL);  
#else
        ret=ptp_transaction(params, &ptp, PTP_DP_NODATA, 0, NULL, NULL);  
#endif
...
ptp_getobject (PTPParams* params, uint32_t handle, char** object)
{
	...
	ptp.Nparam=1;
	len=0;
	return ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, object, (unsigned int*)&len);
}
...
/**
  * @brief	      getpartialobject for devices not supporting GET_PARTIAL_OBJECT 
  * @param		alparams(in) pointer to the global structure AlParams
  * @param		handle(in) object handle
  * @param		offset(in) offset from where object retreiving should start
  * @param		maxbytes(in) size of partial object to get
  * @param		object(out) buffer in which the partial object will be returned
  * @return			PTPAPI_PARTREAD_CONTINUE: when end of file not reached, 
  				PTPAPI_PARTREAD_FINISHED: when end of file is reached,
  				PTPAPI_PARTREAD_ERR: when there is some communication error
*/ 
uint16_t ptp_getpartialobjectNS(PTPParams* params, uint32_t handle, uint32_t offset, 
			uint32_t maxbytes, char** object)
{
	PTPContainer ptp;
	uint16_t ret;
	//printf("\n Entering ptp_getpartialobject_NS\n");
	...
	ret=ptp_transaction_NS(params, &ptp, PTP_DP_GETDATA, 0, object, offset, maxbytes);
	if(ret == PTP_PARTREAD_CONTINUE)
	{
		//printf(" PTP:: Continue Partial Read  \n");		
	}
	else if(ret == PTP_PARTREAD_FINISHED)
	{
		//printf(" PTP:: Finish Partial Read  \n");		
	}
	else
	{
		//printf(" PTP:: Error Partial Read  \n");
	}
	
	//printf(" Leaving ptp_getpartialobjectNS \n");
	return ret;
}
			...
			PTPDevicePropDesc* devicepropertydesc)
{
	...
	ptp.Nparam=1;
	len=0;
	ret=ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, &dpd, (unsigned int*)&len);
			...
			PTPPropertyValue* value, uint16_t datatype)
{
	...
	ptp.Nparam=1;
	len=offset=0;
	ret=ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, &dpv,(unsigned int*) &len);
	if (ret == PTP_RC_OK) 
	{
		if(1 != ptp_unpack_DPV(params, (unsigned char*)dpv, &offset, len, value, datatype))
		{
			PTPFreeMem(dpv);
			return PTP_ERROR_IO;
		}
...
ptp_canon_checkevent (PTPParams* params, PTPUSBEventContainer* event, int* isevent)
{
	...
	*isevent=0;
	...
	ptp.Nparam=0;
	len=0;
	ret=ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, &evdata, (unsigned int*)&len);
	if (evdata!=NULL) {
		if (ret == PTP_RC_OK) {
				...
				uint32_t* readnum)
{
	...
	ptp.Nparam=4;
	len=0;
	ret=ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, &data, (unsigned int*)&len);
	if (ret==PTP_RC_OK) {
		*block=data;
		*readnum=ptp.Param1;
	}
...
ptp_canon_getviewfinderimage (PTPParams* params, char** image, uint32_t* size)
{
	...
	ptp.Nparam=0;
	ret=ptp_transaction(params, &ptp, PTP_DP_GETDATA, 0, image, (unsigned int*)&len);
			    ...
			    PTPCANONFolderEntry** entries, uint32_t* entnum)
{
	...
	if (ret == PTP_RC_OK) {
		uint32_t i;
		*entnum=ptp.Param1;
		*entries=(PTPCANONFolderEntry*)calloc(*entnum, sizeof(PTPCANONFolderEntry));
		if (*entries!=NULL) {
			for(i=0; i<(*entnum); i++)
...
ptp_free_devicepropdesc(PTPDevicePropDesc* dpd)
{
	...
	case PTP_DPFF_Enumeration:
		if (dpd->FORM.Enum.SupportedValue) {
			...
			PTPFreeMem (dpd->FORM.Enum.SupportedValue);
		}
	default:
		break;
	}
...
#if 0	// build warning- not used (08-10-28)
static int64_t
