...
/*
 * stop e2fsck if its mem usage becomes more than 40MB
 * to prevent OOM
 */
#define E2FSCK_MAX_MEM_USAGE    (40 * 1024 * 1024)

static long e2fsck_mem_usage(int pid)
{
	char fname[PATH_MAX], buf[80];
	long mem = 0;
	FILE *e2fsck_rss;

	memset(buf, '\0', sizeof(buf));
	sprintf(fname, "/proc/%d/rss", pid);
	e2fsck_rss = fopen(fname, "r");
	...
	while (!feof(e2fsck_rss)) {
		if (fgets(buf, sizeof(buf), e2fsck_rss) == NULL) {
			fclose(e2fsck_rss);
			return -1;
		}

		if (strstr(buf, "Process Max :"))
			break;
	}
	fclose(e2fsck_rss);

	sscanf(buf, "  Process Max : %7ldK", &mem);
	mem *= 1024; /* convert mem usage to bytes */

	return mem;
}

static void *e2fsck_mem_usage_thread_fn(void *ppid)
{
	int e2fsck_pid = *((int *)ppid);
	unsigned long mem_usage;

	while (1) {
		mem_usage = e2fsck_mem_usage(e2fsck_pid);
		if (mem_usage == -1)
			pthread_exit(NULL);

		if (mem_usage >= E2FSCK_MAX_MEM_USAGE)
			break;

		sleep(1);
	}

	fprintf(stderr, "***** e2fsck mem usage: %ldMB, stop e2fsck *****\n",
			mem_usage/(1024*1024));
	fflush(stderr);
	kill(e2fsck_pid, SIGINT);
...
int main (int argc, char *argv[])
{
	...
	pthread_t e2fsck_mem_usage_thread;
	int pid = getpid();
...
restart:
	...
	if (!ctx->superblock && !(ctx->options & E2F_OPT_PREEN) &&
		...
		fatal_error(ctx, "Currently we do not use backup sb\n");
		if (retval) {
			pctx.errcode = retval;
...
print_unsupp_features:
	...
	if (pthread_create(&e2fsck_mem_usage_thread, NULL,
				e2fsck_mem_usage_thread_fn, (void *)&pid))
