...
#ifdef P2P_SUPPORT
		else if (CONCURNT_MULTI_STA_CASE(pAd))
			reVal = PWR_CONSIDER_HIGH_PWR_FOR_MULTI_STA; 
#endif /* P2P_SUPPORT */
		else
...
#ifdef P2P_SUPPORT
			/* 
				This interface is P2P-Client, not normal STA.
				Need to update Rssi based on RssiSample under ApCfg rather than StaCfg.
			*/
			if (P2P_CLI_ON(pAd) && !INFRA_ON(pAd))
			{
...
#ifdef P2P_SUPPORT
		case PWR_CONSIDER_HIGH_PWR_FOR_MULTI_STA:
		{
			CHAR RssiBase, Rssi2;
			
				Rssi2 = RTMPMaxRssi(pAd, 
						   pAd->ApCfg.RssiSample.AvgRssi0, 
						   pAd->ApCfg.RssiSample.AvgRssi1, 
						   pAd->ApCfg.RssiSample.AvgRssi2);

			RssiBase = (Rssi < Rssi2) ? Rssi : Rssi2;
