...
#ifdef DOT11Z_TDLS_SUPPORT
	if (RTMP_GET_PACKET_TDLS_WAIT_ACK(pPacket))
	{
		pFirstTxWI->TXRPT = 1;
		pFirstTxWI->BW = 0;
	}
...
#endif /* P2P_SUPPORT */
			{
				...
				if(IS_ENTRY_TDLS(pMacEntry))
				{
					...
					if (pAd->StaCfg.TdlsInfo.TdlsAsicOperateChannel != pAd->CommonCfg.Channel)
						pTxBlk->pTransmit->field.BW = pAd->StaCfg.TdlsInfo.TdlsAsicOperateChannelBW;

					if (RTMP_GET_PACKET_TDLS(pTxBlk->pPacket))
...
/*Iverson patch for WMM A5-T07 ,WirelessStaToWirelessSta do not bulk out aggregate */
	...
	if (RTMP_GET_PACKET_TDLS_NO_ACK(pTxBlk->pPacket))
		pTxWI->ACK = 0;
#endif /* DOT11Z_TDLS_SUPPORT */
}
	...
	IN PUCHAR pAddr)
{
	...
	if (pEntry && !IS_ENTRY_NONE(pEntry))
	{
		...
		RTMPReleaseTimer(&pEntry->EnqueueStartForPSKTimer, &Cancelled);
			...
			{
				UCHAR index;
				PP2P_ENTRY_PARM pP2pInfo = &pEntry->P2pInfo;
				...
				{
					index = P2pGroupTabSearch(pAd, pP2pInfo->DevAddr);
				...
				{
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
					P2pSendWirelessEvent(pAd, RT_P2P_AP_STA_DISCONNECTED, NULL, pEntry->Addr);
#endif /* RT_P2P_SPECIFIC_WIRELESS_EVENT */
				}
			}
#endif /* P2P_SUPPORT */
...
//   			NdisZeroMemory(pEntry, sizeof(MAC_TABLE_ENTRY));
	...
	//Patch about de-auth issue. CQ_ID:WCNAE00015762	
	pAd->MacTab.Content[wcid].Sst = SST_NOT_AUTH; 
...
//#endif // CONFIG_AP_SUPPORT //
	...
	while (1)
	{
		...
		*/
		...
		{
			DBGPRINT(RT_DEBUG_TRACE, ("ACTION COME-IN !! TYPE = %d, IsReg = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
				  pHeader->FC.SubType, pAd->Cfg80211RegisterActionFrame, PRINT_MAC(pHeader->Addr2)));

			CFG80211DRV_PrintFrameType(pAd, "RX", pHeader,  pRxWI->MPDUtotalByteCount);
		}
#endif		
		if (RTMP_CFG80211_VIF_P2P_GO_ON(pAd) || RTMP_CFG80211_VIF_P2P_CLI_ON(pAd))
		{
			...
			if (NdisEqualMemory(pAd->P2PCurrentAddress, pHeader->Addr1, MAC_ADDR_LEN) ||
				...
				(pHeader->FC.SubType == SUBTYPE_BEACON 	&& NdisEqualMemory(pAd->ApCfg.ApCliTab[0].CfgApCliBssid, pHeader->Addr2, MAC_ADDR_LEN)) ||
