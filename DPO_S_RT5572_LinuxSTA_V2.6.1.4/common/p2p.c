...
#ifdef CONFIG_MULTI_CHANNEL
	else if (pP2PCtrl->P2pCounter.bListen && INFRA_ON(pAd))
	{
		/* Stop Listen */
		RTMPCancelTimer(&pAd->MlmeAux.ScanTimer, &Cancelled);
		pAd->MlmeAux.Channel = 0;
...
*/
	...
	if (P2P_CLI_ON(pAd))
	{
		...
		if (pAd->P2pCfg.bP2pLinkDown == TRUE)
			pAd->ApCfg.ApCliTab[0].Enable = 0;
		else
			Set_P2pCli_Enable_Proc(pAd, "0");

		pAd->P2pCfg.bP2pLinkDown = FALSE;
#else
		Set_P2pCli_Enable_Proc(pAd, "0");
#endif /*CONFIG_MULTI_CHANNEL*/
	}
	...
	IN	PRTMP_ADAPTER pAd)
{
	...
	for(apcliIdx = 0; apcliIdx < MAX_APCLI_NUM; apcliIdx++)
	{
		...
		pAd->ApCfg.ApCliTab[apcliIdx].P2pCliProbeReqTimeout = 0;
	}
#endif /* APCLI_SUPPORT */
#endif /* P2P_SUPPORT */
