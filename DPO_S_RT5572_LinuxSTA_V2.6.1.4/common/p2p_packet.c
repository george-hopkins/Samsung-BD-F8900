	...
	IN MLME_QUEUE_ELEM *Elem) 
{
		...
		UCHAR		GroupCap, DeviceCap, DeviceName[32], DeviceNameLen = 0;
		ULONG		FrameLen;
		ULONG		TempLen;
		UCHAR		SsidLen = 0;
		...
		*/
		...
		if (index == P2P_NOT_FOUND)
		{
			...
			if (index < MAX_P2P_GROUP_SIZE)
			{
				...
				if (DeviceNameLen != 0)
				{
					RTMPZeroMemory(&pAd->P2pTable.Client[index].DeviceName[0], P2P_DEVICE_NAME_LEN);
...
//QQ TBD, p2p widi need to reply success status code!
				)
			{
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	UCHAR		GroupCap, DeviceCap, DeviceName[P2P_DEVICE_NAME_LEN],	DeviceNameLen =0;
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	UCHAR		DeviceName[P2P_DEVICE_NAME_LEN], DeviceNameLen = 0;
	UCHAR		DevType[8];
	...
	if (IS_P2P_DEVICE_DISCOVERING(pAd))
		P2pStopScan(pAd);
	RTMPZeroMemory(&DeviceName, P2P_DEVICE_NAME_LEN);
	P2pParseSubElmt(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8), 
	...
	*/
		...
		|| (WpsConfigMethod == 0x188))
	{
		...
		if (p2pindex < MAX_P2P_GROUP_SIZE)
		{
			...
			if (DeviceNameLen != 0)
			{
				RTMPZeroMemory(&pAd->P2pTable.Client[p2pindex].DeviceName[0], P2P_DEVICE_NAME_LEN);
				RTMPMoveMemory(&pAd->P2pTable.Client[p2pindex].DeviceName[0], DeviceName, DeviceNameLen);
...
#ifdef RT_P2P_SPECIFIC_WIRELESS_EVENT
					P2pSendWirelessEvent(pAd, RT_P2P_DEVICE_FIND, &pAd->P2pTable.Client[p2pindex], pFrame->p80211Header.Addr2);
					P2pSendWirelessEvent(pAd, RT_P2P_RECV_PROV_REQ, &pAd->P2pTable.Client[p2pindex], pFrame->p80211Header.Addr2);
#endif /* RT_P2P_SPECIFIC_WIRELESS_EVENT */
				}
				else
...
/*	IN UCHAR	WorkingChannel) */
{
		...
		UCHAR		DeviceNameLen = 0, DeviceName[P2P_DEVICE_NAME_LEN];
...
//p2p widi need insert p2p entry in probe req. due to p2p0 only in listen state
		...
		if ((pAd->ApCfg.MBSSID[BSS0].WscControl.WscSelReg) && (pAd->P2pCfg.Dpid == DEV_PASS_ID_PBC))
		{
			if (P2P_GO_ON(pAd))
			{
				...
				*((PUSHORT) (pData + 4)) = cpu2be16(0x18c); /* Support All : PBC, Keypad, Label, Display */
				...
				DBGPRINT(RT_DEBUG_INFO, ("P2pMakeProbeWSCIE: SelReg=1 \n"));
			}
		}	
