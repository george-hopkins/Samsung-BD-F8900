	...
	IN PUCHAR    Addr) 
{	UCHAR	i;
	...
	if ((Addr[0] & 0x1) == 0x1)
	{
		DBGPRINT(RT_DEBUG_LOUD, ("%s::multicast\n", __FUNCTION__));
		return index;
	}
	...
	if (NdisEqualMemory(Allff, Addr, MAC_ADDR_LEN))
	{
		...
		DBGPRINT(RT_DEBUG_LOUD, ("%s::all 0xff\n", __FUNCTION__));
		return index;
	...
	{
		DBGPRINT(RT_DEBUG_LOUD, ("%s::all zero\n", __FUNCTION__));
		return index;
	}

	if (NdisEqualMemory(ZeroSsid, Addr, MAC_ADDR_LEN))
	{
		DBGPRINT(RT_DEBUG_LOUD, ("%s::zero ssid\n", __FUNCTION__));
		return index;
	}
	...
	{		
			...
			|| (NdisEqualMemory(Tab->Client[i].addr, Addr, 6)))
		{
			if (Tab->Client[i].P2pClientState > P2PSTATE_NONE)
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s::Tab->Client[%d].P2pClientState: %d\n",
					__FUNCTION__, i, Tab->Client[i].P2pClientState));
		}
	}
