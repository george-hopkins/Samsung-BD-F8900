	...
	IN UCHAR					FlgIsSuccess)
{
	...
	if (FlgIsSuccess)
	{
		printk("APCLI: (%02X:%02X:%02X:%02X:%02X:%02X) => ReqIeLen %d, RspIeLen, %d\n", PRINT_MAC(pBSSID), ReqIeLen, RspIeLen);
...
/* Actually KERNEL_VERSION(3,2,0) have following member, But if patch the kernel */
