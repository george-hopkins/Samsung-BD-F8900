...
extern USB_DEVICE_ID rtusb_extern_dev_id[];
extern INT const rtusb_extern_usb_id_len;

extern USB_DEVICE_ID rtusb_ll_dev_id[];
extern INT const rtusb_ll_dev_id_len;

unsigned int dongle_type = 0;
module_param(dongle_type, uint, 0644);
...
#ifdef RESOURCE_BOOT_ALLOC
{
	...
	if ( dongle_type == 1 ) {
		rtusb_driver.id_table = rtusb_extern_dev_id;
	}
	else if ( dongle_type == 2 ) {
		rtusb_driver.id_table = rtusb_ll_dev_id;
	}
