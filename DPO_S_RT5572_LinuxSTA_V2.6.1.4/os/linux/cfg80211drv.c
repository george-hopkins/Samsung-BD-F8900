	...
	IN PVOID SystemSpecific3)
{
	...
	cfg80211_remain_on_channel_expired(pAd->dummy_p2p_net_dev, pAd->CfgChanInfo.cookie, pAd->CfgChanInfo.chan, pAd->CfgChanInfo.ChanType, GFP_KERNEL);
#endif
	
	if (RTMP_CFG80211_VIF_P2P_CLI_ON(pAd))
		DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_ROC: RemainOnChannelTimeout APCLI_ON %d\n", pAd->ApCliMlmeAux.Channel));
	...
	IN	ULONG					Data)
{
	...
	switch(cmd)
	{
		...
		case CMD_RTPRIV_IOCTL_80211_CHANNEL_LOCK:
			...
			{
				DBGPRINT(RT_DEBUG_ERROR, ("CFG80211_PKT: TX CHANNEL_LOCK %d\n", Data));	
				AsicSwitchChannel(pAd, Data, FALSE);
				AsicLockChannel(pAd, Data);
				
				DBGPRINT(RT_DEBUG_TRACE, ("OFF-Channel Send Packet:  %d-%d\n", Data, pAd->LatchRfRegs.Channel));
			}
			else
				DBGPRINT(RT_DEBUG_TRACE, ("OFF-Channel Channel Equal:  %d-%d\n", Data, pAd->LatchRfRegs.Channel));
			break;
...
VOID CFG80211DRV_PrintFrameType(
        IN  VOID                                        *pAdOrg,
		IN	PUCHAR										 preStr,
		IN	PUCHAR										 pData,
		IN	UINT                              			 Length)
{
	...
	mgmt = (const struct ieee80211_mgmt *)pData;
	if (ieee80211_is_mgmt(mgmt->frame_control)) 
	{
		if (ieee80211_is_probe_resp(mgmt->frame_control)) 
		{
			DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_PKT: %s ProbeRsp Frame\n", preStr));
		}
		else if (ieee80211_is_disassoc(mgmt->frame_control))
		{
			DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_PKT: %s DISASSOC Frame\n", preStr));
		}
		else if (ieee80211_is_deauth(mgmt->frame_control))
		{
			DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_PKT: %s Deauth Frame\n", preStr));
		}
		else if (ieee80211_is_action(mgmt->frame_control)) 
		{
			DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_PKT: %s ACTION Frame\n", preStr));
			    ...
			    (pFrame->Action == ACTION_WIFI_DIRECT))
			{
				switch (pFrame->Subtype)
				{
					case GO_NEGOCIATION_REQ:
						DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_PKT: %s GO_NEGOCIACTION_REQ %d\n", 
									preStr, pAd->LatchRfRegs.Channel));
						break;	

					case GO_NEGOCIATION_RSP:
						DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_PKT: %s GO_NEGOCIACTION_RSP %d\n", 
									preStr, pAd->LatchRfRegs.Channel));
						break;

					case GO_NEGOCIATION_CONFIRM:
						DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_PKT: %s GO_NEGOCIACTION_CONFIRM %d\n", 
									preStr,  pAd->LatchRfRegs.Channel));
						break;

					case P2P_PROVISION_REQ:
						DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_PKT: %s P2P_PROVISION_REQ %d\n", 
									preStr, pAd->LatchRfRegs.Channel));
						break;

					case P2P_PROVISION_RSP:
						DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_PKT: %s P2P_PROVISION_RSP %d\n", 
									preStr, pAd->LatchRfRegs.Channel));
						break;

					case P2P_INVITE_REQ:
						DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_PKT: %s P2P_INVITE_REQ %d\n", 
									preStr, pAd->LatchRfRegs.Channel));
						break;

					case P2P_INVITE_RSP:
						DBGPRINT(RT_DEBUG_TRACE, ("CFG80211_PKT: %s P2P_INVITE_RSP %d\n", 
									preStr, pAd->LatchRfRegs.Channel));
						break;
				}	
			}		
	...
	UINT8						IfType)
{
...
#if 1 //yiwei added for static p2p0
	...
	switch (IfType)
	{
		case RT_CMD_80211_IFTYPE_MONITOR:
			...
			RTMP_CFG80211_RemoveVifEntry(pAd,pAd->dummy_p2p_net_dev);
			RTMP_CFG80211_AddVifEntry(pAd, pAd->dummy_p2p_net_dev, IfType);
		...
		case RT_CMD_80211_IFTYPE_P2P_CLIENT:
			...
			pApCliEntry->dev = pAd->dummy_p2p_net_dev;
			COPY_MAC_ADDR(pAd->P2PCurrentAddress, pAd->dummy_p2p_net_dev->dev_addr);
			COPY_MAC_ADDR(pApCliEntry->CurrentAddress, pAd->P2PCurrentAddress);

			DBGPRINT(RT_DEBUG_TRACE, ("ApCli_Open\n"));
			pAd->flg_apcli_init = TRUE;
			ApCli_Open(pAd, pAd->dummy_p2p_net_dev);
		...
		case RT_CMD_80211_IFTYPE_P2P_GO:
			...
			RTMP_CFG80211_RemoveVifEntry(pAd,pAd->dummy_p2p_net_dev);
			RTMP_CFG80211_AddVifEntry(pAd, pAd->dummy_p2p_net_dev, RT_CMD_80211_IFTYPE_P2P_GO); /* change to GO */
			break;

		default:
		DBGPRINT(RT_DEBUG_ERROR, ("CFG80211DRV_OpsChgVirtualInf : I/F Type (%d)\n", IfType));
	}
#endif
	...
	BOOLEAN											isAllUpdate)
{
	...
	{
		printk("YF_BEACON ====> CFG80211_UpdateBeacon TRUE\n");
	...
	VOID						*pData)
{
	...
	if (pKeyInfo->KeyType == RT_CMD_80211_KEY_WEP)
	{
/* CFG80211DRV_Connect() will set the wep key, don't set in this point */	
#if 0	
	...
	VOID						*pData)
{
	...
	printk("YF Debug: APCLI Connection onGoing.....\n");
	...
	{
		printk("YF Debug: AP_CLI WPS Connection onGoing.....\n");
	...
	VOID						*pData)
{
												...
												RT_CMD_80211_CONN_ENCRYPT_WEP))
	{
		...
		Set_Key_Proc(pAd,(PSTRING)KeyBuf,(INT)pConnInfo->KeyLen, (INT)pConnInfo->KeyIdx);
#endif
	} /* End of if */
	...
	UINT32 						duration)
{
	...
	{
		DBGPRINT(RT_DEBUG_ERROR, ("CFG80211_PKT: ROC CHANNEL_LOCK %d\n", pChanInfo->ChanId));
