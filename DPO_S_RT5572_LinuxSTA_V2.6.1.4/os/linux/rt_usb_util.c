...
void dump_mem_pool(void)
{
	...
	if ((rtusb_buf_pool == NULL) || (mem_pool_stat == MEM_POOL_INVALID)) {
		...
		DBGPRINT(RT_DEBUG_TRACE,("Dump Pre-allocated mem pool(Hdr:0x%p, flag:%d):\n",
				rtusb_buf_pool, mem_pool_stat));

		pool = rtusb_buf_pool;
		while(pool) {
			DBGPRINT(RT_DEBUG_TRACE,("\tbus(%s): controller=0x%p, pool=0x%p, pool_cnt=%d\n", pool->bus->bus_name, pool->dev, pool, pool->pool_cnt));
			for (idx = 0; idx < pool->pool_cnt; idx++) {
				mem = (struct rtusb_bulk_mem *)(pool->buf_pool + idx);
				DBGPRINT(RT_DEBUG_TRACE,("\t\t%d(0x%p):Flag=%d, buf=0x%p, dma=%llx, len=%x\n",
	...
	*/
	...
	list_for_each_entry(bus, &usb_bus_list, bus_list) {
		if (bus->root_hub) {
			...
			if (!pool) {
				DBGPRINT(RT_DEBUG_TRACE,("%s():Allocate pool structure for bus(%s) failed\n",
			...
			for (idx = 0; idx < pool_cnt; idx++) {
				...
				if (mem->buf)
					...
					DBGPRINT(RT_DEBUG_TRACE,("%s():Alloc membuf(idx:%d) for bus(%s) failed!\n",
