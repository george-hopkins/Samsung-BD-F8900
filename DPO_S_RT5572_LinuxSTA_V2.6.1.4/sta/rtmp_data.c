	...
	IN RX_BLK *pRxBlk)
{
	...
	if (pRxD->Bcast || pRxD->Mcast) {
		DBGPRINT_RAW(RT_DEBUG_TRACE, ("!!! BCAST OR MCAST COMEIN !!!\n"));
	...
	IN RX_BLK *pRxBlk)
{
	...
	if ( ((pHeader->FC.SubType == SUBTYPE_PROBE_REQ)  || (pHeader->FC.SubType == SUBTYPE_ACTION)))
	{
		...
		if (pAd->Cfg80211VifDevSet.vifDevList.size > 0)
		{
			...
			{	 
				if (CFG80211OS_RxMgmt(pNetDev, freq, (PUCHAR)pHeader, pRxWI->MPDUtotalByteCount))
				{
...
#if 0			
			if (pNetDev = RTMP_CFG80211_FindVifEntry_ByType(pAd, RT_CMD_80211_IFTYPE_P2P_CLIENT) != NULL)
			{	 
				if (CFG80211OS_RxMgmt(pNetDev, freq, (PUCHAR)pHeader, pRxWI->MPDUtotalByteCount))
				{
					DBGPRINT(RT_DEBUG_INFO, ("VIF STA CLI RtmpOsCFG80211RxMgmt OK!! TYPE = %d, freq = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
											pHeader->FC.SubType, freq, PRINT_MAC(pHeader->Addr2)));	
				}				  
				else
				{
					DBGPRINT(RT_DEBUG_INFO, ("VIF STA CLI RtmpOsCFG80211RxMgmt FAIL!! TYPE = %d, freq = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
                                            pHeader->FC.SubType, freq, PRINT_MAC(pHeader->Addr2)));	
				}												  
			}
#endif						
		...
		{	 
			if (CFG80211OS_RxMgmt(pAd->dummy_p2p_net_dev, freq, (PUCHAR)pHeader, pRxWI->MPDUtotalByteCount))
			{
