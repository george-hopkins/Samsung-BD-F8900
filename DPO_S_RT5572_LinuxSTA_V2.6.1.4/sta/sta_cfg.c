    ...
    IN  INT          KeyId)
{
    ...
    if ((KeyId <0) || (KeyId > 3))
    {
        DBGPRINT(RT_DEBUG_TRACE, ("Set_Key_Proc::Invalid argument (KeyId=%d)\n", KeyId));
        return FALSE;
    }
    ...
    switch (KeyLen)
    {
        ...
        default: /*Invalid argument */
            DBGPRINT(RT_DEBUG_TRACE, ("Set_Key_Proc::Invalid argument (=%s)\n", Key));       
            return FALSE;
    }

    DBGPRINT(RT_DEBUG_TRACE, ("Set_Key_Proc::(Key%d)\n", KeyId+1));
    hex_dump("Set_Key_Proc ==>",Key,KeyLen);
...
#ifdef ICE_WIFI
static void print_mac_address(unsigned char *mac_addr)
{
        DBGPRINT(RT_DEBUG_TRACE, ("%02x:%02x:%02x:%02x:%02x:%02x",
                mac_addr[0], mac_addr[1], mac_addr[2], mac_addr[3], mac_addr[4], mac_addr[5]));
}

static void print_scan_info(ICE_WiFiScanInfo *pScanInfo)
{
	DBGPRINT(RT_DEBUG_TRACE, ("======================\n"));
	DBGPRINT(RT_DEBUG_TRACE, ("rssi = %u, Channel = %u, , phy_noise = %u, mode_802_11 = %u, ",
		pScanInfo->rssi, (unsigned int) pScanInfo->channel, pScanInfo->phy_noise, pScanInfo->mode_802_11));
	DBGPRINT(RT_DEBUG_TRACE, ("bssid = "));
	print_mac_address(pScanInfo->bssid);
	DBGPRINT(RT_DEBUG_TRACE, (", is_locked = %u, is_wps = %u\n", pScanInfo->is_locked, pScanInfo->is_wps));
	DBGPRINT(RT_DEBUG_TRACE, ("======================\n"));
}
#endif /* ICE_WIFI */
...
/*            kfree(pBssidList); */
            ...
            break;
#ifdef ICE_WIFI
	case OID_802_11_SCAN_NUM:
		DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_802_11_SCAN_NUM\n"));
		Status = copy_to_user(wrq->u.data.pointer, &pAd->ScanTab.BssNr, sizeof(INT));
		break;
	case OID_802_11_ICE_SCAN_RESULT:
	{	
		INT TableInx;
		ICE_WiFiScanResults IwScanResults;
		//RT_CMD_STA_IOCTL_BSS Qbss;
		
		DBGPRINT(RT_DEBUG_TRACE, ("Query::OID_802_11_ICE_SCAN_RESULT (%d BSS returned)\n",pAd->ScanTab.BssNr));
		pAd->StaCfg.bSkipAutoScanConn = FALSE;
		if (!pAd->ScanTab.BssNr)
		{
			DBGPRINT(RT_DEBUG_TRACE, ("No BSS entry found.\n"));
			break;
		}
		/* Condition check */

		/* Allocate memory for the data_buf*/
		BssBufSize = pAd->ScanTab.BssNr * sizeof(ICE_WiFiScanInfo);
		DBGPRINT(RT_DEBUG_TRACE, ("BssBufSize = %lu\n", BssBufSize));
		os_alloc_mem(pAd, (UCHAR **)&pBuf, BssBufSize);
		if(pBuf == NULL)
		{
			Status = -ENOMEM;
		...
		/* */
		IwScanResults.num_ap = pAd->ScanTab.BssNr;
		IwScanResults.scan_data = (ICE_WiFiScanInfo *) pBuf;
		
		/* Get table */
		for (TableInx = 0; TableInx < pAd->ScanTab.BssNr; TableInx++)
		{
			ICE_WiFiScanInfo *pScanInfo = &IwScanResults.scan_data[TableInx];
			ICE_WiFiCredential *pCredential = &pScanInfo->credentials;
			PBSS_ENTRY		pEntry = &pAd->ScanTab.BssEntry[TableInx];
			BOOLEAN isGonly=FALSE;
			INT rateCnt=0;

				/* rssi */
			if (pEntry->Rssi >= -50)
				pScanInfo->rssi = ICE_wifi_signal_excellent;
			else if (pEntry->Rssi >= -65)    /* between -50 ~ -65dbm*/
				pScanInfo->rssi = ICE_wifi_signal_good;
			else if (pEntry->Rssi >= -80)   /* between -65 ~ -80dbm*/
				pScanInfo->rssi = ICE_wifi_signal_fair;
			else if (pEntry->Rssi >= -90)   /* between -80 ~ -90dbm*/
				pScanInfo->rssi = ICE_wifi_signal_poor;
			else
				pScanInfo->rssi = ICE_wifi_signal_none;
				
			/* channel */
			pScanInfo->channel = pEntry->Channel;

			/* noise */
			//set_quality(&Qbss, pEntry);
			pScanInfo->phy_noise = 0;//Qbss.Noise;

			/* mode_802_11 */
			if (pEntry->Channel > 14)
			{
				if (pEntry->HtCapabilityLen!=0)
					pScanInfo->mode_802_11 = ICE_WiFi802_11_a | ICE_WiFi802_11_n;
				else	
					pScanInfo->mode_802_11 = ICE_WiFi802_11_a;
			}
			else
			{
				/*
				...
				*/
				for (rateCnt=0;rateCnt < pEntry->SupRateLen;rateCnt++)
				{									
					/*
					6Mbps(140) 9Mbps(146) and >=12Mbps(152) are supported rate , it mean G only. 
					*/
					if (pEntry->SupRate[rateCnt]==140 || pEntry->SupRate[rateCnt]==146 || pEntry->SupRate[rateCnt]>=152)
					isGonly=TRUE;
				}

				for (rateCnt=0;rateCnt<pEntry->ExtRateLen;rateCnt++)
				{
					if (pEntry->ExtRate[rateCnt]==140 || pEntry->ExtRate[rateCnt]==146 || pEntry->ExtRate[rateCnt]>=152)
					isGonly=TRUE;
				}		
				...
				{
					if (isGonly==TRUE)
						pScanInfo->mode_802_11 = ICE_WiFi802_11_g | ICE_WiFi802_11_n;
					else
						pScanInfo->mode_802_11 = ICE_WiFi802_11_g | ICE_WiFi802_11_g | ICE_WiFi802_11_n;
				}
				else
				{
					if (isGonly==TRUE)
						pScanInfo->mode_802_11 = ICE_WiFi802_11_g;
					else
					{
						if (pEntry->SupRateLen==4 && pEntry->ExtRateLen==0)
							pScanInfo->mode_802_11 = ICE_WiFi802_11_g;
						else
							pScanInfo->mode_802_11 = ICE_WiFi802_11_b | ICE_WiFi802_11_g;
					}
				}
			}

			/* bssid */
			NdisCopyMemory(&pScanInfo->bssid, pEntry->Bssid, MAC_ADDR_LEN);

			/* credential */
			{
				/* auth_type & auth_mode */
			if (pEntry->AuthMode== Ndis802_11AuthModeOpen)
			{
				pCredential->auth_type = ICE_wifi_auth_open;
				pCredential->auth_mode = ICE_wifi_auth_disabled;
			}
			else if (pEntry->AuthMode== Ndis802_11AuthModeShared)
			{
				pCredential->auth_type = ICE_wifi_auth_shared;
				pCredential->auth_mode = ICE_wifi_auth_disabled;
			}
			else if (pEntry->AuthMode== Ndis802_11AuthModeAutoSwitch)
			{
				pCredential->auth_type = ICE_wifi_auth_open_shared;
				pCredential->auth_mode = ICE_wifi_auth_disabled;
			}
			else if (pEntry->AuthMode== Ndis802_11AuthModeWPA)
			{
				pCredential->auth_type = ICE_wifi_auth_invalid;
				pCredential->auth_mode = ICE_wifi_auth_wpa_unsp;
			}			
			else if (pEntry->AuthMode== Ndis802_11AuthModeWPAPSK)
			{
				pCredential->auth_type = ICE_wifi_auth_invalid;
				pCredential->auth_mode = ICE_wifi_auth_wpa_psk;
			}
			else if (pEntry->AuthMode== Ndis802_11AuthModeWPA2 ||
					pEntry->AuthMode== Ndis802_11AuthModeWPA1WPA2)
			{
				pCredential->auth_type = ICE_wifi_auth_invalid;
				pCredential->auth_mode = ICE_wifi_auth_wpa2_unsp;
			}
			else if (pEntry->AuthMode== Ndis802_11AuthModeWPA2PSK ||
					pEntry->AuthMode== Ndis802_11AuthModeWPA1PSKWPA2PSK)
			{
				pCredential->auth_type = ICE_wifi_auth_invalid;
				pCredential->auth_mode = ICE_wifi_auth_wpa2_psk;
			}
			else
			{
				pCredential->auth_type = ICE_wifi_auth_invalid;
				pCredential->auth_mode = ICE_wifi_auth_wpa_invalid;
			}
			
				/* sec_type */
			if (pEntry->WepStatus == Ndis802_11WEPDisabled)
				pCredential->sec_type = ICE_wifi_sec_none;
			else if (pEntry->WepStatus == Ndis802_11WEPEnabled)
				pCredential->sec_type = ICE_wifi_sec_wep;
			else if (pEntry->WepStatus == Ndis802_11Encryption2Enabled)
				pCredential->sec_type = ICE_wifi_sec_tkip;
			else if (pEntry->WepStatus == Ndis802_11Encryption3Enabled ||
					pEntry->WepStatus == Ndis802_11Encryption4Enabled)
				pCredential->sec_type = ICE_wifi_sec_aes;
			else
				pCredential->sec_type = ICE_wifi_sec_invalid;
			
				/* wep_index */
			pCredential->wep_index = 0;
				
				/* algo_override */
			pCredential->algo_override = 0;
				
				/* key_length */
			pCredential->key_length = 0;
				
				/* op_mode */
			if (pEntry->BssType == BSS_INFRA)
				pCredential->op_mode = ICE_wifi_mode_infra;
			else if (pEntry->BssType == BSS_ADHOC)
				pCredential->op_mode = ICE_wifi_mode_ad_hoc;
			else
				pCredential->op_mode = BSS_ANY;	/* Ndis802_11AutoUnknown */
			
				/* key */
			NdisZeroMemory(pCredential->key, ICE_WIFI_MAX_KEY_LEN);
				
				/* ssid */
			strcpy(pCredential->ssid, pEntry->Ssid);
				
				/* bssid */
			NdisCopyMemory(&pCredential->bssid, pEntry->Bssid, MAC_ADDR_LEN);
				
				/* band */
			if (pEntry->Channel > 14)
				pCredential->band = ICE_WiFiBand5G;
			else if (pEntry->Channel <= 14)
				pCredential->band = ICE_WiFiBand2G;
			else
				pCredential->band = ICE_WiFiInvalid;
			
				/* channel */
			pCredential->channel = pEntry->Channel;
				
				/* wps */
			if (pEntry->WpsAP)
			{
				if (pEntry->WscDPIDFromWpsAP == DEV_PASS_ID_PIN)
					pCredential->wps = ICE_wifi_wps_pin;
				else if (pEntry->WscDPIDFromWpsAP == DEV_PASS_ID_PBC)
					pCredential->wps = ICE_wifi_wps_pbc;
				else
					pCredential->wps = ICE_wifi_wps_none;
			}
			else
				pCredential->wps = ICE_wifi_wps_none;
			...
			pCredential->pin = 0;
				/* net_if */
			pCredential->net_if = NULL;
			}
			
			/* is_locked */
			pScanInfo->is_locked = 0;

			/* is_wps */
			pScanInfo->is_wps = pEntry->WpsAP;

		}

		for (TableInx= 0; TableInx < IwScanResults.num_ap; TableInx++)
		{
			print_scan_info(&IwScanResults.scan_data[TableInx]);
		}

              Status = copy_to_user(wrq->u.data.pointer, pBuf, wrq->u.data.length);
		printk("wrq->u.data.pointer.ap_num = %u, length = %u\n", IwScanResults.num_ap, wrq->u.data.length);
	...
	case OID_BW_CAPABILITY:
		{
		INT ChannelWidth = 0;
		ChannelWidth = pAd->CommonCfg.RegTransmitSetting.field.BW;
		Status = copy_to_user(wrq->u.data.pointer, 
							&ChannelWidth,//pAd->CommonCfg.HtCapability.HtCapInfo.ChannelWidth, 
							wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE, ("ChannelWidth=%d\n", ChannelWidth));
		break;
		}
	case OID_AUTH_STATUS:
		{
		INT AuthStatus = 0;		
		AuthStatus = pAd->StaCfg.PortSecured;
		wrq->u.data.length = sizeof(UCHAR);
		Status = copy_to_user(wrq->u.data.pointer, 
							&AuthStatus, 
							wrq->u.data.length);
		DBGPRINT(RT_DEBUG_TRACE, ("AuthStatus=%d\n", AuthStatus));
		break;
		}
#endif /* ICE_WIFI */
