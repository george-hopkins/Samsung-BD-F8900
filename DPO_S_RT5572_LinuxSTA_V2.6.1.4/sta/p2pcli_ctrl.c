...
/*	
	remove unnecessary call BssTableDeleteEntry() to speed up p2p client connection.
	Due to skip BssTableDeleteEntry() in ApCliLinkUp() / ApCliLinkDown, so the ApSiteSurvey() won't be called;
	we need to change BW to 20Mhz.
*/
	if (pAd->CommonCfg.BBPCurrentBW == BW_40)
	{
		UINT32 Data = 0, MTxCycle, macStatus = 0;
		...
		BBPValue &= (~0x18);
		RTMP_BBP_IO_WRITE8_BY_REG_ID(pAd, BBP_R4, BBPValue);
		DBGPRINT(RT_DEBUG_TRACE, ("%s :  Change BBP R4 to 20MHz.l\n", __FUNCTION__));
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s :  Switch Channel to %u\n", __FUNCTION__, pAd->CommonCfg.Channel));
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	*pCurrState = APCLI_CTRL_PROBE;
    ...
    pApCliEntry->P2pCliProbeReqTimeout++;

    if (pApCliEntry->P2pCliProbeReqTimeout >= 3)
    {
        ...
        pApCliEntry->P2pCliProbeReqTimeout = 0;
    }
	...
	if(Status == MLME_SUCCESS)
	{
		...
		if (ApCliLinkUp(pAd, ifIndex))
		{
			*pCurrState = APCLI_CTRL_CONNECTED;
			...
			RT_CFG80211_P2P_CLI_CONN_RESULT_INFORM(pAd->ApCfg.ApCliTab[MAIN_MBSSID].dev, pAd->ApCliMlmeAux.Bssid,
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	pApCliEntry = &pAd->ApCfg.ApCliTab[ifIndex];
#ifdef CONFIG_MULTI_CHANNEL
	if (pAd->P2pCfg.bP2pLinkDown != TRUE)
#endif /*CONFIG_MULTI_CHANNEL*/
	{
