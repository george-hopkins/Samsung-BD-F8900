	...
	IN UINT32 TxIdx)
{
	...
	if (pNullContext->InUse == FALSE)
	{
		...
		pTxInfo = (PTXINFO_STRUC)&pAd->NullContext[0].TransferBuffer->field.WirelessPacket[0];
		...
		pTxWI = (PTXWI_STRUC)&pAd->NullContext[0].TransferBuffer->field.WirelessPacket[TXINFO_SIZE];
