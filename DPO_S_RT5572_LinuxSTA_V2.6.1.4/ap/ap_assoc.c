	...
	/* add Ralink-specific IE here - Byte0.b0=1 for aggregation, Byte0.b1=1 for piggy-back */
{
	...
	if (P2PSubelementLen > 0)
	{
		...
		if (P2pIdx != P2P_NOT_FOUND)
		{
			...
			if (DeviceNameLen != 0)
			{
				RTMPZeroMemory(&pAd->P2pTable.Client[P2pIdx].DeviceName[0], P2P_DEVICE_NAME_LEN);
				RTMPMoveMemory(&pAd->P2pTable.Client[P2pIdx].DeviceName[0], DeviceName, DeviceNameLen);
				pAd->P2pTable.Client[P2pIdx].DeviceNameLen = DeviceNameLen;
			}
...
/*		SendSingalToDaemon(SIGUSR2, pObj->IappPid); */
		...
		if (TRUE)
        {
			...
			((pNetDev = RTMP_CFG80211_FindVifEntry_ByType(pAd, RT_CMD_80211_IFTYPE_P2P_GO)) != NULL))
				CFG80211OS_NewSta(pNetDev, Addr2, (PUCHAR)Elem->Msg, Elem->MsgLen);
			else
