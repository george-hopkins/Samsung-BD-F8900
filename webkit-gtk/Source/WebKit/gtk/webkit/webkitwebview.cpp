...
#if ENABLE(TVGG_BACKINGSTORE)
static void
webkit_web_view_load_finished(WebKitWebView *webView, WebKitWebFrame *frame, WebKitWebView *p);
static void
webkit_web_view_load_started(WebKitWebView *webView, WebKitWebFrame *frame, WebKitWebView *p);
static void
webkit_web_view_load_commited(WebKitWebView *webView, WebKitWebFrame *frame, WebKitWebView *p);
#endif // TVGG_BACKINGSTORE

#if ENABLE(TVGG_CONTROL_SOUP_CACHE)
extern WebKitSoupCache* webkit_soup_cache;
#endif // TVGG_CONTROL_SOUP_CACHE
...
void composite_with_floating_layer(WebKitWebView* webView, GdkWindow* window)
{
...
#ifdef ENABLE_HBBTV
void check_full_transparency(WebKitWebView* webView)
{
#if !defined(GTK_API_VERSION_2) || DGTK_API_VERSION_2 != 1
    ...
    const WebKitWebViewPrivate *viewPrivate = webView->priv;
    if(!viewPrivate)
    {
        return;
    }

    const WebKitViewportAttributes *viewAttr = viewPrivate->viewportAttributes.get();
    if(!viewAttr)
    {
        return;
    }

    const WebKitViewportAttributesPrivate *attr = viewAttr->priv;
    if(!attr)
    {
        return;
    }
    ...
    const WebCore::IntRect rect = WebCore::IntRect(0,0,attr->width,attr->height);
    float factor = 0.1f;
    ...
    webView->priv->fullyTransparent = r;
#else // !defined(GTK_API_VERSION_2) || DGTK_API_VERSION_2 != 1
    webView->priv->fullyTransparent = true;
#endif // !defined(GTK_API_VERSION_2) || DGTK_API_VERSION_2 != 1
}
#endif // ENABLE_HBBTV
...
#if ENABLE(TVGG_ACCELERATED_COMPOSITING)
        frame->view()->paintContents(&context, clipRect);
#else
...
#if ENABLE(TVGG_ACCELERATED_COMPOSITING)
            frame->view()->paintContents(&context, rect);
#else
...
#ifdef ENABLE_HBBTV
    String userAgentStr = frame->document()->userAgent(frame->document()->url());
    userAgentStr.makeLower();
    if (userAgentStr.contains("hbbtv") || userAgentStr.contains("oipf"))
    {
        check_full_transparency(webView);
    }
#endif // ENABLE_HBBTV
...
static gboolean webkit_web_view_draw(GtkWidget* widget, cairo_t* cr)
{
    ...
    if (priv->rootLayer) {
        IntSize size;
        void* ptr = priv->renderer->getBackingStoreBuffer(&size);

        cairoSurface = cairo_image_surface_create_for_data((unsigned char*)ptr, CAIRO_FORMAT_ARGB32, 1280, 720, 1280*4);
        cr2 = cairo_create(cairoSurface);
    }
    ...
    if (frame->contentRenderer() && frame->view()) {
#if ENABLE(TVGG_ACCELERATED_COMPOSITING)
        GraphicsContext gc(priv->rootLayer ? cr2 : cr);
#else
        ...
        if (!rectList->status && rectList->num_rectangles > 0) {
            for (int i = 0; i < rectList->num_rectangles; i++) {
                ...
                IntRect trect = enclosingIntRect(FloatRect(rectList->rectangles[i]));
            }
        }
#if ENABLE(TVGG_ACCELERATED_COMPOSITING)
        if(webView->priv->dirtyRects.size() > 0) {
            paintWebView(frame, priv->transparent, gc, rect, webView->priv->dirtyRects);
            webView->priv->dirtyRects.clear();
        }
#else
...
#if ENABLE(TVGG_ACCELERATED_COMPOSITING)
    if (priv->rootLayer) {
        composite_with_floating_layer(webView, NULL);
        cairo_destroy(cr2);
...
static gboolean webkit_web_view_key_press_event(GtkWidget* widget, GdkEventKey* event)
{
    ...
    String userAgentStr = frame->document()->userAgent(frame->document()->url());
    userAgentStr.makeLower();
    bool hbbtvBehaviour = false;
    if (userAgentStr.contains("hbbtv") || userAgentStr.contains("oipf"))
    {
        hbbtvBehaviour = true;
        printf("keyboardEvent hbbtvBehaviour on\n");
    }

    PlatformKeyboardEvent keyboardEvent(event, hbbtvBehaviour);
#else
...
static void webkit_web_view_size_allocate(GtkWidget* widget, GtkAllocation* allocation)
{
    ...
    if (!frame->view())
        return;
#if ENABLE(TVGG_BACKINGSTORE)
#ifndef FULLZOOM_BACKINGSTORE // for cairo zoom
   if (BackingStore::isAvailable()) {
       BackingStore::getInstance()->resize(allocation->width, allocation->height);
//   return;
   }
#endif // FULLZOOM_BACKINGSTORE
#endif // TVGG_BACKINGSTORE

#if ENABLE(TVGG_BACKINGSTORE)
   if (BackingStore::isAvailable() &&
           BackingStore::getInstance()->webView() == webView)
   {
       IntSize contentsSize;
       contentsSize.setWidth(frame->view()->contentsWidth());

       if (contentsSize.width() < 960)
       {
           contentsSize.setWidth(960);
       }

       contentsSize.setHeight(frame->view()->contentsHeight());

       if (contentsSize != BackingStore::getInstance()->contentsSize())
       {
           BackingStore::getInstance()->setContentsSize(contentsSize);
           //BackingStore::getInstance()->updateBackingStore(false);
       }
   }
#endif // TVGG_BACKINGSTORE
...
#if ENABLE(TVGG_HIDE_ALERT_POPUP_WINDOW)
static gboolean b_debug_js = TRUE;
void set_debug_javascript()
{
    b_debug_js = TRUE;
}
void reset_debug_javascript()
{
    b_debug_js = FALSE;
}
#endif // TVGG_HIDE_ALERT_POPUP_WINDOW
...
#else // TVGG_HIDE_ALERT_POPUP_WINDOW
...
#if ENABLE(TVGG_BROWSER)    // @20100913-sy302.park
    g_signal_new("invalidate-web-view",
            G_TYPE_FROM_CLASS(webViewClass),
            (GSignalFlags)(G_SIGNAL_RUN_LAST),
            0,
            NULL, NULL,
...
//    g_signal_new("get-screen-position",
//            G_TYPE_FROM_CLASS(webViewClass),
//            (GSignalFlags)(G_SIGNAL_RUN_LAST),
//            0,
//            NULL, NULL,
...
#if ENABLE(TVGG_CUSTOM_POPUP_MENU)
...
//                G_TYPE_POINTER, G_TYPE_POINTER, G_TYPE_POINTER);
        ...
        g_signal_new("hide-popup-menu",
                ...
                g_cclosure_marshal_VOID__VOID,
                G_TYPE_NONE, 0);
#endif // TVGG_CUSTOM_POPUP_MENU

#if ENABLE(TVGG_FLASH_IME)
        g_signal_new("show-ime",
                G_TYPE_FROM_CLASS(webViewClass),
                (GSignalFlags)(G_SIGNAL_RUN_LAST),
                0,
                NULL, NULL,
                g_cclosure_marshal_VOID__VOID,
                G_TYPE_NONE, 0);
#endif // TVGG_FLASH_IME

#if ENABLE(TVGG_TRANSLATED_ALERT)        /* @20110126-sanggyu */
        g_signal_new("script-translated-alert",
            G_TYPE_FROM_CLASS(webViewClass),
            (GSignalFlags)G_SIGNAL_RUN_LAST,
            0,
            NULL, NULL,
...
#if ENABLE(TVGG_BACKINGSTORE)
// TODO    settings->setFrameSetFlatteningEnabled(true);//hg2003.yang     2010.09.13
#endif // TVGG_BACKINGSTORE
...
#ifdef ENABLE_HBBTV
    else if (name == g_intern_string("hbbtv-application-boundary"))
        settings->setHbbTVAppBoundary(g_value_get_string(&value));
#endif // ENABLE_HBBTV
#if ENABLE(SPELLCHECK)
...
#if ENABLE(TVGG_BACKINGSTORE)
    priv->x = priv->y = 0;
    priv->zoomFactor = ZOOM_CONVERTER;
    priv->flickTimerID = -1;
    SetRootWindow(GTK_WIDGET(webView));
    webkit_backingstore_enable(true);
#endif // TVGG_BACKINGSTORE

#if ENABLE(TVGG_BACKINGSTORE)
    g_signal_connect_after(webView, "load-finished", G_CALLBACK(webkit_web_view_load_finished), webView);
    g_signal_connect(webView, "load-started", G_CALLBACK(webkit_web_view_load_started), webView);
    g_signal_connect(webView, "load-committed", G_CALLBACK(webkit_web_view_load_commited), webView);
#endif // TVGG_BACKINGSTORE

#if ENABLE(TVGG_PIP)
    MMManager::getInstance()->init();
#endif // TVGG_PIP
}
...
#if ENABLE(TVGG_BACKINGSTORE) // for cairo zoom
#ifndef FULLZOOM_BACKINGSTORE
    if (BackingStore::isAvailable()) {
        float fRate = BackingStore::getInstance()->zoomFactor();
        return fRate;
    }
#endif // FULLZOOM_BACKINGSTORE
#endif // TVGG_BACKINGSTORE
...
#ifndef FULLZOOM_BACKINGSTORE
   if (BackingStore::isAvailable()) {
       BackingStore::getInstance()->setZoomFactor(zoomLevel*100);
//TODO       BackingStore::getInstance()->updateBackingStore(true);
       //return; // hurnjoo.lee
   }
#endif // FULLZOOM_BACKINGSTORE
#endif // TVGG_BACKINGSTORE
   ...
   if (BackingStore::isAvailable()) {
       ...
       float percentDifference = (zoomLevel / webkit_web_view_get_zoom_level(webView));
       //frame->view()->setScrollPosition(IntPoint(scrollPosition.x() * percentDifference, scrollPosition.y() * percentDifference));
       //BackingStore::getInstance()->setZoom(zoomLevel, scrollPosition.x() * percentDifference, scrollPosition.y() * percentDifference);
   }
#endif // TVGG_BACKINGSTORE
...
#if ENABLE(TVGG_BACKINGSTORE)
    if (BackingStore::isAvailable() &&
            webView == BackingStore::getInstance()->webView()) {
        IntPoint scrollPosition = frame->view()->scrollPosition();

        BackingStore::getInstance()->viewRect().setX(scrollPosition.x());
        BackingStore::getInstance()->viewRect().setY(scrollPosition.y());
//        webkit_web_view_load_finished(webView, NULL, NULL);
//TODO        BackingStore::getInstance()->updateBackingStore(true);
        return;
   }
#endif // TVGG_BACKINGSTORE
...
#if ENABLE(VIDEO) && !ENABLE(TVGG_BROWSER) // { @20110804-sanggyu
...
#if ENABLE(TVGG_BROWSER)
void webkit_web_view_move_page(WebKitWebView* aWebView, int* motionX, int* motionY)
{
#if ENABLE(TVGG_BACKINGSTORE)
    if (BackingStore::isAvailable()) {
//TODO        BackingStore::getInstance()->scrollBy(*motionX, *motionY);
        return;
    }
#endif // TVGG_BACKINGSTORE

    Frame* mainFrame = core(aWebView)->mainFrame();
    if (!mainFrame)
        return;

    IntSize motionSize(*motionX, *motionY);
//TODO    mainFrame->view()->scrollBy(motionSize);
}

void webkit_web_view_full_page_to_png(WebKitWebView* webView, const gchar* fileName)
{
    g_return_if_fail(WEBKIT_IS_WEB_VIEW(webView));
    ...
    gint width = frame->view()->contentsWidth();
    gint height = frame->view()->contentsHeight();

    if (frame->contentRenderer() && frame->view()) {
// TODO        frame->view()->layoutIfNeededRecursive();
...
#if ENABLE(TVGG_BACKINGSTORE)
struct updateScrollCallBackData {
...
void webkit_web_view_update_page(WebKitWebView* aWebView)
{
    WebKitWebViewPrivate* priv = aWebView->priv;
    if (BackingStore::isAvailable() && priv->flickTimerID == -1/* && priv->bStore != NULL*/) {
        BackingStore::getInstance()->scrollStop();
        return;
    }
}

static gboolean updateScrollCallBack(gpointer callBackData)
{
    updateScrollCallBackData* data = (updateScrollCallBackData*) callBackData;
    WebKitWebViewPrivate* priv = data->aWebView->priv;

    // get current time and real interval
    struct timeval time;
    gettimeofday(&time, NULL);
    unsigned int currentTime = time.tv_sec * 1000 + time.tv_usec / 1000;
    int realInterval = currentTime - data->time;
    data->time = currentTime;
    Frame* coreFrame = core(webkit_web_view_get_main_frame(data->aWebView));
    if (!coreFrame)
        return false;
    ...
    float curXPos = .0f, curYPos = .0f;

    if(view->verticalScrollbar() && BackingStore::isAvailable()) {
        curYPos = (float)BackingStore::getInstance()->viewRect().y();
    }
    if(view->horizontalScrollbar() && BackingStore::isAvailable()) {
        curXPos = (float)BackingStore::getInstance()->viewRect().x();
    }
...
#ifndef FULLZOOM_BACKINGSTORE
    float zoomFactor = webkit_web_view_get_zoom_level(data->aWebView);
    scrollPos.setY(scrollPos.y() * zoomFactor);
    scrollPos.setX(scrollPos.x() * zoomFactor);
#endif // FULLZOOM_BACKINGSTORE

    data->offsetY = (int)(scrollPos.y() - curYPos);
    data->offsetX = (int)(scrollPos.x() - curXPos);
    ...
    if (abs(dy) == 0 && abs(dx) == 0) {
        g_source_remove(priv->flickTimerID);
        return false;
    }
    ...
    data->offsetX -= dx;
    data->offsetY -= dy;
    webkit_web_view_move_page(data->aWebView, &dx, &dy);

#if USE(UNIPLAYER)
    WebCore::MediaPlayerPrivate::willScroll();
#endif // UNIPLAYER

    // if dx and dy are not zero, stop flicking
    // webkit_web_view_move_page set dx and dy after moving.
    // if it does not set dx and dy as zero, it means we can not move anymore.
    if (dy != 0 && dx !=0) {
        data->offsetX = 0;
        data->offsetY = 0;
        g_source_remove(priv->flickTimerID);
...
static void updateScrollClear(gpointer callBackData)
{
    updateScrollCallBackData* data = (updateScrollCallBackData*) callBackData;
    WebKitWebViewPrivate* priv = data->aWebView->priv;

    priv->flickTimerID = -1;
    if (data->iterationIndex > 1) {
#if USE(UNIPLAYER)
        WebCore::MediaPlayerPrivate::didScroll();
#endif // UNIPLAYER
        webkit_web_view_update_page(data->aWebView);
    }
}

void webkit_web_view_update_scroll(WebKitWebView* aWebView)
{
    static updateScrollCallBackData data = {NULL, 0, 0, 0, 0};

    if (!aWebView)
        return;

    // reset flick operation
    WebKitWebViewPrivate* priv = aWebView->priv;
    if (priv->flickTimerID >= 0) {
        g_source_remove(priv->flickTimerID);
        priv->flickTimerID = -1;
    }

    // set data for flicking
    data.aWebView = aWebView;
    data.offsetX = 0;
    data.offsetY = 0;
    ...
    // start flick timer
    priv->flickTimerID = g_timeout_add_full(G_PRIORITY_HIGH, FLICK_RENDERING_INTERVAL,
                                            updateScrollCallBack, &data, updateScrollClear);
}


void webkit_backingstore_change_web_view(WebKitWebView* webView)
{
//TODO    if (BackingStore::isAvailable())
//TODO        BackingStore::getInstance()->changeWidget(webView);
}

#if ENABLE(TVGG_BACKINGSTORE)
static void webkit_web_view_load_finished(WebKitWebView* webView, WebKitWebFrame* frame, WebKitWebView* p)
{
    WebKitWebViewPrivate* priv = webView->priv;
    ...
    if (BackingStore::isAvailable()
        ...
        IntSize contentsSize;
        contentsSize.setWidth(view->contentsWidth());

        if (contentsSize.width() < 960)
            contentsSize.setWidth(960);

        contentsSize.setHeight(view->contentsHeight());

        if (contentsSize != BackingStore::getInstance()->contentsSize())
            BackingStore::getInstance()->setContentsSize(contentsSize);

        backingStore->pageLoadFinished();
    }
}

static void webkit_web_view_load_started(WebKitWebView* webView, WebKitWebFrame* frame, WebKitWebView* p)
{
    if (BackingStore::isAvailable()
            && BackingStore::getInstance()->webView() == webView)
        BackingStore::getInstance()->pageLoadStart();
}

static void webkit_web_view_load_commited(WebKitWebView* webView, WebKitWebFrame* frame, WebKitWebView* p)
{
    Frame* mainFrame = core(webView)->mainFrame();
    if (!mainFrame->view())
        return;
    if (BackingStore::isAvailable() &&
        ...
        IntSize contentsSize;
        contentsSize.setWidth(mainFrame->view()->contentsWidth());

        if (contentsSize.width() < pluginViewRect.width()) {
            contentsSize.setWidth(pluginViewRect.width());
        }

//TODO        BackingStore::getInstance()->resetViewState();
        contentsSize.setHeight(mainFrame->view()->contentsHeight());
        if (contentsSize.height() < pluginViewRect.height()) {
            contentsSize.setHeight(pluginViewRect.height());
        }
        BackingStore::getInstance()->setContentsSize(contentsSize);
//TODO        BackingStore::getInstance()->clearBS();
//TODO        BackingStore::getInstance()->updateBackingStore(true);
    }
}
#endif // TVGG_BACKINGSTORE

void webkit_backingstore_enable(bool bEnable)
{
    BackingStore::setAvailable(bEnable);
}
#endif // TVGG_BACKINGSTORE


#if ENABLE(TVGG_CACHE_BOUNDARY)
extern void (*cacheOutOfSpaceCallBack)(void);
void soupCacheOutOfSpaceCallback(void)
{
    printf("[shivamidow] %s (%s %d)\n", __FUNCTION__, __FILE__, __LINE__);
    if (cacheOutOfSpaceCallBack)
        cacheOutOfSpaceCallBack();
    else {
        if (webkit_soup_cache)
            webkit_soup_cache_empty_cache_path(webkit_soup_cache);
    }
}
#endif // TVGG_CACHE_BOUNDARY

#if ENABLE(TVGG_CONTROL_SOUP_CACHE)
bool webkit_web_view_soup_cache_init(const char* aPathToCreate, bool isShared)
{
    if(webkit_soup_cache)
        return false;

    if (aPathToCreate) {
        webkit_soup_cache = webkit_soup_cache_new(aPathToCreate
            , isShared ? WEBKIT_SOUP_CACHE_SHARED : WEBKIT_SOUP_CACHE_SINGLE_USER);
    } else {
        webkit_soup_cache = webkit_soup_cache_new(DEFAULT_SOUP_CACHE_PATH
            , isShared ? WEBKIT_SOUP_CACHE_SHARED : WEBKIT_SOUP_CACHE_SINGLE_USER);
    }
...
#if ENABLE(TVGG_CACHE_BOUNDARY)
    g_signal_connect(G_OBJECT(webkit_soup_cache), "cache,boundary,reached", G_CALLBACK(soupCacheOutOfSpaceCallback), 0);
#endif // TVGG_CACHE_BOUNDARY

    SoupSession* soup_session = WebCore::ResourceHandle::defaultSession();
    if(!soup_session)
        return false;
    soup_session_add_feature(soup_session, SOUP_SESSION_FEATURE(webkit_soup_cache));
...
#ifdef ENABLE_HBBTV
WEBKIT_API gboolean
webkit_web_view_is_fully_transparent(WebKitWebView *webView)
{
    if( !webView ||
        !webView->priv )
    {
    ...
    return webView->priv->fullyTransparent;
}
#endif // ENABLE_HBBTV
