...
struct _WebKitWebViewPrivate {
...
#ifdef ENABLE_HBBTV
    // A flag to indicate if webview has anything rendered or not
    gboolean fullyTransparent;
#endif // ENABLE_HBBTV
    ...
    GdkPixmap* rootLayer;
#else
    cairo_surface_t* rootLayer;
#endif // #ifdef GTK_API_VERSION_2
    RefPtr<WebCore::GraphicsLayerRenderer> renderer;
    Vector<WebCore::IntRect> dirtyRects;
...
#if USE(ACCELERATED_COMPOSITING) /* hurnjoo.lee@samsung.com */
void composite_with_floating_layer(WebKitWebView* webView, GdkWindow* window);
void webkit_web_view_create_root_layer(WebKitWebView *webView, WebCore::GraphicsLayer* graphicsLayer);
#endif
