...
#ifdef ENABLE_HBBTV
    /**
    * WebKitWebSettings:hbbtv-application-boundary:
    *
    * Set / Get the hbbtv application boundary.
    */
    g_object_class_install_property(gobject_class, PROP_HBBTV_APP_BOUNDARY,
                                    g_param_spec_string("hbbtv-application-boundary",
                                                       _("HbbTV Application Boundary"),
                                                       _("The application boundary used by HbbTV"),
                                                       0,
                                                       flags));
#endif // ENABLE_HBBTV
...
static void webkit_web_settings_set_property(GObject* object, guint prop_id, const GValue* value, GParamSpec* pspec)
{
    ...
    case PROP_ENABLE_WEBGL:
        ...
        break;
#ifdef ENABLE_HBBTV
    case PROP_HBBTV_APP_BOUNDARY:
        if (!g_value_get_string(value) || !strlen(g_value_get_string(value)))
            priv->hbbtvAppBoundary = "";
        else
            priv->hbbtvAppBoundary = g_value_get_string(value);
        break;
#endif // ENABLE_HBBTV
    default:
...
#ifdef ENABLE_HBBTV
    case PROP_HBBTV_APP_BOUNDARY:
        g_value_set_string(value, priv->hbbtvAppBoundary.data());
        break;
#endif // ENABLE_HBBTV
    default:
        G_OBJECT_WARN_INVALID_PROPERTY_ID(object, prop_id, pspec);
