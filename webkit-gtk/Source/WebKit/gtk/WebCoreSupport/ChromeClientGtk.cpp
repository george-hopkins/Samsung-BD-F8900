...
extern "C" {
int XCreateEGLImage(unsigned char** ppBuffer, void** pHndl, unsigned int nWidth, unsigned int nHeight, unsigned int nBitDepth);
int XDestroyEGLImage(unsigned char* ppBuffer);
}
...
{
#ifdef ENABLE_HBBTV
     return runJavaScriptConfirm(NULL, "shouldInterruptJavaScript");
#else
    notImplemented();
    return false;
#endif
...
#if ENABLE(TVGG_BROWSER) // @20100614-sy302.park
    else {
#if ENABLE(TVGG_BACKINGSTORE)
...
// TODO            rect = BackingStore::getInstance()->addToDirtyRegion(updateRect);
// TODO            if (rect.width == 0 || rect.height == 0)
// TODO                return;

            WebCore::IntRect contentsRect = updateRect;
            UpdateRootWindow(&contentsRect);
        }
#else // TVGG_BACKINGSTORE
#if 0
        if (m_webView->priv)
            m_webView->priv->dirtyRects.append(updateRect);
#endif
        g_signal_emit_by_name(m_webView, "invalidate-web-view", &updateRect, NULL);
#endif // TVGG_BACKINGSTORE
    }
#endif // TVGG_BROWSER
}
...
#if ENABLE(TVGG_BACKINGSTORE)
    if (BackingStore::isAvailable()) {
        webkit_web_view_update_scroll(const_cast<WebKitWebView*>(
                     BackingStore::getInstance()->webView()));
        return;
    }
#endif // TVGG_BACKINGSTORE
...
#if ENABLE(TVGG_BROWSER) // @20100614-sy302.park
    {
        g_signal_emit_by_name(m_webView, "invalidate-web-view", &clipRect, &delta);
#endif // TVGG_BROWSER
        return;
#if ENABLE(TVGG_BROWSER) // @20100614-sy302.park
    }
...
void ChromeClient::contentsSizeChanged(Frame* frame, const IntSize& size) const
{
    ...
    if (WebKit::BackingStore::isAvailable()
        ...
        WebKit::BackingStore::getInstance()->setContentsSize(size);
    }
#else // TVGG_BACKINGSTORE
...
{
#if ENABLE(TVGG_CUSTOM_FILE_DIALOG)
    return;
#endif // TVGG_CUSTOM_FILE_DIALOG
...
void ChromeClient::attachRootGraphicsLayer(Frame*, GraphicsLayer*)
{
