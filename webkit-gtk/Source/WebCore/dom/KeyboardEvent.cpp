...
extern int (*weGetInputDevice)(void);
extern int (*weGetKeypadType)(void);
#endif
...
#if ENABLE(WIDGET_ENGINE)
//3 WEBKIT_WIDGETENGINE
int KeyboardEvent::inputDevice()
{
	int iInputDevice = 0;
	if (weGetInputDevice)
	{
		iInputDevice = weGetInputDevice();
	}
	return iInputDevice;
}

int KeyboardEvent::keypadType()
{
	int iKeypadType = 0;
	if (weGetKeypadType)
	{
		iKeypadType = weGetKeypadType();
	}
	return iKeypadType;
}
