...
#ifdef ENABLE_HBBTV
bool Node::s_flagReattachForHbbTV = false;
#endif
...
{
#if ENABLE(TVGG_SPATIAL_NAVIGATION)
    if (document()->page() && document()->page()->settings()->isSpatialNavigationEnabled())
        return (hasRareData() && rareData()->tabIndexSetExplicitly()) || !const_cast<Node*>(this)->getEventListeners(eventNames().clickEvent).isEmpty();
    else
#endif
...
#ifdef ENABLE_HBBTV
void Node::detach(bool reattaching)
{
    setFlag(InDetachFlag);

    bool delete_renderer = true;

    if (s_flagReattachForHbbTV)
    {
        if(reattaching && hasTagName(objectTag))
        {
            // reattaching is only set to false for HbbTV (currently) based on the user-agent during
            // a call to 'reattach()'.  We don't want to delete the renderer for the plugin, as this
            // will lose the state of the video/broadcast or AV Control plugins.
            delete_renderer = false;
            if (renderer())
            {
                // We do need to remove the renderer from the parent.
                renderer()->remove();
            }
        }
    }
...
#ifdef ENABLE_HBBTV
static bool isInDisplayNone(Node* n)
{
    for (; n; n = n->parentNode())
    {
        Node* p = n->parentNode();
        RenderObject* r = p? p->renderer() : 0;
        RenderStyle* style = r? r->style() : 0;
...
bool Node::rendererIsNeeded(const NodeRenderingContext& context)
{
    ...
    {
        // For HbbTV and OIPF, the style setting 'display: none' must not cause the plugins
        // not to load.  Therefore we create renderers for div and object elements, and if the
        // style is display: none, make the renderer width and height 0, so that they do not
        // effect the layout.
        if (hasTagName(divTag) || hasTagName(objectTag)) {
            return true;
        }
    }

    bool displayNone = (context.style()->display() == NONE);
    if(!displayNone)
    {
        displayNone = isInDisplayNone(this);
    }

    return (document()->documentElement() == this) || (displayNone == false);
#else
...
//3 WEBKIT_WIDGETENGINE
    if (useWidgetEngine == true)
    {
        ...
        bHasPrefix = localName.contains(":");

        if (bHasPrefix == true && firstChild() != NULL)
        {
            Vector<String> strVector;
            String searchString = localName;
            searchString.split(":", strVector);

            String namespaceURI = firstChild()->lookupNamespaceURI(strVector[0]);

            return getElementsByTagNameNS(namespaceURI, strVector[1]);
        }
    }
#endif
...
void Node::reattach()
{
    if (attached())
    {
        String agent = m_document->userAgent(m_document->url());
        if(agent.contains("HbbTV") || agent.contains("OIPF"))
        {
            // Keep the renderer for HbbTV plugins to prevent them being destroyed and
            // re-created by a re-attach.
            detach(true);
        }
        else
        {
...
#else
// This code in in header file orignally.
#endif
