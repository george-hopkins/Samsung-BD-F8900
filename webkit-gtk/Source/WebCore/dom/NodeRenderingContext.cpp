...
RenderObject* NodeRenderingContext::parentRenderer() const
{
    if (RenderObject* renderer = m_node->renderer()) {
#ifdef ENABLE_HBBTV
        if (Node::s_flagReattachForHbbTV)
        {
            // For HbbTV, it is possible to reach this code with no parent renderer, as the renderer
            // for objects (plugins, e.g. video/broadcast) is retained during a reattach() to prevent
            // the destruction of the plugin (and therefore loss of state).
            if(renderer->parent() == NULL && m_location == LocationLightChild) {
...
#ifndef ENABLE_HBBTV
    // HbbTV can reach here with a valid renderer for object tags, as the renderer is retained
    // during reatach() calls.
...
#ifdef ENABLE_HBBTV
    RenderObject* newRenderer = NULL;

    if (Node::s_flagReattachForHbbTV)
    {
        // For HbbTV, if we still have our renderer, use it, otherwise create a new one.
        newRenderer = node->renderer();
        if(newRenderer == NULL) {
            newRenderer = createRendererAndStyle();
        }
    }
    else
    {
