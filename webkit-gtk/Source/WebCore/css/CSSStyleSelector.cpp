...
#if ENABLE(FULLSCREEN_API) && !ENABLE(TVGG_BROWSER)
...
#ifdef ENABLE_HBBTV
StyleNavigationValue CSSStyleSelector::getStyleNavigationValue(CSSValue* value)
{
    if (value->isValueList()) {
        CSSValueList* list = static_cast<CSSValueList*>(value);
        StringImpl* val[2] = {0};
        ...
        for (size_t i = 0; i < 2; i++) {
            ...
            primitiveValue = static_cast<CSSPrimitiveValue*>(item);
            if (primitiveValue->primitiveType() == CSSPrimitiveValue::CSS_STRING)
                val[i] = primitiveValue->getStringValue().impl();
        }
        return StyleNavigationValue(AtomicString(val[0]), AtomicString(val[1]));
    }
