...
#ifdef ENABLE_HBBTV
static PassRefPtr<CSSValue> navDirToCSSValue(const StyleNavigationValue& navValue, CSSPrimitiveValueCache* primitiveValueCache)
{
    RefPtr<CSSValueList> list = CSSValueList::createSpaceSeparated();
    list->append(primitiveValueCache->createValue(navValue.id().string(), CSSPrimitiveValue::CSS_STRING));
    list->append(primitiveValueCache->createValue(navValue.target().string(), CSSPrimitiveValue::CSS_STRING));
...
PassRefPtr<CSSValue> CSSComputedStyleDeclaration::getPropertyCSSValue(int propertyID, EUpdateLayout updateLayout) const
{
    ...
    switch (static_cast<CSSPropertyID>(propertyID)) {
        ...
        case CSSPropertyNavDown:
            return navDirToCSSValue(style->navDown(), primitiveValueCache);
        case CSSPropertyNavLeft:
            return navDirToCSSValue(style->navLeft(), primitiveValueCache);
        case CSSPropertyNavRight:
            return navDirToCSSValue(style->navRight(), primitiveValueCache);
        case CSSPropertyNavUp:
            return navDirToCSSValue(style->navUp(), primitiveValueCache);
#endif
