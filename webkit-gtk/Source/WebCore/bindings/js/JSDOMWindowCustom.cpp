...
bool JSDOMWindow::getOwnPropertySlot(ExecState* exec, const Identifier& propertyName, PropertySlot& slot)
{
    ...
    bool allow = document->isHTMLDocument();
    String agent = document->userAgent(document->url());
    if(agent.contains("HbbTV") || agent.contains("OIPF"))
    {
        allow = allow || document->isXHTMLDocument();
    }

    if (allow) {
        AtomicStringImpl* atomicPropertyName = findAtomicString(propertyName);
        if (atomicPropertyName && (document->hasNamedItem(atomicPropertyName) || document->hasElementWithId(atomicPropertyName))) {
