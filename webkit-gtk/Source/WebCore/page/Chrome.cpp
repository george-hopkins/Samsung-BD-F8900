...
NativeImagePtr Chrome::getPageSnapshot(void* context, uint64_t pageID) const
{
    return m_client->getPageSnapshot(context, pageID);
}
#endif

#if ENABLE(TVGG_ANIMATED_SCROLL) 
 void Chrome::setDragScrollFlag(bool flag)
 {
     m_client->setDragScrollFlag(flag);
 }
 #endif
