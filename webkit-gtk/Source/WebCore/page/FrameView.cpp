...
#ifdef ENABLE_HBBTV
    // In HbbTV context scrollbars are to be avoided.
    const Document *document = m_frame->document();
    if(document)
    {
        String userAgentStr = document->userAgent(document->url());
...
{
#if ENABLE(TVGG_SPATIAL_NAVIGATION)
    if (parent() && m_frame == m_frame->page()->focusController()->focusedFrame()) {
...
void FrameView::repaintContentRectangle(const IntRect& r, bool immediate)
{
    ...
    coverRect.inflateY(visibleRect.height() * (m_coverAreaMultiplier.width() - 1.f) / 2);

    paintRect.intersect(coverRect);
#else
#if !ENABLE(TVGG_BACKINGSTORE)
        // BackingStore needs to paint entire contents.
        if (clipsRepaints() && !paintsEntireContents())
            paintRect.intersect(visibleContentRect());
#endif // TVGG_BACKINGSTORE
#endif // TVGG_EXTENDED_BACKING_STORE
...
#if ENABLE(TVGG_SPATIAL_NAVIGATION)
IntPoint FrameView::paintFocusRing(GraphicsContext* p)
{
    IntPoint location(-1, -1);

    Node* focusedNode = m_frame->document()->focusedNode();
    if (!focusedNode)
        return location;
    ...
    if (focusedNode->hasTagName(areaTag)) {
        ...
        IntPoint absolutePoint(convertToContainingWindow(-scrollPosition()));
        path.translate(FloatSize(absolutePoint.x(), absolutePoint.y()));

        IntRect clippedRect(imageRenderer->absoluteClippedOverflowRect());
        clippedRect.intersect(visibleContentRect());
        clippedRect.move(-scrollOffset());
        clippedRect = convertToContainingWindow(clippedRect);

        IntRect boundingRect(path.boundingRect());
        boundingRect.intersect(clippedRect);
        ...
        RenderStyle* style = imageRenderer->style();
        p->drawFocusRing(path, style->outlineWidth(), style->outlineOffset(), style->visitedDependentColor(CSSPropertyOutlineColor));

        p->restore();

        location = boundingRect.location();
    } else if (focusedNode->renderer()) {
        ...
        absolutePoint = convertToContainingWindow(convertFromRenderer(focusedRenderer, absolutePoint));
        ...
        for (unsigned i = 0; i < rectCount; i++) {
            ...
            if (!isValid && !focusRingRects[i].isEmpty()) {
                isValid = true;
                location = focusRingRects[i].location();
            }
        }

        RenderStyle* style = focusedRenderer->style();
        p->drawFocusRing(focusRingRects, style->outlineWidth(), style->outlineOffset(), style->visitedDependentColor(CSSPropertyOutlineColor));
    }
