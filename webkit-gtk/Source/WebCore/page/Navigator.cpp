...
extern bool (*weUpdateWidgetUserAgent)(const char* pszUserAgent, void* pWebCorePaage);
extern char* (*weGetWidgetUserAgent)(void* pWebCorePage);
#endif
...
//3 WEBKIT_WIDGETENGINE
	...
	if (weGetWidgetUserAgent)
	{
		pszUserAgent = weGetWidgetUserAgent((void*)m_frame->page());
	}
	if (pszUserAgent)
	{
		return pszUserAgent;
...
void Navigator::setLanguage(const String& language)
{
	overrideDefaultLanguage(language);
}

void Navigator::setUserAgent(const String& userAgent)
{
	if (weUpdateWidgetUserAgent)
	{
		String stNewString = String(userAgent);
		weUpdateWidgetUserAgent(stNewString.utf8().data(), (void*)m_frame->page());
	}
}
