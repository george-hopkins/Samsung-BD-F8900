...
#ifdef ENABLE_HBBTV
            {
                String agent = doc->userAgent(doc->url());
                if(agent.contains("HbbTV") || agent.contains("OIPF"))
                {
                    // For HbbTV and OIPF, the style setting 'display: none' must not cause the
                    // plugins not to load.  Therefore we create renderers for everything, and if
                    // the style is display: none, make the renderer width and height 0, so that
                    // they do not effect the layout.
...
#if ENABLE(TVGG_SPATIAL_NAVIGATION)
    repaintUsingContainer(repaintContainer ? repaintContainer : view, rectWithOutlineForRepaint(repaintContainer, GraphicsContext::focusRingLineWidth() + 1), immediate);
#else
