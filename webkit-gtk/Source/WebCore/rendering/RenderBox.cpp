...
int RenderBox::containingBlockLogicalHeightForPositioned(const RenderBoxModelObject* containingBlock, bool checkForPerpendicularWritingMode) const
{
    ...
    // If a backingstore is enabled, fixed position elements show incorrectly.
    // so we recalculate containingBlockHeight in this case.
    if (containingBlock->isRenderView() && style()->position() == FixedPosition)
        heightResult = toRenderView(containingBlock)->docHeight();
