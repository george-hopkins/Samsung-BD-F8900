...
#ifdef ENABLE_HBBTV
    {
        if (Node::s_flagReattachForHbbTV)
        {
            ...
            if(agent.contains("HbbTV") || agent.contains("OIPF"))
            {
                ...
                if (wouldLoadAsNetscapePlugin(m_url, m_serviceType))
                {
                    flagHbbTVPlugIn = true;
                }
                else
                {
                    reattaching = true;
            ...
            if (flagHbbTVPlugIn == false)
            {
                // We should call reattach() here.  However, at the moment, this causes a problem in
                // HbbTV (Das Erste) where the video/broadcast object is rendered above the other items.
                // This particular flow, in all cases so far *doesn't* kill the plugin.. no idea why.
                if(attached())
                    detach(/*reattaching*/);
...
#ifdef ENABLE_HBBTV
void HTMLPlugInImageElement::detach(bool reattaching)
{
    ...
    HTMLPlugInElement::detach(reattaching);
}
#else
