...
{
#if ENABLE(TVGG_TEXT_INPUT_ELEMENT_USE_IME)
    return isTextField();
#else // TVGG_TEXT_INPUT_ELEMENT_USE_IME
    return m_inputType->shouldUseInputMethod();
#endif // TVGG_TEXT_INPUT_ELEMENT_USE_IME
}
...
#ifdef ENABLE_HBBTV
void HTMLInputElement::detach(bool reattaching)
{
    HTMLFormControlElementWithState::detach(reattaching);
    setFormControlValueMatchesRenderer(false);
}
#else
