...
#ifdef ENABLE_HBBTV
void HTMLPlugInElement::detach(bool reattaching)
{
    m_instance.clear();
    ...
    HTMLFrameOwnerElement::detach(reattaching);
}
#else
...
#if ENABLE(WIDGET_ENGINE)
//3 WEBKIT_WIDGETENGINE
void HTMLPlugInElement::focus(bool restorePreviousSelection)
{
	if (useWidgetEngine == true)
	{
		if (weSetFocusObjectElement)
		{
			weSetFocusObjectElement(this, NULL, true);
		}
	...
	return Element::focus(restorePreviousSelection);
}

void HTMLPlugInElement::blur()
{
	if (useWidgetEngine == true)
	{
		if (weSetFocusObjectElement)
		{
			weSetFocusObjectElement(this, NULL, false);
		}
