...
Document* XMLHttpRequest::responseXML(ExceptionCode& ec)
{
    ...
    if (!m_createdDocument) {
        ...
        if (useWidgetEngine == true)
        {
            ...
            String strippedString(m_responseBuilder.toStringPreserveCapacity().stripWhiteSpace());
            strippedString.replace("version=\"1.0\"encoding=", "version=\"1.0\" encoding=");
            strippedString.replace("encoding=\"EUC-KR", "encoding=\"UTF-8");
            strippedString.replace("encoding=\"euc-kr", "encoding=\"UTF-8");
            strippedString.replace("encoding=\"WIN1252", "encoding=\"UTF-8");

            m_responseXML->setContent(strippedString);
...
void XMLHttpRequest::createRequest(ExceptionCode& ec)
{
    ...
    if (useWidgetEngine == true)
    {
        ...
        String cookieVals = document()->cookie(ec);
        if (!cookieVals.isEmpty())
        {
            Vector<String> cookieVector;
            cookieVals.split("; ", cookieVector);

            unsigned size = cookieVector.size();
            for (unsigned i = 0; i < size; ++i)
            {
                String cookie = cookieVector[i];
                setRequestHeaderInternal("Cookie", cookie);
...
void XMLHttpRequest::didFinishLoading(unsigned long identifier, double)
{
    ...
    if (useWidgetEngine == true)
    {
        if (m_response.url().protocol() == "file")
        {
        ...
        ExceptionCode ec = 0;
        String cookie = getResponseHeader("Set-Cookie", ec);
        String cookie2 = getResponseHeader("Set-Cookie2", ec);

        if (!cookie.isEmpty())
        {
            document()->setCookie(cookie, ec);
        }
        if (!cookie2.isEmpty())
        {
            document()->setCookie(cookie2, ec);
        }
    }
#endif
    if (m_decoder)
