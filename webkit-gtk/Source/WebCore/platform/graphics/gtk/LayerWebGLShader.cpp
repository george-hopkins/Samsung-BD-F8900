...
PassOwnPtr<LayerWebGLShader> LayerWebGLShader::create(GraphicsContext3D* context)
{
    char verLayerWebGLShaderString[] =
    ...
    char fragmentShaderString[] =
        ...
        "}                                                   \n";
    unsigned program = loadProgram(context, verLayerWebGLShaderString, fragmentShaderString);
    ASSERT(program);

    return adoptPtr(new LayerWebGLShader(context, program));
}

LayerWebGLShader::LayerWebGLShader(GraphicsContext3D* context, unsigned program)
...
void LayerWebGLShader::setMatrix(float* matrix)
{
...
void LayerWebGLShader::setSampler(int sampler)
{
	m_context->uniform1i(m_samplerLocation, sampler);
}

void LayerWebGLShader::setAlpha(float alpha)
{
