...
#if ENABLE(TVGG_LOOKUP_THEME_RESOURCES_FROM_UI)
#include "BELog.h"
#include "Image.h"
#endif
...
{
#if ENABLE(TVGG_LOOKUP_THEME_RESOURCES_FROM_UI)
    const char* themeDir = Image::getThemeResourceDirectory();
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(icons); ++i) {
        ...
        gchar* iconPath = g_strdup_printf("%s/%s.png", themeDir, icons[i]);

        if (g_file_test(iconPath, G_FILE_TEST_EXISTS))
        {
            BELog("icon [%s] in %s EXIST\n", icons[i], iconPath);
...
void RenderThemeGtk::initMediaColors()
{
    ...
    BELog("\n");
    char* fn = g_build_filename(themeDir, "media_colors");
    BELog("%s\n", fn);
    FILE* fp = fopen(fn, "r");
    if (!fp) {
        BELog("Could not open media controller color file: %s\n", fn);
        g_free(fn);
        return;
    }
    ...
    while (fgets(buf, 128, fp) != NULL)
    {
        int nPanelColor;
        int nSliderColor;
        int nSliderThumbColor;

        BELog("%s\n", buf);
        char *strTarget = NULL;
        if((strTarget = strstr(buf, "mediaPlayer.panelColor")) != NULL){
            sscanf(buf, "mediaPlayer.panelColor %X", &nPanelColor);
            BELog("[mediaPlayer.panelColor] = %x\n", nPanelColor);
            m_panelColor = nPanelColor;
        }
        else if((strTarget = strstr(buf, "mediaPlayer.sliderColor")) != NULL){
            sscanf(buf, "mediaPlayer.sliderColor %X", &nSliderColor);
            BELog("[mediaPlayer.sliderColor] = %X\n", nSliderColor);
            m_sliderColor = nSliderColor;
        }
        else if((strTarget = strstr(buf, "mediaPlayer.sliderThumbColor")) != NULL){
            sscanf(buf, "mediaPlayer.sliderThumbColor %X", &nSliderThumbColor);
            m_sliderThumbColor = nSliderThumbColor;
            BELog("[mediaPlayer.sliderThumbColor] = %X\n", nSliderThumbColor);
        }
        else
            continue;
    }
