...
#if ENABLE(TVGG_CUSTOM_POPUP_MENU)	// @20100913-sy302.park
    IntRect windowRect(view->contentsToWindow(rect.location()), rect.size());
    g_signal_emit_by_name(GTK_WIDGET(view->hostWindow()->platformPageClient()),
        "show-popup-menu", client(), &windowRect, &index);
    return;
...
{
#if ENABLE(TVGG_CUSTOM_POPUP_MENU)	// @20100913-sy302.park
    g_signal_emit_by_name(GTK_WIDGET(client()->hostWindow()->platformPageClient()), "hide-popup-menu");
    return;
#endif // TVGG_CUSTOM_POPUP_MENU
    ASSERT(m_popup);
    m_popup->popDown();
