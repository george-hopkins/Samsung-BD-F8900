...
#ifdef USE_MEMORYWRAPPER
#include "MemoryWrapper.h"
#endif // USE_MEMORYWRAPPER
...
#include <map>
static std::map<std::string, SoupMessage*> s_fileMap;
#endif    // DUMP_FILES

bool (*g_soupDsmccFileRequest)(
    const void* handle,
    const char* szUrl,
    bool (*callback)(const void* handle, const char* szUrl, unsigned char* body, long size, bool result)) = NULL;
...
extern int (*weGetEncryptedFileType)(char* pszEncryptedFileName);
extern char* (*weGetDecrypedMemoryBuffer)(char* pszEncryptedFileName, int iEncryptedFileType, int* pDecryptedBufferSize, char** ppOriginalFileName);
extern const char* (*weGetMimeType)(char* pszFileName);
extern void (*weMemoryFree)(void* ptr);
extern char* (*weMakeAbsoluteUrl)(const char* pszUrl, const char* pszBaseUrl);
extern bool (*weSetSoupHeaders)(SoupMessageHeaders* soupHeaders, const char* pszUrl);
#endif
...
public:
    ...
    long returnDataLen;
    bool returnDataStatus; // True - Good, False - Error

    CDsmRequestResult(void* _handle,
                      char* _pszUrl,
                      unsigned char* _returnData,
                      long _returnDataLen,
                      bool _returnDataStatus) :

        handle(_handle),
        pszUrl(NULL),
        returnData(NULL),
        returnDataLen(_returnDataLen),
        returnDataStatus(_returnDataStatus)
    {
        if (_pszUrl)
        {
            size_t length = strlen(_pszUrl);
            pszUrl = new char[length+1];
            strncpy(pszUrl, _pszUrl, length+1);
        }
        if (_returnData)
        {
            returnData = new unsigned char[_returnDataLen];
            memcpy(returnData, _returnData, _returnDataLen);
        }
    }
...
static bool startDsm(ResourceHandle* handle, String urlString);
#endif
...
/* @20100911-sanggyu: remove static */
#if USE(UNIPLAYER)
void ensureSessionIsInitialized(SoupSession* session)
#else
...
#if ENABLE(TVGG_SOUP_FINISH_CALLBACK)
...
static void sendRequestCallback(GObject* source, GAsyncResult* res, gpointer userData)
{
    ...
    if (!d->m_soupMessage) {
        ...
        {
            int iEncryptedFileType = 0;
            char* pszEncryptedFileName = NULL;
            const char* pszMimeType = NULL;

            pszEncryptedFileName = (char*)malloc(handle->firstRequest().url().string().utf8().length()+1);
            if (pszEncryptedFileName)
            {
                strcpy(pszEncryptedFileName, handle->firstRequest().url().string().utf8().data());

                if (weGetEncryptedFileType)
                {
                    iEncryptedFileType = weGetEncryptedFileType(pszEncryptedFileName);
                }
                if (iEncryptedFileType != 0)
                {
                    if (weGetMimeType)
                    {
                        pszMimeType = weGetMimeType(pszEncryptedFileName);
                    }
                }
                free(pszEncryptedFileName);
            }

            /* Set default */
            if (pszMimeType == NULL)
            {
                pszMimeType = soup_request_get_content_type(d->m_soupRequest.get());
            }

            d->m_response.setURL(handle->firstRequest().url());
            contentType = pszMimeType;
        }
        else
#endif
        {
            d->m_response.setURL(handle->firstRequest().url());
            contentType = soup_request_get_content_type(d->m_soupRequest.get());
        }
...
static bool startHTTPRequest(ResourceHandle* handle)
{
...
#if ENABLE(WIDGET_ENGINE)
    //3 WEBKIT_WIDGETENGINE
    char* pszAbsoluteURL = NULL;
    if (useWidgetEngine == true)
    {
        if (weMakeAbsoluteUrl)
        {
            pszAbsoluteURL = weMakeAbsoluteUrl(url.string().utf8().data(), NULL);
        }
        if (pszAbsoluteURL && strncmp(pszAbsoluteURL, "file://", 7) == 0)
        {
            const KURL newUrl(ParsedURLString, pszAbsoluteURL);
            if (weMemoryFree)
            {
                weMemoryFree(pszAbsoluteURL);
            }

            return startNonHTTPRequest(handle, newUrl);
        }
    }
#endif
    ...
    if (useWidgetEngine == true)
    {
        if (weSetSoupHeaders)
        {
            weSetSoupHeaders(soupMessage->request_headers, url.string().utf8().data());
        }
    }
#endif
    if (!handle->shouldContentSniff())
...
#ifdef ENABLE_HBBTV
    if (equalIgnoringCase(protocol, "dvb")) {
        if (startDsm(this, urlString))
            return true;
    }
#endif
...
static void readCallback(GObject* source, GAsyncResult* asyncResult, gpointer data)
{
    ...
    if (useWidgetEngine == true)
    {
        char* pszDecryptedFileName = NULL;
        char* pszDecryptedBuffer = NULL;
        char* pszOriginFileName = NULL;

        int iEncryptedFileType = 0;
        int iDecryptedBufferSize = 0;

        pszDecryptedFileName = (char*)calloc(1, handle->firstRequest().url().string().utf8().length()+1);

        if (pszDecryptedFileName)
        {
            strncpy(pszDecryptedFileName, handle->firstRequest().url().string().utf8().data(), handle->firstRequest().url().string().utf8().length());
            if (weGetEncryptedFileType)
            {
                iEncryptedFileType = weGetEncryptedFileType(pszDecryptedFileName);
            }
        }

        if (iEncryptedFileType != 0)
        {
            if (weGetDecrypedMemoryBuffer)
            {
                pszDecryptedBuffer = weGetDecrypedMemoryBuffer(pszDecryptedFileName, iEncryptedFileType, &iDecryptedBufferSize, &pszOriginFileName);
            }
            ...
            d->m_bodySize = iDecryptedBufferSize;

            client->didReceiveData(handle.get(), pszDecryptedBuffer, iDecryptedBufferSize, iDecryptedBufferSize);

            free(pszOriginFileName);
            free(pszDecryptedFileName);
            free(pszDecryptedBuffer);
    ...
    if (!bytesRead) {
        ...
        if(soupMessage != NULL)
        {
        std::string filename(soup_uri_to_string(soup_message_get_uri(soupMessage), false));

        // strip any http://, https://, ftp:// etc.
        size_t pos = filename.find("http://");
        if(pos != std::string::npos)
        {
            filename = filename.substr(7);
        }
        // strip anything following a '?...' parameter stuff.
        pos = filename.rfind("?");
        if(pos != std::string::npos)
        {
...
#ifdef TVGG
        filename.insert(0, "/dtv/usb/sda1/http_dump/");
#else
        filename.insert(0, "http_dump/");
#endif
        printf("URI %s\n", filename.c_str());
        std::string dir = filename.substr(0, filename.find_last_of( '/' ) );
        if(dir.length() == filename.length() - 1)
        {
            printf("filename is implicity 'index.html'\n");
            filename.append("index.html");
        }
        std::map<std::string, SoupMessage*>::iterator it;
        it = s_fileMap.find(filename);
        if(it != s_fileMap.end())
        {
            s_fileMap.erase(it);
        }
...
#ifdef ENABLE_DUMP_FILES
    SoupMessage* soupMessage = d->m_soupMessage.get();
    if(soupMessage && soupMessage->status_code == 200)
    {
        std::string filename(soup_uri_to_string(soup_message_get_uri(soupMessage), false));
        ...
        {
            s_fileMap[filename] = soupMessage;
        }

        std::ofstream outfile;
        if(append)
        {
            outfile.open(filename.c_str(), std::ofstream::binary | std::ofstream::app);
        }
        else
        {
            outfile.open(filename.c_str(), std::ofstream::binary);
        }
        outfile.write(d->m_buffer, bytesRead);
    }
#endif
...
static bool startNonHTTPRequest(ResourceHandle* handle, KURL url)
{
    ...
    if (useWidgetEngine == true)
    {
        ...
        {
            const KURL newUrl(ParsedURLString, pszAbsoluteURL);
            urlStr = newUrl.string().utf8();
            handle->firstRequest().setURL(newUrl);
...
static const ExtensionMap dsmExtensionMap [] = {
    { "bmp", "image/bmp" },
    { "css", "text/css" },
    { "gif", "image/gif" },
    { "htm", "application/vnd.hbbtv.xhtml+xml" },
    { "html", "application/vnd.hbbtv.xhtml+xml" },
    { "ico", "image/x-icon" },
    { "jpeg", "image/jpeg" },
    { "jpg", "image/jpeg" },
...
String getMIMETypeForDsmExtension(const String &ext)
{
    String s = ext.lower();
    const ExtensionMap *e = dsmExtensionMap;
    while (e->extension) {
        if (s == e->extension)
            return e->mimeType;
        ++e;
...
String getMIMETypeForDsmPath(const String& path)
{
    int pos = path.reverseFind('.');
    if (pos >= 0) {
        String extension = path.substring(pos + 1);
        String result = getMIMETypeForDsmExtension(extension);
        if (result.length())
            return result;
    }
    return "application/vnd.hbbtv.xhtml+xml";
...
#ifdef DEBUG_DSM_EXTENSION
bool writeToFile(const char* filepath, unsigned char* buffer, long size)
{
    FILE * pFile;
    long result;
    ...
    if (pFile == NULL)
    {
        printf("writeToFile error cannot open file\n");
        return false;
    }

    result = fwrite (buffer ,1 , size, pFile);
    if (result != size)
    {
        printf("writeToFile error failed to write file\n");
        fclose (pFile);
        return false;
    }
...
static gboolean processDsmData(gpointer callback_data)
{
    CDsmRequestResult* dsmResult = static_cast<CDsmRequestResult*>(callback_data);
    ASSERT(dsmResult);
    if(!dsmResult)
    {
        return false;
    ...
    ResourceHandle* handle = static_cast<ResourceHandle*>(dsmResult->handle);
    ASSERT(handle);
    if(!handle)
    {
        return false;
    ...
    ResourceResponse response;
    response.setURL(KURL(ParsedURLString, dsmResult->pszUrl));
    String mimeType = getMIMETypeForDsmPath(dsmResult->pszUrl);
    response.setMimeType(mimeType);
    response.setHTTPHeaderField("Access-Control-Allow-Origin", "*");

    if(dsmResult->returnDataStatus == false)
    {
    ...
    if (dsmResult->returnDataLen > 0)
    {
        client->didReceiveData(handle, (const char*)dsmResult->returnData, sizeof(char)*dsmResult->returnDataLen, 0);
    }

#ifdef DEBUG_DSM_EXTENSION
    string filename = getFilename(string(dsmResult->pszUrl));
    system("mkdir /dtv/usb/sda1/dsmdump");
    filename.insert(0, "/dtv/usb/sda1/dsmdump/");
    writeToFile(filename.c_str(), dsmResult->returnData, dsmResult->returnDataLen);
#endif
...
static bool  dsmCallback(const void* handle1, const char* szUrl, unsigned char* body, long size, bool result)
{
    void* handle2 = const_cast<void*>(handle1);
    if (!handle2)
    {
        return false;
    }

    ResourceHandle* handle = static_cast<ResourceHandle*>(handle2);
    if (!handle)
    {
        return false;
    }
    ...
    CDsmRequestResult* dsmResult = new CDsmRequestResult(handle2, (char*)szUrl, body, size, result);
    d->m_idleHandler = g_timeout_add(0, processDsmData, dsmResult);
    return false;
}

static bool startDsm(ResourceHandle* handle, String urlString)
{
    if (g_soupDsmccFileRequest == NULL)
    {
        return false;
    }
    ...
    bool ret = g_soupDsmccFileRequest((const void*)handle, (const char*)urlString.utf8().data(), dsmCallback);
    if (ret == false)
    {
        return false;
    }
...
#endif // ENABLE_HBBTV ExtensionMap:1206
