...
TextBreakIterator* acquireLineBreakIterator(const UChar* string, int length, const AtomicString& locale)
{
    ...
    if ((string == NULL) || (length == 0) || (locale == 0)) {
        //fprintf(stderr,"ZDEBUG:[%s:%d] DUMA? latchup: string=NULL or length=0 or locale=NULL--------->>>\n",__func__,__LINE__);
        return 0;
    }
#endif
    ...
    if (iterator == NULL) {
        ...
        LOG_ERROR("ZDEBUG:[%s:%d] ICU latchup: iterator=NULL, string=%s, length=%d--------->>>\n",__func__,__LINE__,string, length);
        return 0;
    }
#endif
