...
PassOwnPtr<DirectDrawingAreaProxy> DirectDrawingAreaProxy::create(WebPageProxy* webPageProxy)
{
    return adoptPtr(new DirectDrawingAreaProxy(webPageProxy));
}

DirectDrawingAreaProxy::DirectDrawingAreaProxy(WebPageProxy* webPageProxy)
    : DrawingAreaProxy(DrawingAreaTypeDirect, webPageProxy)
    ...
    , m_discardBackingStoreTimer(RunLoop::current(), this, &DirectDrawingAreaProxy::discardBackingStore)
{
}

DirectDrawingAreaProxy::~DirectDrawingAreaProxy()
{
...
void DirectDrawingAreaProxy::paint(BackingStore::PlatformGraphicsContext context, const IntRect& rect, Region& unpaintedRegion)
{
...
bool DirectDrawingAreaProxy::paint(const WebCore::IntRect&, PlatformDrawingContext)
{
    ASSERT_NOT_REACHED();
    return false;
}

void DirectDrawingAreaProxy::sizeDidChange()
{
    backingStoreStateDidChange(RespondImmediately);
}

void DirectDrawingAreaProxy::visibilityDidChange()
{
...
void DirectDrawingAreaProxy::setPageIsVisible(bool)
{
}

void DirectDrawingAreaProxy::setBackingStoreIsDiscardable(bool isBackingStoreDiscardable)
{
...
void DirectDrawingAreaProxy::waitForBackingStoreUpdateOnNextPaint()
{
    m_hasReceivedFirstUpdate = true;
}

void DirectDrawingAreaProxy::update(uint64_t backingStoreStateID, const UpdateInfo& updateInfo)
{
...
void DirectDrawingAreaProxy::didUpdateBackingStoreState(uint64_t backingStoreStateID, const UpdateInfo& updateInfo, const LayerTreeContext& layerTreeContext)
{
...
void DirectDrawingAreaProxy::enterAcceleratedCompositingMode(uint64_t backingStoreStateID, const LayerTreeContext& layerTreeContext)
{
    ASSERT_ARG(backingStoreStateID, backingStoreStateID <= m_currentBackingStoreStateID);
    if (backingStoreStateID < m_currentBackingStoreStateID)
        return;
...
void DirectDrawingAreaProxy::exitAcceleratedCompositingMode(uint64_t backingStoreStateID, const UpdateInfo& updateInfo)
{
    ASSERT_ARG(backingStoreStateID, backingStoreStateID <= m_currentBackingStoreStateID);
    if (backingStoreStateID < m_currentBackingStoreStateID)
        return;
...
void DirectDrawingAreaProxy::incorporateUpdate(const UpdateInfo& updateInfo)
{
...
void DirectDrawingAreaProxy::backingStoreStateDidChange(RespondImmediatelyOrNot respondImmediatelyOrNot)
{
    ++m_nextBackingStoreStateID;
    sendUpdateBackingStoreState(respondImmediatelyOrNot);
}

void DirectDrawingAreaProxy::sendUpdateBackingStoreState(RespondImmediatelyOrNot respondImmediatelyOrNot)
{
    ASSERT(m_currentBackingStoreStateID < m_nextBackingStoreStateID);
    ...
    if (m_isWaitingForDidUpdateBackingStoreState && !m_layerTreeContext.isEmpty()) {
        // Wait for the DidUpdateBackingStoreState message. Normally we do this in DirectDrawingAreaProxy::paint, but that
...
void DirectDrawingAreaProxy::waitForAndDispatchDidUpdateBackingStoreState()
{
    ASSERT(m_isWaitingForDidUpdateBackingStoreState);
...
#if USE(ACCELERATED_COMPOSITING)
void DirectDrawingAreaProxy::enterAcceleratedCompositingMode(const LayerTreeContext& layerTreeContext)
{
    ASSERT(!isInAcceleratedCompositingMode());
...
void DirectDrawingAreaProxy::exitAcceleratedCompositingMode()
{
    ASSERT(isInAcceleratedCompositingMode());
...
void DirectDrawingAreaProxy::discardBackingStoreSoon()
{
...
void DirectDrawingAreaProxy::discardBackingStore()
{
    m_backingStore = nullptr;
    backingStoreStateDidChange(DoNotRespondImmediately);
}
