...
/* @20101001-sanggyu: time adjusting */
time_t vdtime(time_t* t)
{
    static int isFirstTime = 1;
    static double baseTime = .0f;

    time_t r;

    if (isFirstTime) {
        char* strBaseTime = getenv("BASE_TIME");
        if (strBaseTime) {
            sscanf(strBaseTime, "%lf", &baseTime);
        }
        isFirstTime = 0;
    }
...
{
	return soup_date_new_from_time_t (vdtime (NULL) + offset_seconds);
}
...
soup_date_is_past (SoupDate *date)
{
    ...
    //fprintf(stderr, "[BR] %s %d %s\n", __func__, __LINE__, soup_date_to_string(date, SOUP_DATE_COOKIE));
    //fprintf(stderr, "[BR] %s %d %d %d\n", __func__, __LINE__, (int)soup_date_to_time_t(date), (int)vdtime(NULL));

	/* optimization */
	if (date->year < 2008)
		return TRUE;

	return soup_date_to_time_t (date) < vdtime (NULL);
}
