...
/* add for get average signal strength from scanned AP--Tuesday, July 10 2012*/
int total_found_AP_num = 0;
double average_signal_strength = 0.0;
int print_av_signal_flag = 0;
int cur_found_freq_num = 0;
SIGNAL_LIST *channel_signal[TOTAL_CHAN_MAX] = {NULL, };
...
int main(int argc, char **argv)
{
 ...
 detect:
	...
	if (print_av_signal_flag)
	{
		...
		printf("\n\n");
		printf("Average RSSI Statistics:\n\n");
	
		for (index=0; index < cur_found_freq_num; index++)
		{
			if(channel_signal[index]) printf("freq=%d\t", channel_signal[index]->freq);
			total_found_AP_num = delete_signal_node(&channel_signal[index], &average_signal_strength);
			printf("average signal: %8.2f dBm\t(%d valid AP(s) found)\n", 10*log10(average_signal_strength), total_found_AP_num);
		}
		printf("\n\n");
	}
