...
static int mms_tcp_connect(mms_io_t *io, TYPE *this)
{
  ...
  this->s = io_connect(io, this->connect_host, this->connect_port, this->set_stop);	/***** smins.lee : call stop, 2011-06-16 *****/
  if (this->s == -1) {
...
static void interp_asf_header(TYPE *this)
{
  ...
  while ((i + 24) <= this->asf_header_len) {
    ...
    if (((i + length) > this->asf_header_len) && (GUID_ASF_DATA != guid)) return;	/***** smins.lee : No GUID_ASF_DATA error, 2011-07-01 *****/
    ...
    switch (guid) {
      ...
      case GUID_ASF_DATA:
        this->asf_num_packets = LE_64(this->asf_header + i + 40);	/***** smins.lee : No GUID_ASF_DATA error, 2011-07-01 *****/
        lprintf("num_packets: %d\n", (int)this->asf_num_packets);
        if ((i + length) > this->asf_header_len)	/***** smins.lee : No GUID_ASF_DATA error, 2011-07-01 *****/
        {
        	return;
        }
        break;
