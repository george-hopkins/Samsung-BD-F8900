...
/* flag : 0(MMS->MMSH), 1(MMSH), 2(MMSH->MMS), 3(MMS) */
...
int       mmsx_read (mms_io_t *io, mmsx_t *instance, char *data, int len, int * abort_request);	/***** smins.lee : call stop, 2011-07-18 *****/
int       mmsx_time_seek (mms_io_t *io, mmsx_t *instance, double time_sec, int * abort_request);	/***** smins.lee : call stop, 2011-07-18 *****/
mms_off_t mmsx_seek (mms_io_t *io, mmsx_t *instance, mms_off_t offset, int origin, int * abort_request);	/***** smins.lee : call stop, 2011-07-18 *****/
...
char * mmsx_get_url (mmsx_t * instance);	/***** smins.lee : get_url, 2011-03-25 *****/
int mmsx_get_network_fail (mmsx_t * instance);	/***** smins.lee : fallback_io_read select error, 2011-06-16 *****/
int mmsx_set_network_fail (mmsx_t * instance, int flag);	/***** smins.lee : fallback_io_read select error, 2011-06-16 *****/
int mmsx_get_type (mmsx_t * mmsx);	/***** start for smins.lee : seek error after eos, 2011-07-02 *****/
