...
#ifdef LIBMMS_DEBUG
#define lprintf(...) \
        printf ("   mmsh[%s]<%d>: ", __FUNCTION__, __LINE__), printf (__VA_ARGS__)
#else
#define lprintf(...) \
...
struct mmsh_s {
  ...
  off_t         current_pos;
  int           bandwidth;
  int           interp_asf_header_done;	/***** smins.lee : asf_packet_len error before interp_asf_header, 2011-03-25 *****/
...
static int send_command(mms_io_t *io, mmsh_t *this, char *cmd)
{
  ...
  length = strlen(cmd);
  if (io_write(io, this->s, cmd, length, this->set_stop) != length) {	/***** smins.lee : call stop, 2011-07-18 *****/
    lprintf("send error.\n");
    return 0;
  }
  return 1;
...
static int get_answer (mms_io_t *io, mmsh_t *this) {
  ...
  while (!done) {
    ...
    if (io_read(io, this->s, &(this->buf[len]), 1, &this->network_fail, this->set_stop) != 1) {	/***** smins.lee : fallback_io_read select error, 2011-06-16, call stop, 2011-07-18 *****/
...
static int get_chunk_header (mms_io_t *io, mmsh_t *this) {
  ...
  /* read chunk header */
  read_len = io_read(io, this->s, chunk_header, CHUNK_HEADER_LENGTH, &this->network_fail, this->set_stop);	/***** smins.lee : fallback_io_read select error, 2011-06-16, call stop, 2011-07-18 *****/
  ...
  if (ext_header_len > 0) {
    read_len = io_read (io, this->s, ext_header, ext_header_len, &this->network_fail, this->set_stop);	/***** smins.lee : fallback_io_read select error, 2011-06-16, call stop, 2011-07-18 *****/
...
static int get_header (mms_io_t *io, mmsh_t *this) {
      ...
      if (this->chunk_type == CHUNK_TYPE_ASF_HEADER) {
        if ((this->asf_header_len + this->chunk_length) > ASF_HEADER_SIZE) {
          ...
          len = io_read(io, this->s, this->asf_header + this->asf_header_len,
                        this->chunk_length, &this->network_fail, this->set_stop);	/***** smins.lee : fallback_io_read select error, 2011-06-16, call stop, 2011-07-18 *****/
          if (len > 0)
  ...
  if (this->chunk_type == CHUNK_TYPE_DATA) {
    ...
    len = io_read (io, this->s, this->buf, this->chunk_length, &this->network_fail, this->set_stop);	/***** smins.lee : fallback_io_read select error, 2011-06-16, call stop, 2011-07-18 *****/
    ...
    if (len != this->chunk_length) {
      ...
      if ((this->chunk_length > this->asf_packet_len) &&
        ((0 != this->asf_packet_len) || (0 != this->interp_asf_header_done))) {	/***** smins.lee : asf_packet_len error before interp_asf_header, 2011-03-25 *****/
...
static int mmsh_connect_int (mms_io_t *io, mmsh_t *this, off_t seek, uint32_t time_seek) {
  ...
  this->interp_asf_header_done = 0;	/***** smins.lee : asf_packet_len error before interp_asf_header, 2011-03-25 *****/
  /* Don't check for != MMSH_SUCCESS as EOS is normal here too */    
  if (get_header(io, this) == MMSH_ERROR)
    goto fail;

  interp_asf_header(this);
  this->interp_asf_header_done = 1;	/***** smins.lee : asf_packet_len error before interp_asf_header, 2011-03-25 *****/
...
mmsh_t *mmsh_connect (mms_io_t *io, void *data, const char *url, int bandwidth, int * abort_request) {	/***** smins.lee : call stop, 2011-07-18 *****/
  mmsh_t *this;
  GURI  *uri = NULL;
  GURI  *proxy_uri = NULL;
  char  *proxy_env;
  ...
  this->url             = strdup(url);
//--> start for smins.lee : http_proxy, 2011-03-25
#ifdef LIBMMS_HTTP_PROXY
  if ((proxy_env = getenv("http_proxy")) != NULL)
    this->proxy_url = strdup(proxy_env);
#endif
//<-- end for smins.lee : http_proxy, 2011-03-25
  this->s               = -1;
...
int mmsh_read (mms_io_t *io, mmsh_t *this, char *data, int len, int * abort_request) {	/***** smins.lee : call stop, 2011-07-18 *****/
  int total;
    ...
    if (this->asf_header_read < this->asf_header_len) {
      ...
      if (bytes_left == 0) {
        ...
        case MMSH_SUCCESS:
          break;
        case MMSH_ERROR:
          lprintf("get_media_packet failed return[%d/%d]\n", total, len);	/***** smins.lee : debug, 2011-03-25 *****/
          return total;
        case EOS:
          return total;
...
off_t mmsh_seek (mms_io_t *io, mmsh_t *this, off_t offset, int origin, int * abort_request) {	/***** smins.lee : call stop, 2011-07-18 *****/
  off_t dest;
...
int mmsh_time_seek (mms_io_t *io, mmsh_t *this, double time_sec, int * abort_request) {	/***** smins.lee : call stop, 2011-07-18 *****/
  uint32_t orig_asf_header_len = this->asf_header_len;
  uint32_t orig_asf_packet_len = this->asf_packet_len;
...
int mmsh_get_network_fail (mmsh_t * this)
{
	return this->network_fail;
}


int mmsh_set_network_fail (mmsh_t * this, int flag)
{
	if (this != NULL)
	{
