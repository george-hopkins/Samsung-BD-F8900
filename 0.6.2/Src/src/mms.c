...
#include <stdlib.h>
#include <time.h>
#include <sys/time.h>	/***** smins.lee : select, 2011-03-25 *****/
//--> start for smins.lee : iconv, 2011-03-25
#if defined(HAVE_ICONV) && defined(HAVE_LANGINFO_CODESET)
#include <iconv.h>
#endif
//<-- end for smins.lee : iconv, 2011-03-25

/********** logging **********/
//--> start for smins.lee : libmms debug, 2011-03-25
#ifdef LIBMMS_DEBUG
#define lprintf(...) \
        printf ("   mms[%s]<%d>: ", __FUNCTION__, __LINE__), printf (__VA_ARGS__)
#else
#define lprintf(...) \
...
struct mms_s {
  ...
  off_t         current_pos;
  int           eos;
  int          *set_stop;	/***** smins.lee : call stop, 2011-07-18 *****/
  int           network_fail;	/***** smins.lee : fallback_io_read select error, 2011-06-16 *****/
};
...
static off_t fallback_io_read(void *data, int socket, char *buf, off_t num, int * network_fail, int * set_stop)	/***** smins.lee : fallback_io_read select error, 2011-06-16, call stop, 2011-07-18 *****/
{
  off_t len = 0, ret;
//--> start for smins.lee : select, 2011-03-25
#if 1
#define FALLBACK_IO_READ_TIMEOUT	10
#define FALLBACK_IO_READ_SELECT_TIMEOUT_SEC	0
#define FALLBACK_IO_READ_SELECT_TIMEOUT_USEC	100000
#define FALLBACK_IO_READ_COUNT_MAX	100
  struct timeval tvSelect = {FALLBACK_IO_READ_SELECT_TIMEOUT_SEC, FALLBACK_IO_READ_SELECT_TIMEOUT_USEC};
  struct timeval tvSelectStart = {0, 0};
  struct timeval tvSelectCurrent = {0, 0};
  fd_set set;
  int count = 0;
#endif
//<-- end for smins.lee : select, 2011-03-25
...
//--> start for smins.lee : select, 2011-03-25
#if 1
	if (1 == *set_stop)	/***** smins.lee : call stop, 2011-07-18 *****/
	{
		lprintf ("stop call\n");
		return len ? len : -1;
	}
	...
	FD_ZERO (&set);
	FD_SET (socket, &set);
	tvSelect.tv_sec = FALLBACK_IO_READ_SELECT_TIMEOUT_SEC;
	tvSelect.tv_usec = FALLBACK_IO_READ_SELECT_TIMEOUT_USEC;
	ret = select (socket + 1, &set, NULL, NULL, &tvSelect);
	if ((0 != ret) || (0 == count))
	{
		count = 1;
		gettimeofday (&tvSelectStart , NULL);
		tvSelectCurrent.tv_sec = tvSelectStart.tv_sec;
	}
	else
	{
		count++;
		if (0 != gettimeofday (&tvSelectCurrent , NULL))
		{
			lprintf ("gettimeofday error\n");
			tvSelectCurrent.tv_sec += FALLBACK_IO_READ_SELECT_TIMEOUT_SEC;
			tvSelectCurrent.tv_usec += FALLBACK_IO_READ_SELECT_TIMEOUT_USEC;
			if (1000000 <= tvSelectCurrent.tv_usec)
			{
				tvSelectCurrent.tv_sec += 1;
				tvSelectCurrent.tv_usec -= 1000000;
			}
		}
	}
	
	if ((0 < ret) && FD_ISSET (socket, &set))
	{
		ret = (off_t)read (socket, buf + len, num - len);
		if (0 > ret)
		{
			lprintf ("read error @ len = %lld: err[%d]%s\n", (long long int)ret, get_errno(), strerror (get_errno()));
			switch (get_errno())
			{
				case EINTR :
				case EAGAIN :
		...
		{
			lprintf ("end of file ret[%lld/%lld]: by read\n", (long long int)len, (long long int)num);
			return len;
		}
		else
		{
	...
	if ((FALLBACK_IO_READ_COUNT_MAX < count) || (FALLBACK_IO_READ_TIMEOUT <= (tvSelectCurrent.tv_sec  - tvSelectStart.tv_sec)))
	{
		lprintf ("socket read TIMEOUT\n");
		*network_fail = 1;
		return len ? len : ret;
	}
	else
	{
#ifdef CNX_NOFIX_USLEEP_SIGBLOCK_BUS_SEGV_ABRT_ILL
		usleep (1000);
#else
		usleep (4000); /* minimum 4ms */
#endif
	}
	continue;
#if 0
...
static off_t fallback_io_write(void *data, int socket, char *buf, off_t num, int * set_stop)	/***** smins.lee : call stop, 2011-07-18 *****/
{
//--> start for smins.lee : select, 2011-03-25
#if 1
#define FALLBACK_IO_WRITE_TIMEOUT	10
#define FALLBACK_IO_WRITE_SELECT_TIMEOUT_SEC	0
#define FALLBACK_IO_WRITE_SELECT_TIMEOUT_USEC	100000
#define FALLBACK_IO_WRITE_COUNT_MAX	100
  struct timeval tvSelect = {FALLBACK_IO_WRITE_SELECT_TIMEOUT_SEC, FALLBACK_IO_WRITE_SELECT_TIMEOUT_USEC};
  struct timeval tvSelectStart = {0, 0};
  struct timeval tvSelectCurrent = {0, 0};
  fd_set set;
  off_t len = 0;
...
//--> start for smins.lee : select, 2011-03-25
	...
	while (len < num)
	{
		...
		FD_ZERO (&set);
		FD_SET (socket, &set);
		tvSelect.tv_sec = FALLBACK_IO_WRITE_SELECT_TIMEOUT_SEC;
		tvSelect.tv_usec = FALLBACK_IO_WRITE_SELECT_TIMEOUT_USEC;
		ret = select (socket + 1, NULL, &set, NULL, &tvSelect);
		if ((0 != ret) || (0 == count))
		{
			count = 1;
			gettimeofday (&tvSelectStart , NULL);
		...
		else
		{
			...
			{
				lprintf ("gettimeofday error\n");
				tvSelectCurrent.tv_sec += FALLBACK_IO_WRITE_SELECT_TIMEOUT_SEC;
				tvSelectCurrent.tv_usec += FALLBACK_IO_WRITE_SELECT_TIMEOUT_USEC;
				if (1000000 <= tvSelectCurrent.tv_usec)
				{
					tvSelectCurrent.tv_sec += 1;
					tvSelectCurrent.tv_usec -= 1000000;
		...
		if ((0 < ret) && FD_ISSET (socket, &set))
		{
			ret = (off_t)write (socket, buf + len, num - len);
			if (0 > ret)
			{
				lprintf ("write error @ len = %lld: err[%d]%s\n", (long long int)ret, get_errno(), strerror (get_errno()));
				switch (get_errno())
				{
					case EINTR :
					case EAGAIN :
			...
			{
				lprintf ("nothing was written ret[%lld/%lld] : by write\n", (long long int)len, (long long int)num);
				return len;
			}
			else
			{
		...
		if ((FALLBACK_IO_WRITE_COUNT_MAX < count) || (FALLBACK_IO_WRITE_TIMEOUT <= (tvSelectCurrent.tv_sec  - tvSelectStart.tv_sec)))
		{
			lprintf ("socket write TIMEOUT\n");
			return len ? len : ret;
		}
		else
		{
...
static int fallback_io_tcp_connect(void *data, const char *host, int port, int * set_stop)	/***** smins.lee : call stop, 2011-06-16 *****/
{
...
//--> start for smins.lee : select, 2011-03-25
#if 1
#define FALLBACK_IO_TCP_CONNECT_TIMEOUT	5
#define FALLBACK_IO_TCP_CONNECT_SELECT_TIMEOUT_SEC	0
#define FALLBACK_IO_TCP_CONNECT_SELECT_TIMEOUT_USEC	100000
#define FALLBACK_IO_TCP_CONNECT_COUNT_MAX	50
  struct timeval tvSelect = {FALLBACK_IO_TCP_CONNECT_SELECT_TIMEOUT_SEC, FALLBACK_IO_TCP_CONNECT_SELECT_TIMEOUT_USEC};
  struct timeval tvSelectStart = {0, 0};
  struct timeval tvSelectCurrent = {0, 0};
  fd_set set;
...
//--> start for smins.lee : select, 2011-03-25
		...
		if ((-1 != connect(s, r->ai_addr, r->ai_addrlen)) && (EINPROGRESS != errno))
		{
			lprintf ("connect error\n");
			closesocket (s);
			break;
		}
		
		count = 0;
		if (0 != gettimeofday (&tvSelectStart , NULL))
		{
			lprintf ("gettimeofday error\n");
			tvSelectStart.tv_sec = 0;
		}
		tvSelectCurrent.tv_sec = tvSelectStart.tv_sec;
		while ((FALLBACK_IO_TCP_CONNECT_COUNT_MAX >= ++count) && (FALLBACK_IO_TCP_CONNECT_TIMEOUT > (tvSelectCurrent.tv_sec  - tvSelectStart.tv_sec)))
		{
			if (1 == *set_stop)	/***** smins.lee : call stop, 2011-07-18 *****/
			{
				lprintf ("stop call\n");
			...
			FD_ZERO (&set);
			FD_SET (s, &set);
			tvSelect.tv_sec = FALLBACK_IO_TCP_CONNECT_SELECT_TIMEOUT_SEC;
			tvSelect.tv_usec = FALLBACK_IO_TCP_CONNECT_SELECT_TIMEOUT_USEC;
			ret = select (s + 1, NULL, &set, NULL, &tvSelect);
			if (0 != ret)
			{
				break;
			}
			else if (0 != gettimeofday (&tvSelectCurrent , NULL))
			{
				lprintf ("gettimeofday error\n");
				tvSelectCurrent.tv_sec += FALLBACK_IO_TCP_CONNECT_SELECT_TIMEOUT_SEC;
				tvSelectCurrent.tv_usec += FALLBACK_IO_TCP_CONNECT_SELECT_TIMEOUT_USEC;
				if (1000000 <= tvSelectCurrent.tv_usec)
				{
					tvSelectCurrent.tv_sec += 1;
					tvSelectCurrent.tv_usec -= 1000000;
		...
		if (0 > ret)
		{
			lprintf ("select[%d] < 0, error ret[-1]\n", ret);
			closesocket (s);
			break;
		}
		else if (0 == ret)
		{
			lprintf ("select[0], next address continue\n");
			closesocket (s);
#ifdef CNX_NOFIX_USLEEP_SIGBLOCK_BUS_SEGV_ABRT_ILL
			usleep (1000);
#else
		...
		ret = getsockopt (s, SOL_SOCKET, SO_ERROR, (char *)&err, &len);
		if ((0 > ret) || (0 > err))
		{
			lprintf ("getsockopt error\n");
			closesocket (s);
			break;
		}
...
static int send_command (mms_io_t *io, mms_t *this, int command,
  ...
  n = io_write(io,  this->s, this->scmd, len8 * 8 + CMD_HEADER_LEN + CMD_PREFIX_LEN, this->set_stop);	/***** smins.lee : call stop, 2011-07-18 *****/
...
#else
static int string_utf16 (int unused, char * dest, char * src, int dest_len)
{
	char * ip = src;
	char * op = dest;
	size_t ip_len = strlen (src);
...
static int get_packet_header (mms_io_t *io, mms_t *this, mms_packet_header_t *header) {
  ...
  len = io_read(io,  this->s, this->buf, 8, &this->network_fail, this->set_stop);	/***** smins.lee : fallback_io_read select error, 2011-06-16, call stop, 2011-07-18 *****/
  ...
  if (LE_32(this->buf + 4) == 0xb00bface) {
    ...
    len = io_read(io,  this->s, this->buf + 8, 4, &this->network_fail, this->set_stop);	/***** smins.lee : fallback_io_read select error, 2011-06-16, call stop, 2011-07-18 *****/
    if (len != 4)
      goto error;
...
static int get_packet_command (mms_io_t *io, mms_t *this, uint32_t packet_len) {
  ...
  len = io_read(io,  this->s, this->buf + 12, packet_len, &this->network_fail, this->set_stop);	/***** smins.lee : fallback_io_read select error, 2011-06-16, call stop, 2011-07-18 *****/
...
static int get_asf_header (mms_io_t *io, mms_t *this) {
  ...
  while (!stop) {
    if (1 == *this->set_stop)	/***** smins.lee : call stop, 2011-07-18 *****/
    {
      lprintf ("stop call\n");
      return 0;
    }
    ...
    switch (get_packet_header (io, this, &header)) {
      ...
      case MMS_PACKET_ASF_PACKET:
        ...
        len = io_read(io,  this->s,
                              this->asf_header + this->asf_header_len, header.packet_len, &this->network_fail, this->set_stop);	/***** smins.lee : fallback_io_read select error, 2011-06-16, call stop, 2011-07-18 *****/
 ...
 * return 0 on error
 */
static int mms_choose_best_streams(mms_io_t *io, mms_t *this) {
  int     i;
  int     video_stream = -1;
  int     audio_stream = -1;
  int     res;
...
mms_t *mms_connect (mms_io_t *io, void *data, const char *url, int bandwidth, int * abort_request) {	/***** smins.lee : call stop, 2011-07-18 *****/
//--> start for smins.lee : iconv, 2011-03-25
#if defined(HAVE_ICONV) && defined(HAVE_LANGINFO_CODESET)
...
//<-- end for smins.lee : iconv, 2011-03-25
  ...
  this->set_stop        = abort_request;	/***** smins.lee : call stop, 2011-07-18 *****/
  this->network_fail	= 0;	/***** smins.lee : fallback_io_read select error, 2011-06-16 *****/
...
#else
  res = string_utf16 (-1, this->scmd_body + command_buffer.pos, this->str,
                     CMD_BODY_LEN - command_buffer.pos);
#endif
//<-- end for smins.lee : iconv, 2011-03-25
  if(!res)
...
//--> start for smins.lee : iconv, 2011-03-25
  ...
  res = string_utf16(url_conv, this->scmd_body + command_buffer.pos,
                     ...
                     CMD_BODY_LEN - command_buffer.pos);
#else
  res = string_utf16 (-1, this->scmd_body + command_buffer.pos,
                     "\\\\192.168.0.129\\TCP\\1037",
                     CMD_BODY_LEN - command_buffer.pos);
#endif
//<-- end for smins.lee : iconv, 2011-03-25
    ...
    case MMS_PACKET_ASF_PACKET:
      {
        ...
        len = io_read(io,  this->s, this->buf, header.packet_len, &this->network_fail, this->set_stop);	/***** smins.lee : fallback_io_read select error, 2011-06-16, call stop, 2011-07-18 *****/
        if (len != header.packet_len) {
...
int mms_read (mms_io_t *io, mms_t *this, char *data, int len, int * abort_request) {	/***** smins.lee : call stop, 2011-07-18 *****/
  int total;
...
int mms_time_seek (mms_io_t *io, mms_t *this, double time_sec, int * abort_request) {		/***** smins.lee : call stop, 2011-07-18 *****/
  if (1 == *abort_request)	/***** smins.lee : call stop, 2011-07-18 *****/
  {
    lprintf ("stop call\n");
    return 0;
...
off_t mms_seek (mms_io_t *io, mms_t *this, off_t offset, int origin, int * abort_request) {	/***** smins.lee : call stop, 2011-07-18 *****/
  off_t dest;
...
int mms_get_network_fail (mms_t * this)
{
	return this->network_fail;
}


int mms_set_network_fail (mms_t * this, int flag)
{
	if (this != NULL)
	{
		this->network_fail = flag ? 1 : 0;
		return 1;
	}
	else
	{
